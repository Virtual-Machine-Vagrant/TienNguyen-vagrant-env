...
#define NS_DECL_NON_VIRTUAL_NSISUPPORTSARRAY \
  NS_METHOD_(bool) Equals(const nsISupportsArray *other); \
  NS_METHOD_(int32_t) IndexOf(const nsISupports *aPossibleElement); \
  NS_METHOD_(int32_t) IndexOfStartingAt(const nsISupports *aPossibleElement, uint32_t aStartIndex); \
  NS_METHOD_(int32_t) LastIndexOf(const nsISupports *aPossibleElement); \
  NS_METHOD GetIndexOf(nsISupports *aPossibleElement, int32_t *_retval); \
  NS_METHOD GetIndexOfStartingAt(nsISupports *aPossibleElement, uint32_t aStartIndex, int32_t *_retval); \
  NS_METHOD GetLastIndexOf(nsISupports *aPossibleElement, int32_t *_retval); \
  ...
  NS_METHOD_(bool) RemoveLastElement(const nsISupports *aElement); \
  NS_METHOD DeleteLastElement(nsISupports *aElement); \
  NS_METHOD DeleteElementAt(uint32_t aIndex); \
  NS_METHOD_(bool) AppendElements(nsISupportsArray *aElements); \
  NS_METHOD Compact(void); \
  NS_METHOD Clone(nsISupportsArray * *_retval); \
  NS_METHOD_(bool) MoveElement(int32_t aFrom, int32_t aTo); \
  NS_METHOD_(bool) InsertElementsAt(nsISupportsArray *aOther, uint32_t aIndex); \
  NS_METHOD_(bool) RemoveElementsAt(uint32_t aIndex, uint32_t aCount); \
  NS_METHOD_(bool) SizeTo(int32_t aSize); 
