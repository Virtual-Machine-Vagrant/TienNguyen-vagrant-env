...
#define NS_DECL_NSIFEEDRESULTLISTENER \
  NS_IMETHOD HandleResult(nsIFeedResult *result) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIFEEDRESULTLISTENER \
  NS_METHOD HandleResult(nsIFeedResult *result); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIFEEDRESULTLISTENER(_to) \
  NS_IMETHOD HandleResult(nsIFeedResult *result) override { return _to HandleResult(result); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIFEEDRESULTLISTENER(_to) \
  NS_IMETHOD HandleResult(nsIFeedResult *result) override { return !_to ? NS_ERROR_NULL_POINTER : _to->HandleResult(result); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsFeedResultListener, nsIFeedResultListener)
...
#define NS_DECL_NSIFEEDPROGRESSLISTENER \
  NS_IMETHOD ReportError(const nsAString & errorText, int32_t lineNumber, bool bozo) override; \
  NS_IMETHOD HandleStartFeed(nsIFeedResult *result) override; \
  NS_IMETHOD HandleFeedAtFirstEntry(nsIFeedResult *result) override; \
  NS_IMETHOD HandleEntry(nsIFeedEntry *entry, nsIFeedResult *result) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIFEEDPROGRESSLISTENER \
  NS_METHOD ReportError(const nsAString & errorText, int32_t lineNumber, bool bozo); \
  NS_METHOD HandleStartFeed(nsIFeedResult *result); \
  NS_METHOD HandleFeedAtFirstEntry(nsIFeedResult *result); \
  NS_METHOD HandleEntry(nsIFeedEntry *entry, nsIFeedResult *result); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIFEEDPROGRESSLISTENER(_to) \
  NS_IMETHOD ReportError(const nsAString & errorText, int32_t lineNumber, bool bozo) override { return _to ReportError(errorText, lineNumber, bozo); } \
  NS_IMETHOD HandleStartFeed(nsIFeedResult *result) override { return _to HandleStartFeed(result); } \
  NS_IMETHOD HandleFeedAtFirstEntry(nsIFeedResult *result) override { return _to HandleFeedAtFirstEntry(result); } \
  NS_IMETHOD HandleEntry(nsIFeedEntry *entry, nsIFeedResult *result) override { return _to HandleEntry(entry, result); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIFEEDPROGRESSLISTENER(_to) \
  NS_IMETHOD ReportError(const nsAString & errorText, int32_t lineNumber, bool bozo) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ReportError(errorText, lineNumber, bozo); } \
  NS_IMETHOD HandleStartFeed(nsIFeedResult *result) override { return !_to ? NS_ERROR_NULL_POINTER : _to->HandleStartFeed(result); } \
  NS_IMETHOD HandleFeedAtFirstEntry(nsIFeedResult *result) override { return !_to ? NS_ERROR_NULL_POINTER : _to->HandleFeedAtFirstEntry(result); } \
  NS_IMETHOD HandleEntry(nsIFeedEntry *entry, nsIFeedResult *result) override { return !_to ? NS_ERROR_NULL_POINTER : _to->HandleEntry(entry, result); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsFeedProgressListener, nsIFeedProgressListener)
