...
#define NS_DECL_NON_VIRTUAL_NSIIDENTITYCRYPTOSERVICE \
  NS_METHOD GenerateKeyPair(const nsACString & algorithm, nsIIdentityKeyGenCallback *callback); \
  NS_METHOD Base64UrlEncode(const nsACString & toEncode, nsACString & _retval); 
...
#define NS_DECL_NON_VIRTUAL_NSIIDENTITYKEYPAIR \
  NS_METHOD GetKeyType(nsACString & aKeyType); \
  NS_METHOD GetHexRSAPublicKeyExponent(nsACString & aHexRSAPublicKeyExponent); \
  NS_METHOD GetHexRSAPublicKeyModulus(nsACString & aHexRSAPublicKeyModulus); \
  NS_METHOD GetHexDSAPrime(nsACString & aHexDSAPrime); \
  NS_METHOD GetHexDSASubPrime(nsACString & aHexDSASubPrime); \
  NS_METHOD GetHexDSAGenerator(nsACString & aHexDSAGenerator); \
  NS_METHOD GetHexDSAPublicValue(nsACString & aHexDSAPublicValue); \
  NS_METHOD Sign(const nsACString & aText, nsIIdentitySignCallback *callback); 
...
#define NS_DECL_NON_VIRTUAL_NSIIDENTITYKEYGENCALLBACK \
  NS_METHOD GenerateKeyPairFinished(nsresult rv, nsIIdentityKeyPair *keyPair); 
...
#define NS_DECL_NON_VIRTUAL_NSIIDENTITYSIGNCALLBACK \
  NS_METHOD SignFinished(nsresult rv, const nsACString & base64urlSignature); 
