...
#define NS_DECL_NON_VIRTUAL_NSIHTTPAUTHENTICABLECHANNEL \
  NS_METHOD GetIsSSL(bool *aIsSSL); \
  NS_METHOD GetProxyMethodIsConnect(bool *aProxyMethodIsConnect); \
  NS_METHOD Cancel(nsresult aStatus); \
  NS_METHOD GetLoadFlags(nsLoadFlags *aLoadFlags); \
  ...
  NS_METHOD GetServerResponseHeader(nsACString & aServerResponseHeader); \
  NS_METHOD GetProxyChallenges(nsACString & aProxyChallenges); \
  NS_METHOD GetWWWChallenges(nsACString & aWWWChallenges); \
  NS_METHOD SetProxyCredentials(const nsACString & credentials); \
  NS_METHOD SetWWWCredentials(const nsACString & credentials); \
  NS_METHOD OnAuthAvailable(void); \
  NS_METHOD OnAuthCancelled(bool userCancel); 
