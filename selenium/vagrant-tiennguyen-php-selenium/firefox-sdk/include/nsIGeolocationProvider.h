...
#define NS_DECL_NON_VIRTUAL_NSIGEOLOCATIONUPDATE \
  NS_METHOD Update(nsIDOMGeoPosition *position); \
  NS_METHOD NotifyError(uint16_t error); 
...
#define NS_DECL_NON_VIRTUAL_NSIGEOLOCATIONPROVIDER \
  NS_METHOD Startup(void); \
  NS_METHOD Watch(nsIGeolocationUpdate *callback); \
  NS_METHOD Shutdown(void); \
  NS_METHOD SetHighAccuracy(bool enable); 
