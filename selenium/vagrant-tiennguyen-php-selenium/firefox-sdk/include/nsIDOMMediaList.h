...
#define NS_DECL_NON_VIRTUAL_NSIDOMMEDIALIST \
  NS_METHOD GetMediaText(nsAString & aMediaText); \
  NS_METHOD SetMediaText(const nsAString & aMediaText); \
  NS_METHOD GetLength(uint32_t *aLength); \
  NS_METHOD Item(uint32_t index, nsAString & _retval); \
  NS_METHOD DeleteMedium(const nsAString & oldMedium); \
  NS_METHOD AppendMedium(const nsAString & newMedium); 
