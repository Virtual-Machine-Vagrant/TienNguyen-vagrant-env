...
#define NS_DECL_NON_VIRTUAL_NSIHANGREPORT \
  NS_METHOD GetHangType(uint32_t *aHangType); \
  NS_METHOD GetScriptBrowser(nsIDOMElement * *aScriptBrowser); \
  NS_METHOD GetScriptFileName(nsACString & aScriptFileName); \
  NS_METHOD GetScriptLineNo(uint32_t *aScriptLineNo); \
  NS_METHOD GetPluginName(nsACString & aPluginName); \
  NS_METHOD UserCanceled(void); \
  NS_METHOD TerminateScript(void); \
  NS_METHOD TerminatePlugin(void); \
  NS_METHOD BeginStartingDebugger(void); \
  NS_METHOD EndStartingDebugger(void); \
  NS_METHOD IsReportForBrowser(nsIFrameLoader *aFrameLoader, bool *_retval); 
