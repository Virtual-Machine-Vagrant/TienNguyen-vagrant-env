...
#define NS_DECL_NON_VIRTUAL_NSIDOMPARSER \
  NS_METHOD ParseFromString(const char16_t * str, const char * contentType, nsIDOMDocument * *_retval); \
  NS_METHOD ParseFromBuffer(const uint8_t *buf, uint32_t bufLen, const char * contentType, nsIDOMDocument * *_retval); \
  NS_METHOD ParseFromStream(nsIInputStream *stream, const char * charset, int32_t contentLength, const char * contentType, nsIDOMDocument * *_retval); \
  NS_METHOD Init(nsIPrincipal *principal, nsIURI *documentURI, nsIURI *baseURI, nsIGlobalObject *scriptObject); 
