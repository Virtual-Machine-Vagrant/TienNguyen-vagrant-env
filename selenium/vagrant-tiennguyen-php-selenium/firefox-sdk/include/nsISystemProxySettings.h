...
#define NS_DECL_NON_VIRTUAL_NSISYSTEMPROXYSETTINGS \
  NS_METHOD GetMainThreadOnly(bool *aMainThreadOnly); \
  NS_METHOD GetPACURI(nsACString & aPACURI); \
  NS_METHOD GetProxyForURI(const nsACString & testSpec, const nsACString & testScheme, const nsACString & testHost, int32_t testPort, nsACString & _retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsSystemProxySettings, nsISystemProxySettings)
