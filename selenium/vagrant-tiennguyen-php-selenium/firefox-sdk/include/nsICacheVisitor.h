...
#define NS_DECL_NON_VIRTUAL_NSICACHEVISITOR \
  NS_METHOD VisitDevice(const char * deviceID, nsICacheDeviceInfo *deviceInfo, bool *_retval); \
  NS_METHOD VisitEntry(const char * deviceID, nsICacheEntryInfo *entryInfo, bool *_retval); 
...
#define NS_DECL_NON_VIRTUAL_NSICACHEDEVICEINFO \
  ...
  NS_METHOD GetUsageReport(char * *aUsageReport); \
  NS_METHOD GetEntryCount(uint32_t *aEntryCount); \
  NS_METHOD GetTotalSize(uint32_t *aTotalSize); \
  NS_METHOD GetMaximumSize(uint32_t *aMaximumSize); 
...
#define NS_DECL_NON_VIRTUAL_NSICACHEENTRYINFO \
  NS_METHOD GetClientID(char * *aClientID); \
  NS_METHOD GetDeviceID(char * *aDeviceID); \
  ...
  NS_METHOD GetDataSize(uint32_t *aDataSize); \
  NS_METHOD IsStreamBased(bool *_retval); 
