...
#define NS_DECL_NON_VIRTUAL_NSIZIPENTRY \
  NS_METHOD GetCompression(uint16_t *aCompression); \
  NS_METHOD GetSize(uint32_t *aSize); \
  NS_METHOD GetRealSize(uint32_t *aRealSize); \
  NS_METHOD GetCRC32(uint32_t *aCRC32); \
  NS_METHOD GetIsDirectory(bool *aIsDirectory); \
  NS_METHOD GetLastModifiedTime(PRTime *aLastModifiedTime); \
  NS_METHOD GetIsSynthetic(bool *aIsSynthetic); \
...
#define NS_DECL_NON_VIRTUAL_NSIZIPREADER \
  NS_METHOD Open(nsIFile *zipFile); \
  NS_METHOD OpenInner(nsIZipReader *zipReader, const nsACString & zipEntry); \
  NS_METHOD OpenMemory(void *aData, uint32_t aLength); \
  NS_METHOD GetFile(nsIFile * *aFile); \
  NS_METHOD Close(void); \
  NS_METHOD Test(const nsACString & aEntryName); \
  NS_METHOD Extract(const nsACString & zipEntry, nsIFile *outFile); \
  NS_METHOD GetEntry(const nsACString & zipEntry, nsIZipEntry * *_retval); \
  NS_METHOD HasEntry(const nsACString & zipEntry, bool *_retval); \
  NS_METHOD FindEntries(const nsACString & aPattern, nsIUTF8StringEnumerator * *_retval); \
  NS_METHOD GetInputStream(const nsACString & zipEntry, nsIInputStream * *_retval); \
  NS_METHOD GetInputStreamWithSpec(const nsACString & aJarSpec, const nsACString & zipEntry, nsIInputStream * *_retval); \
  NS_METHOD GetSigningCert(const nsACString & aEntryName, nsIX509Cert * *_retval); \
  NS_METHOD GetManifestEntriesCount(uint32_t *aManifestEntriesCount); 
...
#define NS_DECL_NON_VIRTUAL_NSIZIPREADERCACHE \
  NS_METHOD Init(uint32_t cacheSize); \
  NS_METHOD GetZip(nsIFile *zipFile, nsIZipReader * *_retval); \
  NS_METHOD IsCached(nsIFile *zipFile, bool *_retval); \
  NS_METHOD GetInnerZip(nsIFile *zipFile, const nsACString & zipEntry, nsIZipReader * *_retval); \
  NS_METHOD GetFd(nsIFile *zipFile, PRFileDesc **_retval); 
