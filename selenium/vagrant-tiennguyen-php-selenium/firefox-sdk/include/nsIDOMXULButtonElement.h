...
#define NS_DECL_NON_VIRTUAL_NSIDOMXULBUTTONELEMENT \
  ...
  NS_METHOD GetDlgType(nsAString & aDlgType); \
  NS_METHOD SetDlgType(const nsAString & aDlgType); \
  NS_METHOD GetOpen(bool *aOpen); \
  NS_METHOD SetOpen(bool aOpen); \
  NS_METHOD GetChecked(bool *aChecked); \
  NS_METHOD SetChecked(bool aChecked); \
  NS_METHOD GetCheckState(int32_t *aCheckState); \
  NS_METHOD SetCheckState(int32_t aCheckState); \
  NS_METHOD GetAutoCheck(bool *aAutoCheck); \
  NS_METHOD SetAutoCheck(bool aAutoCheck); \
  NS_METHOD GetGroup(nsAString & aGroup); \
  NS_METHOD SetGroup(const nsAString & aGroup); 
