...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLEHYPERLINK \
  NS_METHOD GetStartIndex(int32_t *aStartIndex); \
  NS_METHOD GetEndIndex(int32_t *aEndIndex); \
  NS_METHOD GetValid(bool *aValid); \
  NS_METHOD GetAnchorCount(int32_t *aAnchorCount); \
  NS_METHOD GetURI(int32_t index, nsIURI * *_retval); \
  NS_METHOD GetAnchor(int32_t index, nsIAccessible * *_retval); 
