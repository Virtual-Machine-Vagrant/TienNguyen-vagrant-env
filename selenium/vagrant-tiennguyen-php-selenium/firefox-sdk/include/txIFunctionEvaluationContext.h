...
#define NS_DECL_TXIFUNCTIONEVALUATIONCONTEXT \
  NS_IMETHOD GetPosition(uint32_t *aPosition) override; \
  NS_IMETHOD GetSize(uint32_t *aSize) override; \
  NS_IMETHOD GetContextNode(nsIDOMNode * *aContextNode) override; \
  NS_IMETHOD GetState(nsISupports * *aState) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_TXIFUNCTIONEVALUATIONCONTEXT \
  NS_METHOD GetPosition(uint32_t *aPosition); \
  NS_METHOD GetSize(uint32_t *aSize); \
  NS_METHOD GetContextNode(nsIDOMNode * *aContextNode); \
  NS_METHOD GetState(nsISupports * *aState); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_TXIFUNCTIONEVALUATIONCONTEXT(_to) \
  NS_IMETHOD GetPosition(uint32_t *aPosition) override { return _to GetPosition(aPosition); } \
  NS_IMETHOD GetSize(uint32_t *aSize) override { return _to GetSize(aSize); } \
  NS_IMETHOD GetContextNode(nsIDOMNode * *aContextNode) override { return _to GetContextNode(aContextNode); } \
  NS_IMETHOD GetState(nsISupports * *aState) override { return _to GetState(aState); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_TXIFUNCTIONEVALUATIONCONTEXT(_to) \
  NS_IMETHOD GetPosition(uint32_t *aPosition) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetPosition(aPosition); } \
  NS_IMETHOD GetSize(uint32_t *aSize) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetSize(aSize); } \
  NS_IMETHOD GetContextNode(nsIDOMNode * *aContextNode) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetContextNode(aContextNode); } \
  NS_IMETHOD GetState(nsISupports * *aState) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetState(aState); } 
