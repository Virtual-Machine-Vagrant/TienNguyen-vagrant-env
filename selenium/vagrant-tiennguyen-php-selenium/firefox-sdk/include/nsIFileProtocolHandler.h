...
#define NS_DECL_NON_VIRTUAL_NSIFILEPROTOCOLHANDLER \
  ...
  NS_METHOD GetURLSpecFromFile(nsIFile *file, nsACString & _retval); \
  NS_METHOD GetURLSpecFromActualFile(nsIFile *file, nsACString & _retval); \
  NS_METHOD GetURLSpecFromDir(nsIFile *file, nsACString & _retval); \
  NS_METHOD GetFileFromURLSpec(const nsACString & url, nsIFile * *_retval); \
  NS_METHOD ReadURLFile(nsIFile *file, nsIURI * *_retval); 
