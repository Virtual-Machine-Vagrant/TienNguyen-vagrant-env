...
#define NS_DECL_NON_VIRTUAL_NSITAGGINGSERVICE \
  NS_METHOD TagURI(nsIURI *aURI, nsIVariant *aTags); \
  NS_METHOD UntagURI(nsIURI *aURI, nsIVariant *aTags); \
  NS_METHOD GetURIsForTag(const nsAString & aTag, nsIVariant * *_retval); \
  NS_METHOD GetTagsForURI(nsIURI *aURI, uint32_t *length, char16_t * **aTags); \
  NS_METHOD GetAllTags(nsIVariant * *aAllTags); \
  NS_METHOD GetHasTags(bool *aHasTags); 
