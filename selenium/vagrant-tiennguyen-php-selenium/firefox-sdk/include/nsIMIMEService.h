...
#define NS_DECL_NON_VIRTUAL_NSIMIMESERVICE \
  NS_METHOD GetFromTypeAndExtension(const nsACString & aMIMEType, const nsACString & aFileExt, nsIMIMEInfo * *_retval); \
  NS_METHOD GetTypeFromExtension(const nsACString & aFileExt, nsACString & _retval); \
  NS_METHOD GetTypeFromURI(nsIURI *aURI, nsACString & _retval); \
  NS_METHOD GetTypeFromFile(nsIFile *aFile, nsACString & _retval); \
  NS_METHOD GetPrimaryExtension(const nsACString & aMIMEType, const nsACString & aFileExt, nsACString & _retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsMIMEService, nsIMIMEService)
