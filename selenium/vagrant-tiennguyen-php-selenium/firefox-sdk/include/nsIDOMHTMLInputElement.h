...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLINPUTELEMENT \
  NS_METHOD GetAccept(nsAString & aAccept); \
  NS_METHOD SetAccept(const nsAString & aAccept); \
  ...
  NS_METHOD GetAutofocus(bool *aAutofocus); \
  NS_METHOD SetAutofocus(bool aAutofocus); \
  NS_METHOD GetDefaultChecked(bool *aDefaultChecked); \
  NS_METHOD SetDefaultChecked(bool aDefaultChecked); \
  NS_METHOD GetChecked(bool *aChecked); \
  NS_METHOD SetChecked(bool aChecked); \
  NS_METHOD GetDisabled(bool *aDisabled); \
  NS_METHOD SetDisabled(bool aDisabled); \
  NS_METHOD GetForm(nsIDOMHTMLFormElement * *aForm); \
  NS_METHOD GetFormAction(nsAString & aFormAction); \
  NS_METHOD SetFormAction(const nsAString & aFormAction); \
  NS_METHOD GetFormEnctype(nsAString & aFormEnctype); \
  NS_METHOD SetFormEnctype(const nsAString & aFormEnctype); \
  NS_METHOD GetFormMethod(nsAString & aFormMethod); \
  NS_METHOD SetFormMethod(const nsAString & aFormMethod); \
  NS_METHOD GetFormNoValidate(bool *aFormNoValidate); \
  NS_METHOD SetFormNoValidate(bool aFormNoValidate); \
  NS_METHOD GetFormTarget(nsAString & aFormTarget); \
  NS_METHOD SetFormTarget(const nsAString & aFormTarget); \
  NS_METHOD GetFiles(nsIDOMFileList * *aFiles); \
  NS_METHOD GetHeight(uint32_t *aHeight); \
  NS_METHOD SetHeight(uint32_t aHeight); \
  NS_METHOD GetIndeterminate(bool *aIndeterminate); \
  NS_METHOD SetIndeterminate(bool aIndeterminate); \
  NS_METHOD GetInputMode(nsAString & aInputMode); \
  NS_METHOD SetInputMode(const nsAString & aInputMode); \
  NS_METHOD GetList(nsIDOMHTMLElement * *aList); \
  NS_METHOD GetMax(nsAString & aMax); \
  NS_METHOD SetMax(const nsAString & aMax); \
  NS_METHOD GetMaxLength(int32_t *aMaxLength); \
  NS_METHOD SetMaxLength(int32_t aMaxLength); \
  NS_METHOD GetMin(nsAString & aMin); \
  NS_METHOD SetMin(const nsAString & aMin); \
  NS_METHOD GetMultiple(bool *aMultiple); \
  NS_METHOD SetMultiple(bool aMultiple); \
  NS_METHOD GetName(nsAString & aName); \
  NS_METHOD SetName(const nsAString & aName); \
  NS_METHOD GetPattern(nsAString & aPattern); \
  NS_METHOD SetPattern(const nsAString & aPattern); \
  NS_METHOD GetPlaceholder(nsAString & aPlaceholder); \
  NS_METHOD SetPlaceholder(const nsAString & aPlaceholder); \
  NS_METHOD GetReadOnly(bool *aReadOnly); \
  NS_METHOD SetReadOnly(bool aReadOnly); \
  NS_METHOD GetRequired(bool *aRequired); \
  NS_METHOD SetRequired(bool aRequired); \
  NS_METHOD GetStep(nsAString & aStep); \
  NS_METHOD SetStep(const nsAString & aStep); \
  ...
  NS_METHOD GetDefaultValue(nsAString & aDefaultValue); \
  NS_METHOD SetDefaultValue(const nsAString & aDefaultValue); \
  NS_METHOD GetValue(nsAString & aValue); \
  NS_METHOD SetValue(const nsAString & aValue); \
  NS_METHOD GetValueAsNumber(double *aValueAsNumber); \
  NS_METHOD SetValueAsNumber(double aValueAsNumber); \
  NS_METHOD StepDown(int32_t n, uint8_t _argc); \
  NS_METHOD StepUp(int32_t n, uint8_t _argc); \
  NS_METHOD GetWillValidate(bool *aWillValidate); \
  NS_METHOD GetValidity(nsIDOMValidityState * *aValidity); \
  NS_METHOD GetValidationMessage(nsAString & aValidationMessage); \
  NS_METHOD CheckValidity(bool *_retval); \
  NS_METHOD SetCustomValidity(const nsAString & error); \
  NS_METHOD Select(void); \
  NS_METHOD GetSelectionStart(int32_t *aSelectionStart); \
  NS_METHOD SetSelectionStart(int32_t aSelectionStart); \
  NS_METHOD GetSelectionEnd(int32_t *aSelectionEnd); \
  NS_METHOD SetSelectionEnd(int32_t aSelectionEnd); \
  NS_METHOD SetSelectionRange(int32_t selectionStart, int32_t selectionEnd, const nsAString & direction); \
  NS_METHOD GetSelectionDirection(nsAString & aSelectionDirection); \
  NS_METHOD SetSelectionDirection(const nsAString & aSelectionDirection); \
  NS_METHOD GetUseMap(nsAString & aUseMap); \
  NS_METHOD SetUseMap(const nsAString & aUseMap); \
  NS_METHOD GetControllers(nsIControllers * *aControllers); \
  NS_METHOD GetTextLength(int32_t *aTextLength); \
  NS_METHOD MozGetFileNameArray(uint32_t *aLength, char16_t * **aFileNames); \
  NS_METHOD MozSetFileNameArray(const char16_t * *aFileNames, uint32_t aLength); \
  NS_METHOD MozIsTextField(bool aExcludePassword, bool *_retval); 
