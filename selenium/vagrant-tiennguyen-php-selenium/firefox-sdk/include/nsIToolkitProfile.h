...
#define NS_DECL_NON_VIRTUAL_NSIPROFILELOCK \
  ...
  NS_METHOD GetLocalDirectory(nsIFile * *aLocalDirectory); \
  NS_METHOD GetReplacedLockTime(PRTime *aReplacedLockTime); \
  NS_METHOD Unlock(void); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsProfileLock, nsIProfileLock)
...
#define NS_DECL_NON_VIRTUAL_NSITOOLKITPROFILE \
  NS_METHOD GetRootDir(nsIFile * *aRootDir); \
  NS_METHOD GetLocalDir(nsIFile * *aLocalDir); \
  NS_METHOD GetName(nsACString & aName); \
  NS_METHOD SetName(const nsACString & aName); \
  NS_METHOD Remove(bool removeFiles); \
  NS_METHOD Lock(nsIProfileUnlocker * *aUnlocker, nsIProfileLock * *_retval); 
