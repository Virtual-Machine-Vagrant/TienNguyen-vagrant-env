...
#define NS_DECL_NSIPREFLOCALIZEDSTRING \
  NS_IMETHOD GetData(char16_t * *aData) override; \
  NS_IMETHOD SetData(const char16_t * aData) override; \
  NS_IMETHOD ToString(char16_t * *_retval) override; \
  NS_IMETHOD SetDataWithLength(uint32_t length, const char16_t * data) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIPREFLOCALIZEDSTRING \
  NS_METHOD GetData(char16_t * *aData); \
  NS_METHOD SetData(const char16_t * aData); \
  NS_METHOD ToString(char16_t * *_retval); \
  NS_METHOD SetDataWithLength(uint32_t length, const char16_t * data); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIPREFLOCALIZEDSTRING(_to) \
  NS_IMETHOD GetData(char16_t * *aData) override { return _to GetData(aData); } \
  NS_IMETHOD SetData(const char16_t * aData) override { return _to SetData(aData); } \
  NS_IMETHOD ToString(char16_t * *_retval) override { return _to ToString(_retval); } \
  NS_IMETHOD SetDataWithLength(uint32_t length, const char16_t * data) override { return _to SetDataWithLength(length, data); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIPREFLOCALIZEDSTRING(_to) \
  NS_IMETHOD GetData(char16_t * *aData) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetData(aData); } \
  NS_IMETHOD SetData(const char16_t * aData) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetData(aData); } \
  NS_IMETHOD ToString(char16_t * *_retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ToString(_retval); } \
  NS_IMETHOD SetDataWithLength(uint32_t length, const char16_t * data) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetDataWithLength(length, data); } 
