...
#define NS_DECL_NON_VIRTUAL_NSIINPUTSTREAM \
  ...
  NS_METHOD Available(uint64_t *_retval); \
  NS_METHOD Read(char *aBuf, uint32_t aCount, uint32_t *_retval); \
  NS_METHOD ReadSegments(nsWriteSegmentFun aWriter, void *aClosure, uint32_t aCount, uint32_t *_retval); \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsInputStream, nsIInputStream)
