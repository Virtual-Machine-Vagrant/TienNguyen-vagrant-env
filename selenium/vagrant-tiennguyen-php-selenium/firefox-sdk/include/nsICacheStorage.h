...
#define NS_DECL_NON_VIRTUAL_NSICACHESTORAGE \
  NS_METHOD AsyncOpenURI(nsIURI *aURI, const nsACString & aIdExtension, uint32_t aFlags, nsICacheEntryOpenCallback *aCallback); \
  NS_METHOD OpenTruncate(nsIURI *aURI, const nsACString & aIdExtension, nsICacheEntry * *_retval); \
  NS_METHOD Exists(nsIURI *aURI, const nsACString & aIdExtension, bool *_retval); \
  NS_METHOD AsyncDoomURI(nsIURI *aURI, const nsACString & aIdExtension, nsICacheEntryDoomCallback *aCallback); \
  NS_METHOD AsyncEvictStorage(nsICacheEntryDoomCallback *aCallback); \
  NS_METHOD AsyncVisitStorage(nsICacheStorageVisitor *aVisitor, bool aVisitEntries); 
