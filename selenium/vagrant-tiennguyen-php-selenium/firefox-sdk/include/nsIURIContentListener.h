...
#define NS_DECL_NON_VIRTUAL_NSIURICONTENTLISTENER \
  NS_METHOD OnStartURIOpen(nsIURI *aURI, bool *_retval); \
  NS_METHOD DoContent(const nsACString & aContentType, bool aIsContentPreferred, nsIRequest *aRequest, nsIStreamListener * *aContentHandler, bool *_retval); \
  NS_METHOD IsPreferred(const char * aContentType, char * *aDesiredContentType, bool *_retval); \
  NS_METHOD CanHandleContent(const char * aContentType, bool aIsContentPreferred, char * *aDesiredContentType, bool *_retval); \
  NS_METHOD GetLoadCookie(nsISupports * *aLoadCookie); \
  NS_METHOD SetLoadCookie(nsISupports *aLoadCookie); \
  NS_METHOD GetParentContentListener(nsIURIContentListener * *aParentContentListener); \
  NS_METHOD SetParentContentListener(nsIURIContentListener *aParentContentListener); 
