...
#define NS_FORWARD_NSIRDFREMOTEDATASOURCE(_to) \
  ...
  NS_IMETHOD Refresh(bool aBlocking) override { return _to Refresh(aBlocking); } \
  NS_IMETHOD Flush(void) override { return _to Flush(); } \
  NS_IMETHOD FlushTo(const char * aURI) override { return _to FlushTo(aURI); } 
...
#define NS_FORWARD_SAFE_NSIRDFREMOTEDATASOURCE(_to) \
  ...
  NS_IMETHOD Refresh(bool aBlocking) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Refresh(aBlocking); } \
  NS_IMETHOD Flush(void) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Flush(); } \
  NS_IMETHOD FlushTo(const char * aURI) override { return !_to ? NS_ERROR_NULL_POINTER : _to->FlushTo(aURI); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsRDFRemoteDataSource, nsIRDFRemoteDataSource)
