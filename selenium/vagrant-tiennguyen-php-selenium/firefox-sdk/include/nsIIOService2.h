...
#define NS_DECL_NON_VIRTUAL_NSIIOSERVICE2 \
  NS_METHOD GetManageOfflineStatus(bool *aManageOfflineStatus); \
  NS_METHOD SetManageOfflineStatus(bool aManageOfflineStatus); \
  NS_METHOD NewChannelFromURIWithProxyFlags2(nsIURI *aURI, nsIURI *aProxyURI, uint32_t aProxyFlags, nsIDOMNode *aLoadingNode, nsIPrincipal *aLoadingPrincipal, nsIPrincipal *aTriggeringPrincipal, uint32_t aSecurityFlags, uint32_t aContentPolicyType, nsIChannel * *_retval); \
  NS_METHOD NewChannelFromURIWithProxyFlags(nsIURI *aURI, nsIURI *aProxyURI, uint32_t aProxyFlags, nsIChannel * *_retval); 
