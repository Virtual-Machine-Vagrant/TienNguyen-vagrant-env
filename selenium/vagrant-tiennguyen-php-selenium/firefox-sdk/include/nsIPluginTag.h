...
#define NS_DECL_NON_VIRTUAL_NSIPLUGINTAG \
  ...
  NS_METHOD GetFullpath(nsACString & aFullpath); \
  NS_METHOD GetVersion(nsACString & aVersion); \
  NS_METHOD GetName(nsACString & aName); \
  NS_METHOD GetNiceName(nsACString & aNiceName); \
  NS_METHOD GetBlocklisted(bool *aBlocklisted); \
  NS_METHOD GetIsEnabledStateLocked(bool *aIsEnabledStateLocked); \
  NS_METHOD GetActive(bool *aActive); \
  NS_METHOD GetBlocklistState(uint32_t *aBlocklistState); \
  NS_METHOD GetDisabled(bool *aDisabled); \
  NS_METHOD GetClicktoplay(bool *aClicktoplay); \
  NS_METHOD GetLoaded(bool *aLoaded); \
  NS_METHOD GetEnabledState(uint32_t *aEnabledState); \
  NS_METHOD SetEnabledState(uint32_t aEnabledState); \
  NS_METHOD GetLastModifiedTime(PRTime *aLastModifiedTime); \
  NS_METHOD GetMimeTypes(uint32_t *aCount, char16_t * **aResults); \
  NS_METHOD GetMimeDescriptions(uint32_t *aCount, char16_t * **aResults); \
  NS_METHOD GetExtensions(uint32_t *aCount, char16_t * **aResults); 
...
/* readonly attribute boolean loaded; */
NS_IMETHODIMP nsPluginTag::GetLoaded(bool *aLoaded)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
...
#define NS_DECL_NON_VIRTUAL_NSIFAKEPLUGINTAG \
  NS_METHOD GetHandlerURI(nsIURI * *aHandlerURI); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsFakePluginTag, nsIFakePluginTag)
...
/* readonly attribute nsIURI handlerURI; */
NS_IMETHODIMP nsFakePluginTag::GetHandlerURI(nsIURI * *aHandlerURI)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
