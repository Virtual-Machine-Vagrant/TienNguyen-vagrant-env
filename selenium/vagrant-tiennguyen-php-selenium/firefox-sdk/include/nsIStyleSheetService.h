...
#define NS_DECL_NON_VIRTUAL_NSISTYLESHEETSERVICE \
  NS_METHOD LoadAndRegisterSheet(nsIURI *sheetURI, uint32_t type); \
  NS_METHOD SheetRegistered(nsIURI *sheetURI, uint32_t type, bool *_retval); \
  NS_METHOD PreloadSheet(nsIURI *sheetURI, uint32_t type, nsIDOMStyleSheet * *_retval); \
  NS_METHOD UnregisterSheet(nsIURI *sheetURI, uint32_t type); 
