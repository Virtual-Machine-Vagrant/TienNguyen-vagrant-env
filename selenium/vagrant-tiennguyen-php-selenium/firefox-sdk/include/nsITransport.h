...
#define NS_DECL_NON_VIRTUAL_NSITRANSPORT \
  NS_METHOD OpenInputStream(uint32_t aFlags, uint32_t aSegmentSize, uint32_t aSegmentCount, nsIInputStream * *_retval); \
  NS_METHOD OpenOutputStream(uint32_t aFlags, uint32_t aSegmentSize, uint32_t aSegmentCount, nsIOutputStream * *_retval); \
  NS_METHOD Close(nsresult aReason); \
  NS_METHOD SetEventSink(nsITransportEventSink *aSink, nsIEventTarget *aEventTarget); \
...
#define NS_DECL_NON_VIRTUAL_NSITRANSPORTEVENTSINK \
  NS_METHOD OnTransportStatus(nsITransport *aTransport, nsresult aStatus, int64_t aProgress, int64_t aProgressMax); 
