...
#define NS_DECL_NSIWEBBROWSERSETUP \
  NS_IMETHOD SetProperty(uint32_t aId, uint32_t aValue) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIWEBBROWSERSETUP \
  NS_METHOD SetProperty(uint32_t aId, uint32_t aValue); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIWEBBROWSERSETUP(_to) \
  NS_IMETHOD SetProperty(uint32_t aId, uint32_t aValue) override { return _to SetProperty(aId, aValue); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIWEBBROWSERSETUP(_to) \
  NS_IMETHOD SetProperty(uint32_t aId, uint32_t aValue) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetProperty(aId, aValue); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsWebBrowserSetup, nsIWebBrowserSetup)
