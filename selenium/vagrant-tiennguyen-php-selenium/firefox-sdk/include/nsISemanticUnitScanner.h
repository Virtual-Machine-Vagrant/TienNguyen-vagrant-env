...
class NS_NO_VTABLE nsISemanticUnitScanner : public nsISupports {
 public:
  ...
  NS_IMETHOD Next(const char16_t * text, int32_t length, int32_t pos, bool isLastBuffer, int32_t *begin, int32_t *end, bool *_retval) = 0;
...
#define NS_DECL_NSISEMANTICUNITSCANNER \
  NS_IMETHOD Start(const char * characterSet) override; \
  NS_IMETHOD Next(const char16_t * text, int32_t length, int32_t pos, bool isLastBuffer, int32_t *begin, int32_t *end, bool *_retval) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSISEMANTICUNITSCANNER \
  NS_METHOD Start(const char * characterSet); \
  NS_METHOD Next(const char16_t * text, int32_t length, int32_t pos, bool isLastBuffer, int32_t *begin, int32_t *end, bool *_retval); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSISEMANTICUNITSCANNER(_to) \
  NS_IMETHOD Start(const char * characterSet) override { return _to Start(characterSet); } \
  NS_IMETHOD Next(const char16_t * text, int32_t length, int32_t pos, bool isLastBuffer, int32_t *begin, int32_t *end, bool *_retval) override { return _to Next(text, length, pos, isLastBuffer, begin, end, _retval); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSISEMANTICUNITSCANNER(_to) \
  NS_IMETHOD Start(const char * characterSet) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Start(characterSet); } \
  NS_IMETHOD Next(const char16_t * text, int32_t length, int32_t pos, bool isLastBuffer, int32_t *begin, int32_t *end, bool *_retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Next(text, length, pos, isLastBuffer, begin, end, _retval); } 
