...
#define NS_DECL_NSIARRAYBUFFERINPUTSTREAM \
  NS_IMETHOD SetData(JS::HandleValue buffer, uint32_t byteOffset, uint32_t byteLen, JSContext* cx) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIARRAYBUFFERINPUTSTREAM \
  NS_METHOD SetData(JS::HandleValue buffer, uint32_t byteOffset, uint32_t byteLen, JSContext* cx); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIARRAYBUFFERINPUTSTREAM(_to) \
  NS_IMETHOD SetData(JS::HandleValue buffer, uint32_t byteOffset, uint32_t byteLen, JSContext* cx) override { return _to SetData(buffer, byteOffset, byteLen, cx); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIARRAYBUFFERINPUTSTREAM(_to) \
  NS_IMETHOD SetData(JS::HandleValue buffer, uint32_t byteOffset, uint32_t byteLen, JSContext* cx) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetData(buffer, byteOffset, byteLen, cx); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsArrayBufferInputStream, nsIArrayBufferInputStream)
