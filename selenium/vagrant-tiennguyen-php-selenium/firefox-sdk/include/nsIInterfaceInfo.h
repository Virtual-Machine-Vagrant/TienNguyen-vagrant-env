...
#define NS_DECL_NON_VIRTUAL_NSIINTERFACEINFO \
  NS_METHOD GetName(char * *aName); \
  NS_METHOD GetInterfaceIID(nsIID **aInterfaceIID); \
  NS_METHOD IsScriptable(bool *_retval); \
  NS_METHOD IsBuiltinClass(bool *_retval); \
  NS_METHOD GetParent(nsIInterfaceInfo * *aParent); \
  NS_METHOD GetMethodCount(uint16_t *aMethodCount); \
  NS_METHOD GetConstantCount(uint16_t *aConstantCount); \
  NS_METHOD GetMethodInfo(uint16_t index, const nsXPTMethodInfo **info); \
  NS_METHOD GetMethodInfoForName(const char * methodName, uint16_t *index, const nsXPTMethodInfo **info); \
  NS_METHOD GetConstant(uint16_t index, JS::MutableHandleValue constant, char * *name); \
  NS_METHOD GetInfoForParam(uint16_t methodIndex, const nsXPTParamInfo *param, nsIInterfaceInfo * *_retval); \
  NS_METHOD GetIIDForParam(uint16_t methodIndex, const nsXPTParamInfo *param, nsIID **_retval); \
  NS_METHOD GetTypeForParam(uint16_t methodIndex, const nsXPTParamInfo *param, uint16_t dimension, nsXPTType *_retval); \
  NS_METHOD GetSizeIsArgNumberForParam(uint16_t methodIndex, const nsXPTParamInfo *param, uint16_t dimension, uint8_t *_retval); \
  NS_METHOD GetInterfaceIsArgNumberForParam(uint16_t methodIndex, const nsXPTParamInfo *param, uint8_t *_retval); \
  NS_METHOD IsIID(const nsIID *IID, bool *_retval); \
  NS_METHOD GetNameShared(const char * *name); \
  NS_METHOD GetIIDShared(const nsIID **iid); \
  NS_METHOD IsFunction(bool *_retval); \
  NS_METHOD HasAncestor(const nsIID *iid, bool *_retval); \
  NS_METHOD_(nsresult) GetIIDForParamNoAlloc(uint16_t methodIndex, const nsXPTParamInfo *param, nsIID *iid); \
  NS_METHOD IsMainProcessScriptableOnly(bool *_retval); 
