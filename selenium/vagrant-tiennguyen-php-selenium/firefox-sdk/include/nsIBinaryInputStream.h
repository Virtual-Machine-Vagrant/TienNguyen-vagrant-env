...
#define NS_DECL_NON_VIRTUAL_NSIBINARYINPUTSTREAM \
  NS_METHOD SetInputStream(nsIInputStream *aInputStream); \
  NS_METHOD ReadBoolean(bool *_retval); \
  NS_METHOD Read8(uint8_t *_retval); \
  NS_METHOD Read16(uint16_t *_retval); \
  NS_METHOD Read32(uint32_t *_retval); \
  NS_METHOD Read64(uint64_t *_retval); \
  NS_METHOD ReadFloat(float *_retval); \
  NS_METHOD ReadDouble(double *_retval); \
  NS_METHOD ReadCString(nsACString & _retval); \
  NS_METHOD ReadString(nsAString & _retval); \
  NS_METHOD ReadBytes(uint32_t aLength, char * *aString); \
  NS_METHOD ReadByteArray(uint32_t aLength, uint8_t **aBytes); \
  NS_METHOD ReadArrayBuffer(uint32_t aLength, JS::HandleValue aArrayBuffer, JSContext* cx, uint32_t *_retval); 
