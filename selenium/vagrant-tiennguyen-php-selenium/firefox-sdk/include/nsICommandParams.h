...
#define NS_DECL_NON_VIRTUAL_NSICOMMANDPARAMS \
  NS_METHOD GetValueType(const char * name, int16_t *_retval); \
  NS_METHOD GetBooleanValue(const char * name, bool *_retval); \
  NS_METHOD GetLongValue(const char * name, int32_t *_retval); \
  NS_METHOD GetDoubleValue(const char * name, double *_retval); \
  NS_METHOD GetStringValue(const char * name, nsAString & _retval); \
  NS_METHOD GetCStringValue(const char * name, char * *_retval); \
  NS_METHOD GetISupportsValue(const char * name, nsISupports * *_retval); \
  NS_METHOD SetBooleanValue(const char * name, bool value); \
  NS_METHOD SetLongValue(const char * name, int32_t value); \
  NS_METHOD SetDoubleValue(const char * name, double value); \
  NS_METHOD SetStringValue(const char * name, const nsAString & value); \
  NS_METHOD SetCStringValue(const char * name, const char * value); \
  NS_METHOD SetISupportsValue(const char * name, nsISupports *value); \
  NS_METHOD RemoveValue(const char * name); 
