...
#define NS_DECL_NON_VIRTUAL_NSIDOMCSSSTYLEDECLARATION \
  NS_METHOD GetCssText(nsAString & aCssText); \
  NS_METHOD SetCssText(const nsAString & aCssText); \
  NS_METHOD GetPropertyValue(const nsAString & propertyName, nsAString & _retval); \
  NS_METHOD GetPropertyCSSValue(const nsAString & propertyName, nsIDOMCSSValue * *_retval); \
  NS_METHOD RemoveProperty(const nsAString & propertyName, nsAString & _retval); \
  NS_METHOD GetPropertyPriority(const nsAString & propertyName, nsAString & _retval); \
  NS_METHOD SetProperty(const nsAString & propertyName, const nsAString & value, const nsAString & priority); \
  NS_METHOD GetLength(uint32_t *aLength); \
  NS_METHOD Item(uint32_t index, nsAString & _retval); \
  NS_METHOD GetParentRule(nsIDOMCSSRule * *aParentRule); 
