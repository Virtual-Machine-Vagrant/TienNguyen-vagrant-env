...
#define NS_DECL_NSITHREADMANAGER \
  ...
  NS_IMETHOD GetIsMainThread(bool *aIsMainThread) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSITHREADMANAGER \
  NS_METHOD NewThread(uint32_t creationFlags, uint32_t stackSize, nsIThread * *_retval); \
  NS_METHOD GetThreadFromPRThread(PRThread *prthread, nsIThread * *_retval); \
  NS_METHOD GetMainThread(nsIThread * *aMainThread); \
  NS_METHOD GetCurrentThread(nsIThread * *aCurrentThread); \
  NS_METHOD GetIsMainThread(bool *aIsMainThread); 
...
#define NS_FORWARD_NSITHREADMANAGER(_to) \
  ...
  NS_IMETHOD GetIsMainThread(bool *aIsMainThread) override { return _to GetIsMainThread(aIsMainThread); } 
...
#define NS_FORWARD_SAFE_NSITHREADMANAGER(_to) \
  ...
  NS_IMETHOD GetIsMainThread(bool *aIsMainThread) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetIsMainThread(aIsMainThread); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsThreadManager, nsIThreadManager)
