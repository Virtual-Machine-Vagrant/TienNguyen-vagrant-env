...
#define NS_DECL_NSIPARENTREDIRECTINGCHANNEL \
  NS_IMETHOD StartRedirect(uint32_t newChannelId, nsIChannel *newChannel, uint32_t redirectFlags, nsIAsyncVerifyRedirectCallback *callback) override; \
...
#define NS_DECL_NON_VIRTUAL_NSIPARENTREDIRECTINGCHANNEL \
  NS_METHOD StartRedirect(uint32_t newChannelId, nsIChannel *newChannel, uint32_t redirectFlags, nsIAsyncVerifyRedirectCallback *callback); \
  NS_METHOD CompleteRedirect(bool succeeded); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIPARENTREDIRECTINGCHANNEL(_to) \
  NS_IMETHOD StartRedirect(uint32_t newChannelId, nsIChannel *newChannel, uint32_t redirectFlags, nsIAsyncVerifyRedirectCallback *callback) override { return _to StartRedirect(newChannelId, newChannel, redirectFlags, callback); } \
...
#define NS_FORWARD_SAFE_NSIPARENTREDIRECTINGCHANNEL(_to) \
  NS_IMETHOD StartRedirect(uint32_t newChannelId, nsIChannel *newChannel, uint32_t redirectFlags, nsIAsyncVerifyRedirectCallback *callback) override { return !_to ? NS_ERROR_NULL_POINTER : _to->StartRedirect(newChannelId, newChannel, redirectFlags, callback); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsParentRedirectingChannel, nsIParentRedirectingChannel)
