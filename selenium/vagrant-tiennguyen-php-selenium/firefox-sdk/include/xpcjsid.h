...
#define NS_DECL_NON_VIRTUAL_NSIJSID \
  NS_METHOD GetName(char * *aName); \
  NS_METHOD GetNumber(char * *aNumber); \
  NS_METHOD GetValid(bool *aValid); \
  NS_METHOD Equals(nsIJSID *other, bool *_retval); \
  NS_METHOD ToString(char * *_retval); \
  NS_METHOD Initialize(const char * idString); \
  NS_METHOD_(const nsID *) GetID(void); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIJSIID \
  /* no methods! */
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIJSCID \
  NS_METHOD CreateInstance(JS::HandleValue iid, JSContext* cx, uint8_t _argc, JS::MutableHandleValue _retval); \
  NS_METHOD GetService(JS::HandleValue iid, JSContext* cx, uint8_t _argc, JS::MutableHandleValue _retval); 
