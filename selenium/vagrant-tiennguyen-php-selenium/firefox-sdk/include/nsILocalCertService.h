...
#define NS_DECL_NON_VIRTUAL_NSILOCALCERTSERVICE \
  NS_METHOD GetOrCreateCert(const nsACString & nickname, nsILocalCertGetCallback *cb); \
  NS_METHOD RemoveCert(const nsACString & nickname, nsILocalCertCallback *cb); \
  NS_METHOD GetLoginPromptRequired(bool *aLoginPromptRequired); 
...
#define NS_DECL_NON_VIRTUAL_NSILOCALCERTGETCALLBACK \
  NS_METHOD HandleCert(nsIX509Cert *cert, nsresult result); 
...
#define NS_DECL_NON_VIRTUAL_NSILOCALCERTCALLBACK \
  NS_METHOD HandleResult(nsresult result); 
