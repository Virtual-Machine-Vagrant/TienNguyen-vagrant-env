...
#define NS_DECL_NSICOMMANDLINE \
  ...
  NS_IMETHOD GetWindowContext(nsIDOMWindow * *aWindowContext) override; \
...
#define NS_DECL_NON_VIRTUAL_NSICOMMANDLINE \
  ...
  NS_METHOD GetArgument(int32_t aIndex, nsAString & _retval); \
  NS_METHOD FindFlag(const nsAString & aFlag, bool aCaseSensitive, int32_t *_retval); \
  NS_METHOD RemoveArguments(int32_t aStart, int32_t aEnd); \
  NS_METHOD HandleFlag(const nsAString & aFlag, bool aCaseSensitive, bool *_retval); \
  NS_METHOD HandleFlagWithParam(const nsAString & aFlag, bool aCaseSensitive, nsAString & _retval); \
  NS_METHOD GetState(uint32_t *aState); \
  NS_METHOD GetPreventDefault(bool *aPreventDefault); \
  NS_METHOD SetPreventDefault(bool aPreventDefault); \
  NS_METHOD GetWorkingDirectory(nsIFile * *aWorkingDirectory); \
  NS_METHOD GetWindowContext(nsIDOMWindow * *aWindowContext); \
  NS_METHOD ResolveFile(const nsAString & aArgument, nsIFile * *_retval); \
  NS_METHOD ResolveURI(const nsAString & aArgument, nsIURI * *_retval); 
...
#define NS_FORWARD_NSICOMMANDLINE(_to) \
  ...
  NS_IMETHOD GetWindowContext(nsIDOMWindow * *aWindowContext) override { return _to GetWindowContext(aWindowContext); } \
...
#define NS_FORWARD_SAFE_NSICOMMANDLINE(_to) \
  ...
  NS_IMETHOD GetWindowContext(nsIDOMWindow * *aWindowContext) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetWindowContext(aWindowContext); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsCommandLine, nsICommandLine)
