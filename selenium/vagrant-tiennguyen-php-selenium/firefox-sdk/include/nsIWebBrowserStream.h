...
#define NS_DECL_NSIWEBBROWSERSTREAM \
  NS_IMETHOD OpenStream(nsIURI *aBaseURI, const nsACString & aContentType) override; \
  NS_IMETHOD AppendToStream(const uint8_t *aData, uint32_t aLen) override; \
  NS_IMETHOD CloseStream(void) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIWEBBROWSERSTREAM \
...
#define NS_FORWARD_NSIWEBBROWSERSTREAM(_to) \
  NS_IMETHOD OpenStream(nsIURI *aBaseURI, const nsACString & aContentType) override { return _to OpenStream(aBaseURI, aContentType); } \
  NS_IMETHOD AppendToStream(const uint8_t *aData, uint32_t aLen) override { return _to AppendToStream(aData, aLen); } \
  NS_IMETHOD CloseStream(void) override { return _to CloseStream(); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIWEBBROWSERSTREAM(_to) \
  NS_IMETHOD OpenStream(nsIURI *aBaseURI, const nsACString & aContentType) override { return !_to ? NS_ERROR_NULL_POINTER : _to->OpenStream(aBaseURI, aContentType); } \
  NS_IMETHOD AppendToStream(const uint8_t *aData, uint32_t aLen) override { return !_to ? NS_ERROR_NULL_POINTER : _to->AppendToStream(aData, aLen); } \
  NS_IMETHOD CloseStream(void) override { return !_to ? NS_ERROR_NULL_POINTER : _to->CloseStream(); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsWebBrowserStream, nsIWebBrowserStream)
