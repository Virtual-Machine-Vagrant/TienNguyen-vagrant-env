...
#define NS_DECL_NON_VIRTUAL_NSIOUTPUTSTREAM \
  ...
  NS_METHOD Write(const char * aBuf, uint32_t aCount, uint32_t *_retval); \
  NS_METHOD WriteFrom(nsIInputStream *aFromStream, uint32_t aCount, uint32_t *_retval); \
  NS_METHOD WriteSegments(nsReadSegmentFun aReader, void *aClosure, uint32_t aCount, uint32_t *_retval); \
  NS_METHOD IsNonBlocking(bool *_retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsOutputStream, nsIOutputStream)
