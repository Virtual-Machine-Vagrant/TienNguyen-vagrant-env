...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLTEXTAREAELEMENT \
  NS_METHOD GetAutofocus(bool *aAutofocus); \
  NS_METHOD SetAutofocus(bool aAutofocus); \
  NS_METHOD GetCols(uint32_t *aCols); \
  NS_METHOD SetCols(uint32_t aCols); \
  ...
  NS_METHOD GetRows(uint32_t *aRows); \
  NS_METHOD SetRows(uint32_t aRows); \
  NS_METHOD GetWrap(nsAString & aWrap); \
  NS_METHOD SetWrap(const nsAString & aWrap); \
