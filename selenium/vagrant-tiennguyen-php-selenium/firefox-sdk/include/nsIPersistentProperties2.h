...
#define NS_DECL_NON_VIRTUAL_NSIPERSISTENTPROPERTIES \
  NS_METHOD Load(nsIInputStream *input); \
  NS_METHOD Save(nsIOutputStream *output, const nsACString & header); \
  NS_METHOD Enumerate(nsISimpleEnumerator * *_retval); \
  NS_METHOD GetStringProperty(const nsACString & key, nsAString & _retval); \
  NS_METHOD SetStringProperty(const nsACString & key, const nsAString & value, nsAString & _retval); 
