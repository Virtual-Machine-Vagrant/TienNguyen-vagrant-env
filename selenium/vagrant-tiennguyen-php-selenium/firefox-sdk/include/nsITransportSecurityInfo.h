...
#define NS_DECL_NON_VIRTUAL_NSITRANSPORTSECURITYINFO \
  NS_METHOD GetSecurityState(uint32_t *aSecurityState); \
  NS_METHOD GetErrorMessage(char16_t * *aErrorMessage); \
  NS_METHOD GetErrorCode(int32_t *aErrorCode); \
  NS_METHOD GetFailedCertChain(nsIX509CertList * *aFailedCertChain); 
