...
#define NS_DECL_NON_VIRTUAL_NSIPROFILESTARTUP \
  NS_METHOD GetDirectory(nsIFile * *aDirectory); \
  NS_METHOD DoStartup(void); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsProfileStartup, nsIProfileStartup)
...
#define NS_DECL_NSIPROFILEMIGRATOR \
  NS_IMETHOD Migrate(nsIProfileStartup *aStartup, const nsACString & aKey) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIPROFILEMIGRATOR \
  NS_METHOD Migrate(nsIProfileStartup *aStartup, const nsACString & aKey); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIPROFILEMIGRATOR(_to) \
  NS_IMETHOD Migrate(nsIProfileStartup *aStartup, const nsACString & aKey) override { return _to Migrate(aStartup, aKey); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIPROFILEMIGRATOR(_to) \
  NS_IMETHOD Migrate(nsIProfileStartup *aStartup, const nsACString & aKey) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Migrate(aStartup, aKey); } 
