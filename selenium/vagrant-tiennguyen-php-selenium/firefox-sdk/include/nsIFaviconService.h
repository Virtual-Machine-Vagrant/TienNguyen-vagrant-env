...
#define NS_DECL_NON_VIRTUAL_NSIFAVICONSERVICE \
  NS_METHOD GetFaviconLinkForIcon(nsIURI *aFaviconURI, nsIURI * *_retval); \
  NS_METHOD ExpireAllFavicons(void); \
  NS_METHOD AddFailedFavicon(nsIURI *aFaviconURI); \
  NS_METHOD RemoveFailedFavicon(nsIURI *aFaviconURI); \
  NS_METHOD IsFailedFavicon(nsIURI *aFaviconURI, bool *_retval); \
  NS_METHOD GetDefaultFavicon(nsIURI * *aDefaultFavicon); 
...
#define NS_DECL_NON_VIRTUAL_NSIFAVICONDATACALLBACK \
  NS_METHOD OnComplete(nsIURI *aFaviconURI, uint32_t aDataLen, const uint8_t *aData, const nsACString & aMimeType); 
