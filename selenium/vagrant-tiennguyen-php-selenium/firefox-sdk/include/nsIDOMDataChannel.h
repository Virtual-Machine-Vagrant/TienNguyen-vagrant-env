...
#define NS_DECL_NON_VIRTUAL_NSIDOMDATACHANNEL \
  NS_METHOD GetLabel(nsAString & aLabel); \
  NS_METHOD GetProtocol(nsAString & aProtocol); \
  NS_METHOD GetReliable(bool *aReliable); \
  NS_METHOD GetOrdered(bool *aOrdered); \
  NS_METHOD GetReadyState(nsAString & aReadyState); \
  NS_METHOD GetBufferedAmount(uint32_t *aBufferedAmount); \
  NS_METHOD GetId(uint16_t *aId); \
  NS_METHOD GetStream(uint16_t *aStream); \
  NS_METHOD GetOnopen(JSContext* cx, JS::MutableHandleValue aOnopen); \
  NS_METHOD SetOnopen(JSContext* cx, JS::HandleValue aOnopen); \
  ...
  NS_METHOD GetOnclose(JSContext* cx, JS::MutableHandleValue aOnclose); \
  NS_METHOD SetOnclose(JSContext* cx, JS::HandleValue aOnclose); \
  NS_METHOD GetOnmessage(JSContext* cx, JS::MutableHandleValue aOnmessage); \
  NS_METHOD SetOnmessage(JSContext* cx, JS::HandleValue aOnmessage); \
  NS_METHOD GetBinaryType(nsAString & aBinaryType); \
  NS_METHOD SetBinaryType(const nsAString & aBinaryType); \
