...
#define NS_DECL_NON_VIRTUAL_MOZISPELLCHECKINGENGINE \
  NS_METHOD GetDictionary(char16_t * *aDictionary); \
  NS_METHOD SetDictionary(const char16_t * aDictionary); \
  NS_METHOD GetLanguage(char16_t * *aLanguage); \
  NS_METHOD GetProvidesPersonalDictionary(bool *aProvidesPersonalDictionary); \
  NS_METHOD GetProvidesWordUtils(bool *aProvidesWordUtils); \
  NS_METHOD GetName(char16_t * *aName); \
  NS_METHOD GetCopyright(char16_t * *aCopyright); \
  NS_METHOD GetPersonalDictionary(mozIPersonalDictionary * *aPersonalDictionary); \
  NS_METHOD SetPersonalDictionary(mozIPersonalDictionary *aPersonalDictionary); \
  NS_METHOD GetDictionaryList(char16_t * **dictionaries, uint32_t *count); \
  NS_METHOD Check(const char16_t * word, bool *_retval); \
  NS_METHOD Suggest(const char16_t * word, char16_t * **suggestions, uint32_t *count); \
  NS_METHOD LoadDictionariesFromDir(nsIFile *dir); \
  NS_METHOD AddDirectory(nsIFile *dir); \
  NS_METHOD RemoveDirectory(nsIFile *dir); 
