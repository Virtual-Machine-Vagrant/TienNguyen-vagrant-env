...
#define NS_DECL_NON_VIRTUAL_NSIRDFXMLSINKOBSERVER \
  NS_METHOD OnBeginLoad(nsIRDFXMLSink *aSink); \
  NS_METHOD OnInterrupt(nsIRDFXMLSink *aSink); \
  NS_METHOD OnResume(nsIRDFXMLSink *aSink); \
  NS_METHOD OnEndLoad(nsIRDFXMLSink *aSink); \
  NS_METHOD OnError(nsIRDFXMLSink *aSink, nsresult aStatus, const char16_t * aErrorMsg); 
...
#define NS_DECL_NON_VIRTUAL_NSIRDFXMLSINK \
  ...
  NS_METHOD BeginLoad(void); \
  NS_METHOD Interrupt(void); \
  NS_METHOD Resume(void); \
  NS_METHOD EndLoad(void); \
  NS_METHOD AddNameSpace(nsIAtom *aPrefix, const nsString & aURI); \
  NS_METHOD AddXMLSinkObserver(nsIRDFXMLSinkObserver *aObserver); \
  NS_METHOD RemoveXMLSinkObserver(nsIRDFXMLSinkObserver *aObserver); 
