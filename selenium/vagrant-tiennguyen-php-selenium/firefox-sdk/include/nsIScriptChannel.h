...
#define NS_DECL_NON_VIRTUAL_NSISCRIPTCHANNEL \
  NS_METHOD GetExecutionPolicy(uint32_t *aExecutionPolicy); \
  NS_METHOD SetExecutionPolicy(uint32_t aExecutionPolicy); \
  NS_METHOD GetExecuteAsync(bool *aExecuteAsync); \
  NS_METHOD SetExecuteAsync(bool aExecuteAsync); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsScriptChannel, nsIScriptChannel)
