...
#define NS_DECL_NON_VIRTUAL_NSIDATASIGNATUREVERIFIER \
  NS_METHOD VerifyData(const nsACString & aData, const nsACString & aSignature, const nsACString & aPublicKey, bool *_retval); \
  NS_METHOD VerifySignature(const char * aSignature, uint32_t aSignatureLen, const char * plaintext, uint32_t plaintextLen, int32_t *errorCode, nsIX509Cert * *_retval); 
