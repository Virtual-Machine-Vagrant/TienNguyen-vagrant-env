...
#define NS_DECL_NON_VIRTUAL_NSIDIRINDEX \
  ...
  NS_METHOD GetLocation(char * *aLocation); \
  NS_METHOD SetLocation(const char * aLocation); \
  NS_METHOD GetDescription(char16_t * *aDescription); \
  NS_METHOD SetDescription(const char16_t * aDescription); \
  NS_METHOD GetSize(int64_t *aSize); \
  NS_METHOD SetSize(int64_t aSize); \
  NS_METHOD GetLastModified(PRTime *aLastModified); \
  NS_METHOD SetLastModified(PRTime aLastModified); 
