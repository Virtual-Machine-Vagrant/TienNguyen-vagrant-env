...
#define NS_DECL_NON_VIRTUAL_NSICOMPONENTMANAGER \
  NS_METHOD GetClassObject(const nsCID & aClass, const nsIID & aIID, void **result); \
  NS_METHOD GetClassObjectByContractID(const char * aContractID, const nsIID & aIID, void **result); \
  NS_METHOD CreateInstance(const nsCID & aClass, nsISupports *aDelegate, const nsIID & aIID, void **result); \
  NS_METHOD CreateInstanceByContractID(const char * aContractID, nsISupports *aDelegate, const nsIID & aIID, void **result); \
  NS_METHOD AddBootstrappedManifestLocation(nsIFile *aLocation); \
  NS_METHOD RemoveBootstrappedManifestLocation(nsIFile *aLocation); \
  NS_METHOD GetManifestLocations(nsIArray * *_retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsComponentManager, nsIComponentManager)
