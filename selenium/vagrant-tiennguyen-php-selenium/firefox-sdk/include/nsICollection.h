...
#define NS_DECL_NON_VIRTUAL_NSICOLLECTION \
  NS_METHOD Count(uint32_t *_retval); \
  NS_METHOD GetElementAt(uint32_t index, nsISupports * *_retval); \
  NS_METHOD QueryElementAt(uint32_t index, const nsIID & uuid, void **result); \
  NS_METHOD SetElementAt(uint32_t index, nsISupports *item); \
  NS_METHOD AppendElement(nsISupports *item); \
  NS_METHOD RemoveElement(nsISupports *item); \
  NS_METHOD Enumerate(nsIEnumerator * *_retval); \
