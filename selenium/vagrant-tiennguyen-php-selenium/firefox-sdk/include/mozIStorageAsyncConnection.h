...
#define NS_DECL_NON_VIRTUAL_MOZISTORAGEASYNCCONNECTION \
  NS_METHOD AsyncClose(mozIStorageCompletionCallback *aCallback); \
  NS_METHOD AsyncClone(bool aReadOnly, mozIStorageCompletionCallback *aCallback); \
  NS_METHOD GetDatabaseFile(nsIFile * *aDatabaseFile); \
  NS_METHOD CreateAsyncStatement(const nsACString & aSQLStatement, mozIStorageAsyncStatement * *_retval); \
  NS_METHOD ExecuteAsync(mozIStorageBaseStatement **aStatements, uint32_t aNumStatements, mozIStorageStatementCallback *aCallback, mozIStoragePendingStatement * *_retval); \
  NS_METHOD ExecuteSimpleSQLAsync(const nsACString & aSQLStatement, mozIStorageStatementCallback *aCallback, mozIStoragePendingStatement * *_retval); \
  NS_METHOD CreateFunction(const nsACString & aFunctionName, int32_t aNumArguments, mozIStorageFunction *aFunction); \
  NS_METHOD CreateAggregateFunction(const nsACString & aFunctionName, int32_t aNumArguments, mozIStorageAggregateFunction *aFunction); \
  NS_METHOD RemoveFunction(const nsACString & aFunctionName); \
  NS_METHOD SetProgressHandler(int32_t aGranularity, mozIStorageProgressHandler *aHandler, mozIStorageProgressHandler * *_retval); \
  NS_METHOD RemoveProgressHandler(mozIStorageProgressHandler * *_retval); 
