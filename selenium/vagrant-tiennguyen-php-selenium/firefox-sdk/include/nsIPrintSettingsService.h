...
#define NS_DECL_NON_VIRTUAL_NSIPRINTSETTINGSSERVICE \
  ...
  NS_METHOD GetNewPrintSettings(nsIPrintSettings * *aNewPrintSettings); \
  ...
  NS_METHOD InitPrintSettingsFromPrefs(nsIPrintSettings *aPrintSettings, bool aUsePrinterNamePrefix, uint32_t aFlags); \
  NS_METHOD SavePrintSettingsToPrefs(nsIPrintSettings *aPrintSettings, bool aUsePrinterNamePrefix, uint32_t aFlags); 
...
/* void initPrintSettingsFromPrinter (in wstring aPrinterName, in nsIPrintSettings aPrintSettings); */
NS_IMETHODIMP nsPrintSettingsService::InitPrintSettingsFromPrinter(const char16_t * aPrinterName, nsIPrintSettings *aPrintSettings)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
