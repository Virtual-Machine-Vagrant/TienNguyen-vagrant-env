...
#define NS_DECL_NON_VIRTUAL_NSIJSINSPECTOR \
  NS_METHOD EnterNestedEventLoop(JS::HandleValue requestor, uint32_t *_retval); \
  NS_METHOD ExitNestedEventLoop(uint32_t *_retval); \
  NS_METHOD GetEventLoopNestLevel(uint32_t *aEventLoopNestLevel); \
  NS_METHOD GetLastNestRequestor(JS::MutableHandleValue aLastNestRequestor); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsJSInspector, nsIJSInspector)
