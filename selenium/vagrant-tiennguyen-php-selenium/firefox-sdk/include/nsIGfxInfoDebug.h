...
#define NS_DECL_NON_VIRTUAL_NSIGFXINFODEBUG \
  NS_METHOD SpoofVendorID(const nsAString & aVendorID); \
  NS_METHOD SpoofDeviceID(const nsAString & aDeviceID); \
  NS_METHOD SpoofDriverVersion(const nsAString & aDriverVersion); \
  NS_METHOD SpoofOSVersion(uint32_t aVersion); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsGfxInfoDebug, nsIGfxInfoDebug)
