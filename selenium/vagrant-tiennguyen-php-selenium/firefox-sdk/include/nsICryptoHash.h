...
#define NS_DECL_NSICRYPTOHASH \
  ...
  NS_IMETHOD InitWithString(const nsACString & aAlgorithm) override; \
  NS_IMETHOD Update(const uint8_t *aData, uint32_t aLen) override; \
  NS_IMETHOD UpdateFromStream(nsIInputStream *aStream, uint32_t aLen) override; \
...
#define NS_DECL_NON_VIRTUAL_NSICRYPTOHASH \
  NS_METHOD Init(uint32_t aAlgorithm); \
  NS_METHOD InitWithString(const nsACString & aAlgorithm); \
  NS_METHOD Update(const uint8_t *aData, uint32_t aLen); \
  NS_METHOD UpdateFromStream(nsIInputStream *aStream, uint32_t aLen); \
  NS_METHOD Finish(bool aASCII, nsACString & _retval); 
...
#define NS_FORWARD_NSICRYPTOHASH(_to) \
  ...
  NS_IMETHOD InitWithString(const nsACString & aAlgorithm) override { return _to InitWithString(aAlgorithm); } \
  NS_IMETHOD Update(const uint8_t *aData, uint32_t aLen) override { return _to Update(aData, aLen); } \
  NS_IMETHOD UpdateFromStream(nsIInputStream *aStream, uint32_t aLen) override { return _to UpdateFromStream(aStream, aLen); } \
...
#define NS_FORWARD_SAFE_NSICRYPTOHASH(_to) \
  ...
  NS_IMETHOD InitWithString(const nsACString & aAlgorithm) override { return !_to ? NS_ERROR_NULL_POINTER : _to->InitWithString(aAlgorithm); } \
  NS_IMETHOD Update(const uint8_t *aData, uint32_t aLen) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Update(aData, aLen); } \
  NS_IMETHOD UpdateFromStream(nsIInputStream *aStream, uint32_t aLen) override { return !_to ? NS_ERROR_NULL_POINTER : _to->UpdateFromStream(aStream, aLen); } \
