...
#define NS_DECL_NON_VIRTUAL_NSIAUTOCOMPLETECONTROLLER \
  ...
  NS_METHOD SetInput(nsIAutoCompleteInput *aInput); \
  NS_METHOD GetSearchStatus(uint16_t *aSearchStatus); \
  NS_METHOD GetMatchCount(uint32_t *aMatchCount); \
  NS_METHOD StartSearch(const nsAString & searchString); \
  NS_METHOD StopSearch(void); \
  NS_METHOD HandleText(void); \
  NS_METHOD HandleEnter(bool aIsPopupSelection, bool *_retval); \
  NS_METHOD HandleEscape(bool *_retval); \
  NS_METHOD HandleStartComposition(void); \
  NS_METHOD HandleEndComposition(void); \
  NS_METHOD HandleTab(void); \
  NS_METHOD HandleKeyNavigation(uint32_t key, bool *_retval); \
  NS_METHOD HandleDelete(bool *_retval); \
  NS_METHOD GetValueAt(int32_t index, nsAString & _retval); \
  NS_METHOD GetLabelAt(int32_t index, nsAString & _retval); \
  NS_METHOD GetCommentAt(int32_t index, nsAString & _retval); \
  NS_METHOD GetStyleAt(int32_t index, nsAString & _retval); \
  NS_METHOD GetImageAt(int32_t index, nsAString & _retval); \
  NS_METHOD GetFinalCompleteValueAt(int32_t index, nsAString & _retval); \
