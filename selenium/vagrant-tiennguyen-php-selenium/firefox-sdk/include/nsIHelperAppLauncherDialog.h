...
#define NS_DECL_NON_VIRTUAL_NSIHELPERAPPLAUNCHERDIALOG \
  NS_METHOD Show(nsIHelperAppLauncher *aLauncher, nsISupports *aWindowContext, uint32_t aReason); \
  NS_METHOD PromptForSaveToFileAsync(nsIHelperAppLauncher *aLauncher, nsISupports *aWindowContext, const char16_t * aDefaultFileName, const char16_t * aSuggestedFileExtension, bool aForcePrompt); 
