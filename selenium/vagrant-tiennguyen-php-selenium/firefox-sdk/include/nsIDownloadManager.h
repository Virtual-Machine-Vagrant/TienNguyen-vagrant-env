...
#define NS_DECL_NON_VIRTUAL_NSIDOWNLOADMANAGERRESULT \
  NS_METHOD HandleResult(nsresult aStatus, nsIDownload *aDownload); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIDOWNLOADMANAGER \
  NS_METHOD AddDownload(int16_t aDownloadType, nsIURI *aSource, nsIURI *aTarget, const nsAString & aDisplayName, nsIMIMEInfo *aMIMEInfo, PRTime aStartTime, nsIFile *aTempFile, nsICancelable *aCancelable, bool aIsPrivate, nsIDownload * *_retval); \
  NS_METHOD GetDownload(uint32_t aID, nsIDownload * *_retval); \
  NS_METHOD GetDownloadByGUID(const nsACString & aGUID, nsIDownloadManagerResult *aCallback); \
  NS_METHOD CancelDownload(uint32_t aID); \
  NS_METHOD RemoveDownload(uint32_t aID); \
  NS_METHOD RemoveDownloadsByTimeframe(int64_t aBeginTime, int64_t aEndTime); \
  NS_METHOD PauseDownload(uint32_t aID); \
  NS_METHOD ResumeDownload(uint32_t aID); \
  NS_METHOD RetryDownload(uint32_t aID); \
  NS_METHOD GetDBConnection(mozIStorageConnection * *aDBConnection); \
  NS_METHOD GetPrivateDBConnection(mozIStorageConnection * *aPrivateDBConnection); \
  NS_METHOD GetCanCleanUp(bool *aCanCleanUp); \
  NS_METHOD GetCanCleanUpPrivate(bool *aCanCleanUpPrivate); \
  NS_METHOD CleanUp(void); \
  NS_METHOD CleanUpPrivate(void); \
  NS_METHOD GetActiveDownloadCount(int32_t *aActiveDownloadCount); \
  NS_METHOD GetActivePrivateDownloadCount(int32_t *aActivePrivateDownloadCount); \
  NS_METHOD GetActiveDownloads(nsISimpleEnumerator * *aActiveDownloads); \
  NS_METHOD GetActivePrivateDownloads(nsISimpleEnumerator * *aActivePrivateDownloads); \
  NS_METHOD AddListener(nsIDownloadProgressListener *aListener); \
  NS_METHOD AddPrivacyAwareListener(nsIDownloadProgressListener *aListener); \
  NS_METHOD RemoveListener(nsIDownloadProgressListener *aListener); \
  NS_METHOD GetDefaultDownloadsDirectory(nsIFile * *aDefaultDownloadsDirectory); \
  NS_METHOD GetUserDownloadsDirectory(nsIFile * *aUserDownloadsDirectory); 
