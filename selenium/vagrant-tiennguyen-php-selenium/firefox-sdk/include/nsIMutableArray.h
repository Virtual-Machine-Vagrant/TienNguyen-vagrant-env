...
#define NS_DECL_NON_VIRTUAL_NSIMUTABLEARRAY \
  NS_METHOD AppendElement(nsISupports *element, bool weak); \
  NS_METHOD RemoveElementAt(uint32_t index); \
  NS_METHOD InsertElementAt(nsISupports *element, uint32_t index, bool weak); \
  NS_METHOD ReplaceElementAt(nsISupports *element, uint32_t index, bool weak); \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsMutableArray, nsIMutableArray)
