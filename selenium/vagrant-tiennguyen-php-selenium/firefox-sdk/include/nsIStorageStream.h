...
#define NS_DECL_NON_VIRTUAL_NSISTORAGESTREAM \
  NS_METHOD Init(uint32_t segmentSize, uint32_t maxSize); \
  NS_METHOD GetOutputStream(int32_t startPosition, nsIOutputStream * *_retval); \
  NS_METHOD NewInputStream(int32_t startPosition, nsIInputStream * *_retval); \
  NS_METHOD GetLength(uint32_t *aLength); \
  NS_METHOD SetLength(uint32_t aLength); \
  NS_METHOD GetWriteInProgress(bool *aWriteInProgress); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsStorageStream, nsIStorageStream)
