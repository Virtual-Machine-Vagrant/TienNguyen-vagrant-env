...
#define NS_DECL_NON_VIRTUAL_NSIEDITOR \
  NS_METHOD GetSelection(nsISelection * *aSelection); \
  NS_METHOD FinalizeSelection(void); \
  NS_METHOD Init(nsIDOMDocument *doc, nsIContent *aRoot, nsISelectionController *aSelCon, uint32_t aFlags, const nsAString & initialValue); \
  NS_METHOD SetAttributeOrEquivalent(nsIDOMElement *element, const nsAString & sourceAttrName, const nsAString & sourceAttrValue, bool aSuppressTransaction); \
  NS_METHOD RemoveAttributeOrEquivalent(nsIDOMElement *element, const nsAString & sourceAttrName, bool aSuppressTransaction); \
  NS_METHOD PostCreate(void); \
  NS_METHOD PreDestroy(bool aDestroyingFrames); \
  NS_METHOD GetFlags(uint32_t *aFlags); \
  NS_METHOD SetFlags(uint32_t aFlags); \
  NS_METHOD GetContentsMIMEType(char * *aContentsMIMEType); \
  NS_METHOD SetContentsMIMEType(const char * aContentsMIMEType); \
  NS_METHOD GetIsDocumentEditable(bool *aIsDocumentEditable); \
  NS_METHOD GetIsSelectionEditable(bool *aIsSelectionEditable); \
  NS_METHOD GetDocument(nsIDOMDocument * *aDocument); \
  NS_METHOD GetRootElement(nsIDOMElement * *aRootElement); \
  NS_METHOD GetSelectionController(nsISelectionController * *aSelectionController); \
  NS_METHOD DeleteSelection(int16_t action, int16_t stripWrappers); \
  NS_METHOD GetDocumentIsEmpty(bool *aDocumentIsEmpty); \
  NS_METHOD GetDocumentModified(bool *aDocumentModified); \
  NS_METHOD GetDocumentCharacterSet(nsACString & aDocumentCharacterSet); \
  NS_METHOD SetDocumentCharacterSet(const nsACString & aDocumentCharacterSet); \
  NS_METHOD ResetModificationCount(void); \
  NS_METHOD GetModificationCount(int32_t *_retval); \
  NS_METHOD IncrementModificationCount(int32_t aModCount); \
  ...
  NS_METHOD DoTransaction(nsITransaction *txn); \
  NS_METHOD EnableUndo(bool enable); \
  NS_METHOD GetNumberOfUndoItems(int32_t *aNumberOfUndoItems); \
  NS_METHOD GetNumberOfRedoItems(int32_t *aNumberOfRedoItems); \
  NS_METHOD Undo(uint32_t count); \
  NS_METHOD CanUndo(bool *isEnabled, bool *canUndo); \
  NS_METHOD Redo(uint32_t count); \
  NS_METHOD CanRedo(bool *isEnabled, bool *canRedo); \
  NS_METHOD BeginTransaction(void); \
  NS_METHOD EndTransaction(void); \
  NS_METHOD BeginPlaceHolderTransaction(nsIAtom *name); \
  NS_METHOD EndPlaceHolderTransaction(void); \
  NS_METHOD ShouldTxnSetSelection(bool *_retval); \
  NS_METHOD SetShouldTxnSetSelection(bool should); \
  NS_METHOD GetInlineSpellChecker(bool autoCreate, nsIInlineSpellChecker * *_retval); \
  NS_METHOD SyncRealTimeSpell(void); \
  NS_METHOD SetSpellcheckUserOverride(bool enable); \
  NS_METHOD Cut(void); \
  NS_METHOD CanCut(bool *_retval); \
  NS_METHOD Copy(void); \
  NS_METHOD CanCopy(bool *_retval); \
  NS_METHOD CanDelete(bool *_retval); \
  NS_METHOD Paste(int32_t aSelectionType); \
  NS_METHOD PasteTransferable(nsITransferable *aTransferable); \
  NS_METHOD CanPaste(int32_t aSelectionType, bool *_retval); \
  NS_METHOD CanPasteTransferable(nsITransferable *aTransferable, bool *_retval); \
  NS_METHOD SelectAll(void); \
  NS_METHOD BeginningOfDocument(void); \
  NS_METHOD EndOfDocument(void); \
  NS_METHOD SetAttribute(nsIDOMElement *aElement, const nsAString & attributestr, const nsAString & attvalue); \
  NS_METHOD GetAttributeValue(nsIDOMElement *aElement, const nsAString & attributestr, nsAString & resultValue, bool *_retval); \
  NS_METHOD RemoveAttribute(nsIDOMElement *aElement, const nsAString & aAttribute); \
  NS_METHOD CloneAttribute(const nsAString & aAttribute, nsIDOMNode *aDestNode, nsIDOMNode *aSourceNode); \
  NS_METHOD CloneAttributes(nsIDOMNode *destNode, nsIDOMNode *sourceNode); \
  NS_METHOD CreateNode(const nsAString & tag, nsIDOMNode *parent, int32_t position, nsIDOMNode * *_retval); \
  NS_METHOD InsertNode(nsIDOMNode *node, nsIDOMNode *parent, int32_t aPosition); \
  NS_METHOD SplitNode(nsIDOMNode *existingRightNode, int32_t offset, nsIDOMNode * *newLeftNode); \
  NS_METHOD JoinNodes(nsIDOMNode *leftNode, nsIDOMNode *rightNode, nsIDOMNode *parent); \
  NS_METHOD DeleteNode(nsIDOMNode *child); \
  NS_METHOD_(bool) OutputsMozDirty(void); \
  NS_METHOD MarkNodeDirty(nsIDOMNode *node); \
  NS_METHOD SwitchTextDirection(void); \
  NS_METHOD OutputToString(const nsAString & formatType, uint32_t flags, nsAString & _retval); \
  NS_METHOD OutputToStream(nsIOutputStream *aStream, const nsAString & formatType, const nsACString & charsetOverride, uint32_t flags); \
  NS_METHOD AddEditorObserver(nsIEditorObserver *observer); \
  NS_METHOD RemoveEditorObserver(nsIEditorObserver *observer); \
  NS_METHOD AddEditActionListener(nsIEditActionListener *listener); \
  NS_METHOD RemoveEditActionListener(nsIEditActionListener *listener); \
  NS_METHOD AddDocumentStateListener(nsIDocumentStateListener *listener); \
  NS_METHOD RemoveDocumentStateListener(nsIDocumentStateListener *listener); \
  NS_METHOD DumpContentTree(void); \
  NS_METHOD DebugDumpContent(void); \
  NS_METHOD DebugUnitTests(int32_t *outNumTests, int32_t *outNumTestsFailed); \
  NS_METHOD_(bool) IsModifiableNode(nsIDOMNode *aNode); \
  NS_METHOD GetSuppressDispatchingInputEvent(bool *aSuppressDispatchingInputEvent); \
  NS_METHOD SetSuppressDispatchingInputEvent(bool aSuppressDispatchingInputEvent); \
  NS_METHOD GetIsInEditAction(bool *aIsInEditAction); 
