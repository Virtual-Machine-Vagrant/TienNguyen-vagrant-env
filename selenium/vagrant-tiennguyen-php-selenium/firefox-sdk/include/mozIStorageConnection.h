...
#define NS_DECL_NON_VIRTUAL_MOZISTORAGECONNECTION \
  ...
  NS_METHOD Clone(bool aReadOnly, mozIStorageConnection * *_retval); \
  NS_METHOD GetDefaultPageSize(int32_t *aDefaultPageSize); \
  NS_METHOD GetConnectionReady(bool *aConnectionReady); \
  NS_METHOD GetLastInsertRowID(int64_t *aLastInsertRowID); \
  NS_METHOD GetAffectedRows(int32_t *aAffectedRows); \
  NS_METHOD GetLastError(int32_t *aLastError); \
  NS_METHOD GetLastErrorString(nsACString & aLastErrorString); \
  NS_METHOD GetSchemaVersion(int32_t *aSchemaVersion); \
  NS_METHOD SetSchemaVersion(int32_t aSchemaVersion); \
  NS_METHOD CreateStatement(const nsACString & aSQLStatement, mozIStorageStatement * *_retval); \
  NS_METHOD ExecuteSimpleSQL(const nsACString & aSQLStatement); \
  NS_METHOD TableExists(const nsACString & aTableName, bool *_retval); \
  NS_METHOD IndexExists(const nsACString & aIndexName, bool *_retval); \
  NS_METHOD GetTransactionInProgress(bool *aTransactionInProgress); \
  NS_METHOD BeginTransaction(void); \
  NS_METHOD BeginTransactionAs(int32_t transactionType); \
  NS_METHOD CommitTransaction(void); \
  NS_METHOD RollbackTransaction(void); \
  NS_METHOD CreateTable(const char * aTableName, const char * aTableSchema); \
  NS_METHOD SetGrowthIncrement(int32_t aIncrement, const nsACString & aDatabaseName); \
  NS_METHOD EnableModule(const nsACString & aModuleName); 
