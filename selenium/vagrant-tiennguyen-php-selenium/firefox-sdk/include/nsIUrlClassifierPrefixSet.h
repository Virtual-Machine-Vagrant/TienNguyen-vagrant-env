...
#define NS_DECL_NON_VIRTUAL_NSIURLCLASSIFIERPREFIXSET \
  NS_METHOD Init(const nsACString & aName); \
  NS_METHOD SetPrefixes(const uint32_t *aPrefixes, uint32_t aLength); \
  NS_METHOD GetPrefixes(uint32_t *aCount, uint32_t **aPrefixes); \
  NS_METHOD Contains(uint32_t aPrefix, bool *_retval); \
  NS_METHOD IsEmpty(bool *_retval); \
  NS_METHOD LoadFromFile(nsIFile *aFile); \
  NS_METHOD StoreToFile(nsIFile *aFile); 
...
#define NS_FORWARD_NSIURLCLASSIFIERPREFIXSET(_to) \
  ...
  NS_IMETHOD LoadFromFile(nsIFile *aFile) override { return _to LoadFromFile(aFile); } \
  NS_IMETHOD StoreToFile(nsIFile *aFile) override { return _to StoreToFile(aFile); } 
...
#define NS_FORWARD_SAFE_NSIURLCLASSIFIERPREFIXSET(_to) \
  ...
  NS_IMETHOD LoadFromFile(nsIFile *aFile) override { return !_to ? NS_ERROR_NULL_POINTER : _to->LoadFromFile(aFile); } \
  NS_IMETHOD StoreToFile(nsIFile *aFile) override { return !_to ? NS_ERROR_NULL_POINTER : _to->StoreToFile(aFile); } 
