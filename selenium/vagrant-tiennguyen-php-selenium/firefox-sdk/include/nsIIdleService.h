...
#define NS_DECL_NON_VIRTUAL_NSIIDLESERVICE \
  NS_METHOD GetIdleTime(uint32_t *aIdleTime); \
  NS_METHOD AddIdleObserver(nsIObserver *observer, uint32_t time); \
  NS_METHOD RemoveIdleObserver(nsIObserver *observer, uint32_t time); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsIdleService, nsIIdleService)
