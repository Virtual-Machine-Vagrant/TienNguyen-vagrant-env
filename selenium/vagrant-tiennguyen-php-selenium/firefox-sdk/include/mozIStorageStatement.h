...
#define NS_DECL_MOZISTORAGESTATEMENT \
  ...
  NS_IMETHOD GetColumnDecltype(uint32_t aParamIndex, nsACString & _retval) override; \
...
#define NS_DECL_NON_VIRTUAL_MOZISTORAGESTATEMENT \
  NS_METHOD Clone(mozIStorageStatement * *_retval); \
  NS_METHOD GetParameterCount(uint32_t *aParameterCount); \
  NS_METHOD GetParameterName(uint32_t aParamIndex, nsACString & _retval); \
  NS_METHOD GetParameterIndex(const nsACString & aName, uint32_t *_retval); \
  NS_METHOD GetColumnCount(uint32_t *aColumnCount); \
  NS_METHOD GetColumnName(uint32_t aColumnIndex, nsACString & _retval); \
  NS_METHOD GetColumnIndex(const nsACString & aName, uint32_t *_retval); \
  NS_METHOD GetColumnDecltype(uint32_t aParamIndex, nsACString & _retval); \
  NS_METHOD Reset(void); \
  NS_METHOD Execute(void); \
  NS_METHOD ExecuteStep(bool *_retval); \
  NS_METHOD GetNumEntries(uint32_t *aNumEntries); \
  NS_METHOD GetTypeOfIndex(uint32_t aIndex, int32_t *_retval); \
  NS_METHOD GetInt32(uint32_t aIndex, int32_t *_retval); \
  NS_METHOD GetInt64(uint32_t aIndex, int64_t *_retval); \
  NS_METHOD GetDouble(uint32_t aIndex, double *_retval); \
  NS_METHOD GetUTF8String(uint32_t aIndex, nsACString & _retval); \
  NS_METHOD GetString(uint32_t aIndex, nsAString & _retval); \
  NS_METHOD GetBlob(uint32_t aIndex, uint32_t *aDataSize, uint8_t **aData); \
  NS_METHOD GetBlobAsString(uint32_t aIndex, nsAString & _retval); \
  NS_METHOD GetBlobAsUTF8String(uint32_t aIndex, nsACString & _retval); \
  NS_METHOD GetIsNull(uint32_t aIndex, bool *_retval); \
  NS_METHOD GetSharedUTF8String(uint32_t aIndex, uint32_t *aLength, const char * *aResult); \
  NS_METHOD GetSharedString(uint32_t aIndex, uint32_t *aLength, const char16_t * *aResult); \
  NS_METHOD GetSharedBlob(uint32_t aIndex, uint32_t *aLength, const uint8_t **aResult); \
...
#define NS_FORWARD_MOZISTORAGESTATEMENT(_to) \
  ...
  NS_IMETHOD GetColumnDecltype(uint32_t aParamIndex, nsACString & _retval) override { return _to GetColumnDecltype(aParamIndex, _retval); } \
...
#define NS_FORWARD_SAFE_MOZISTORAGESTATEMENT(_to) \
  ...
  NS_IMETHOD GetColumnDecltype(uint32_t aParamIndex, nsACString & _retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetColumnDecltype(aParamIndex, _retval); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(_MYCLASS_, mozIStorageStatement)
...
/* AString getBlobAsString (in unsigned long aIndex); */
NS_IMETHODIMP _MYCLASS_::GetBlobAsString(uint32_t aIndex, nsAString & _retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

/* AUTF8String getBlobAsUTF8String (in unsigned long aIndex); */
NS_IMETHODIMP _MYCLASS_::GetBlobAsUTF8String(uint32_t aIndex, nsACString & _retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
