...
#define NS_DECL_NSISCRIPTLOADEROBSERVER \
  NS_IMETHOD ScriptAvailable(nsresult aResult, nsIScriptElement *aElement, bool aIsInline, nsIURI *aURI, int32_t aLineNo) override; \
...
#define NS_DECL_NON_VIRTUAL_NSISCRIPTLOADEROBSERVER \
  NS_METHOD ScriptAvailable(nsresult aResult, nsIScriptElement *aElement, bool aIsInline, nsIURI *aURI, int32_t aLineNo); \
  NS_METHOD ScriptEvaluated(nsresult aResult, nsIScriptElement *aElement, bool aIsInline); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSISCRIPTLOADEROBSERVER(_to) \
  NS_IMETHOD ScriptAvailable(nsresult aResult, nsIScriptElement *aElement, bool aIsInline, nsIURI *aURI, int32_t aLineNo) override { return _to ScriptAvailable(aResult, aElement, aIsInline, aURI, aLineNo); } \
...
#define NS_FORWARD_SAFE_NSISCRIPTLOADEROBSERVER(_to) \
  NS_IMETHOD ScriptAvailable(nsresult aResult, nsIScriptElement *aElement, bool aIsInline, nsIURI *aURI, int32_t aLineNo) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ScriptAvailable(aResult, aElement, aIsInline, aURI, aLineNo); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsScriptLoaderObserver, nsIScriptLoaderObserver)
