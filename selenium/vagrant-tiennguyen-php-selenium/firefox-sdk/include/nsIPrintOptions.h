...
#define NS_DECL_NON_VIRTUAL_NSIPRINTOPTIONS \
  NS_METHOD ShowPrintSetupDialog(nsIPrintSettings *aThePrintSettings); \
  NS_METHOD CreatePrintSettings(nsIPrintSettings * *_retval); \
  NS_METHOD GetPrinterPrefInt(nsIPrintSettings *aPrintSettings, const char16_t * aPrefName, int32_t *_retval); \
  NS_METHOD DisplayJobProperties(const char16_t * aPrinter, nsIPrintSettings *aPrintSettings, bool *aDisplayed); \
  NS_METHOD GetNativeData(int16_t aDataType, void **_retval); \
  NS_METHOD SerializeToPrintData(nsIPrintSettings *aPrintSettings, nsIWebBrowserPrint *aWebBrowserPrint, mozilla::embedding::PrintData *data); \
  NS_METHOD DeserializeToPrintSettings(const mozilla::embedding::PrintData & data, nsIPrintSettings *aPrintSettings); 
...
#define NS_DECL_NON_VIRTUAL_NSIPRINTERENUMERATOR \
  ...
  NS_METHOD DisplayPropertiesDlg(const char16_t * aPrinter, nsIPrintSettings *aPrintSettings); 
