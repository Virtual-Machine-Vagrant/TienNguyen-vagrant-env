...
#define NS_DECL_NON_VIRTUAL_NSIAUTHMODULE \
  NS_METHOD Init(const char * aServiceName, uint32_t aServiceFlags, const char16_t * aDomain, const char16_t * aUsername, const char16_t * aPassword); \
  NS_METHOD GetNextToken(const void *aInToken, uint32_t aInTokenLength, void **aOutToken, uint32_t *aOutTokenLength); \
  NS_METHOD Wrap(const void *aInToken, uint32_t aInTokenLength, bool confidential, void **aOutToken, uint32_t *aOutTokenLength); \
  NS_METHOD Unwrap(const void *aInToken, uint32_t aInTokenLength, void **aOutToken, uint32_t *aOutTokenLength); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsAuthModule, nsIAuthModule)
