...
#define NS_ITELEPHONYLISTENER_IID_STR "baa9f5f3-5cab-40e0-81e9-ae0abd917907"

#define NS_ITELEPHONYLISTENER_IID \
  {0xbaa9f5f3, 0x5cab, 0x40e0, \
    { 0x81, 0xe9, 0xae, 0x0a, 0xbd, 0x91, 0x79, 0x07 }}
...
#define NS_DECL_NON_VIRTUAL_NSITELEPHONYLISTENER \
  NS_METHOD EnumerateCallStateComplete(void); \
  NS_METHOD EnumerateCallState(nsITelephonyCallInfo *info); \
  NS_METHOD CallStateChanged(uint32_t length, nsITelephonyCallInfo **allInfo); \
  NS_METHOD SupplementaryServiceNotification(uint32_t clientId, int32_t callIndex, uint16_t notification); \
  NS_METHOD NotifyCdmaCallWaiting(uint32_t clientId, const nsAString & number, uint16_t numberPresentation, const nsAString & name, uint16_t namePresentation); \
  NS_METHOD NotifyConferenceError(const nsAString & name, const nsAString & message); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSITELEPHONYCALLBACK \
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSITELEPHONYDIALCALLBACK \
  NS_METHOD NotifyDialMMI(const nsAString & serviceCode); \
  NS_METHOD NotifyDialCallSuccess(uint32_t cliendId, uint32_t callIndex, const nsAString & number); \
  NS_METHOD NotifyDialMMISuccess(const nsAString & statusMessage); \
  NS_METHOD NotifyDialMMISuccessWithInteger(const nsAString & statusMessage, uint16_t aAdditionalInformation); \
  NS_METHOD NotifyDialMMISuccessWithStrings(const nsAString & statusMessage, uint32_t aLength, const char16_t * *aAdditionalInformation); \
  NS_METHOD NotifyDialMMISuccessWithCallForwardingOptions(const nsAString & statusMessage, uint32_t aLength, nsIMobileCallForwardingOptions **aAdditionalInformation); \
  NS_METHOD NotifyDialMMIError(const nsAString & error); \
  NS_METHOD NotifyDialMMIErrorWithInfo(const nsAString & error, uint16_t info); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSITELEPHONYSERVICE \
  NS_METHOD GetDefaultServiceId(uint32_t *aDefaultServiceId); \
  NS_METHOD RegisterListener(nsITelephonyListener *listener); \
  NS_METHOD UnregisterListener(nsITelephonyListener *listener); \
  NS_METHOD EnumerateCalls(nsITelephonyListener *listener); \
  NS_METHOD Dial(uint32_t clientId, const nsAString & number, bool isEmergency, nsITelephonyDialCallback *callback); \
  NS_METHOD SendTones(uint32_t clientId, const nsAString & dtmfChars, uint32_t pauseDuration, uint32_t toneDuration, nsITelephonyCallback *callback); \
  NS_METHOD StartTone(uint32_t clientId, const nsAString & dtmfChar); \
  NS_METHOD StopTone(uint32_t clientId); \
  NS_METHOD AnswerCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD RejectCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD HangUpCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD HoldCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD ResumeCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD ConferenceCall(uint32_t clientId, nsITelephonyCallback *callback); \
  NS_METHOD SeparateCall(uint32_t clientId, uint32_t callIndex, nsITelephonyCallback *callback); \
  NS_METHOD HangUpConference(uint32_t clientId, nsITelephonyCallback *callback); \
  NS_METHOD HoldConference(uint32_t clientId, nsITelephonyCallback *callback); \
  NS_METHOD ResumeConference(uint32_t clientId, nsITelephonyCallback *callback); \
  NS_METHOD SendUSSD(uint32_t clientId, const nsAString & ussd, nsITelephonyCallback *callback); \
  NS_METHOD CancelUSSD(uint32_t cliendId, nsITelephonyCallback *callback); \
  NS_METHOD GetMicrophoneMuted(bool *aMicrophoneMuted); \
  NS_METHOD SetMicrophoneMuted(bool aMicrophoneMuted); \
  NS_METHOD GetSpeakerEnabled(bool *aSpeakerEnabled); \
  NS_METHOD SetSpeakerEnabled(bool aSpeakerEnabled); 
