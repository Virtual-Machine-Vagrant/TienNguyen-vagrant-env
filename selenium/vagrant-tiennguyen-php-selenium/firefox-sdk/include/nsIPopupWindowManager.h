...
#define NS_DECL_NSIPOPUPWINDOWMANAGER \
  NS_IMETHOD TestPermission(nsIPrincipal *principal, uint32_t *_retval) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIPOPUPWINDOWMANAGER \
  NS_METHOD TestPermission(nsIPrincipal *principal, uint32_t *_retval); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIPOPUPWINDOWMANAGER(_to) \
  NS_IMETHOD TestPermission(nsIPrincipal *principal, uint32_t *_retval) override { return _to TestPermission(principal, _retval); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIPOPUPWINDOWMANAGER(_to) \
  NS_IMETHOD TestPermission(nsIPrincipal *principal, uint32_t *_retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->TestPermission(principal, _retval); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsPopupWindowManager, nsIPopupWindowManager)
