...
#define NS_DECL_NON_VIRTUAL_NSICACHEENTRY \
  ...
  NS_METHOD GetPersistent(bool *aPersistent); \
  NS_METHOD GetFetchCount(int32_t *aFetchCount); \
  NS_METHOD GetLastFetched(uint32_t *aLastFetched); \
  NS_METHOD GetLastModified(uint32_t *aLastModified); \
  NS_METHOD GetExpirationTime(uint32_t *aExpirationTime); \
  NS_METHOD SetExpirationTime(uint32_t expirationTime); \
  NS_METHOD ForceValidFor(uint32_t aSecondsToTheFuture); \
  NS_METHOD GetIsForcedValid(bool *aIsForcedValid); \
  NS_METHOD OpenInputStream(int64_t offset, nsIInputStream * *_retval); \
  NS_METHOD OpenOutputStream(int64_t offset, nsIOutputStream * *_retval); \
  NS_METHOD GetPredictedDataSize(int64_t *aPredictedDataSize); \
  NS_METHOD SetPredictedDataSize(int64_t aPredictedDataSize); \
  NS_METHOD GetSecurityInfo(nsISupports * *aSecurityInfo); \
  NS_METHOD SetSecurityInfo(nsISupports *aSecurityInfo); \
  NS_METHOD GetStorageDataSize(uint32_t *aStorageDataSize); \
  NS_METHOD AsyncDoom(nsICacheEntryDoomCallback *listener); \
  NS_METHOD GetMetaDataElement(const char * key, char * *_retval); \
  NS_METHOD SetMetaDataElement(const char * key, const char * value); \
  NS_METHOD VisitMetaData(nsICacheEntryMetaDataVisitor *visitor); \
  NS_METHOD MetaDataReady(void); \
  NS_METHOD SetValid(void); \
  NS_METHOD Recreate(bool aMemoryOnly, nsICacheEntry * *_retval); \
  NS_METHOD GetDataSize(int64_t *aDataSize); \
  NS_METHOD Close(void); \
  NS_METHOD MarkValid(void); \
  NS_METHOD MaybeMarkValid(void); \
  NS_METHOD HasWriteAccess(bool aWriteAllowed, bool *_retval); 
...
#define NS_DECL_NON_VIRTUAL_NSICACHEENTRYMETADATAVISITOR \
  NS_METHOD OnMetaDataElement(const char * key, const char * value); 
