...
#define NS_DECL_NON_VIRTUAL_NSICONTEXTMENULISTENER2 \
  NS_METHOD OnShowContextMenu(uint32_t aContextFlags, nsIContextMenuInfo *aUtils); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSICONTEXTMENUINFO \
  NS_METHOD GetMouseEvent(nsIDOMEvent * *aMouseEvent); \
  NS_METHOD GetTargetNode(nsIDOMNode * *aTargetNode); \
  NS_METHOD GetAssociatedLink(nsAString & aAssociatedLink); \
  NS_METHOD GetImageContainer(imgIContainer * *aImageContainer); \
  NS_METHOD GetImageSrc(nsIURI * *aImageSrc); \
  NS_METHOD GetBackgroundImageContainer(imgIContainer * *aBackgroundImageContainer); \
  NS_METHOD GetBackgroundImageSrc(nsIURI * *aBackgroundImageSrc); 
