...
#define NS_DECL_NSICHILDCHANNEL \
  NS_IMETHOD ConnectParent(uint32_t id) override; \
  NS_IMETHOD CompleteRedirectSetup(nsIStreamListener *aListener, nsISupports *aContext) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSICHILDCHANNEL \
  NS_METHOD ConnectParent(uint32_t id); \
  NS_METHOD CompleteRedirectSetup(nsIStreamListener *aListener, nsISupports *aContext); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSICHILDCHANNEL(_to) \
  NS_IMETHOD ConnectParent(uint32_t id) override { return _to ConnectParent(id); } \
  NS_IMETHOD CompleteRedirectSetup(nsIStreamListener *aListener, nsISupports *aContext) override { return _to CompleteRedirectSetup(aListener, aContext); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSICHILDCHANNEL(_to) \
  NS_IMETHOD ConnectParent(uint32_t id) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ConnectParent(id); } \
  NS_IMETHOD CompleteRedirectSetup(nsIStreamListener *aListener, nsISupports *aContext) override { return !_to ? NS_ERROR_NULL_POINTER : _to->CompleteRedirectSetup(aListener, aContext); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsChildChannel, nsIChildChannel)
