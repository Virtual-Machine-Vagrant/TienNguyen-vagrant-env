...
#define NS_DECL_NON_VIRTUAL_MOZISTORAGEVACUUMPARTICIPANT \
  NS_METHOD GetExpectedDatabasePageSize(int32_t *aExpectedDatabasePageSize); \
  NS_METHOD GetDatabaseConnection(mozIStorageConnection * *aDatabaseConnection); \
  NS_METHOD OnBeginVacuum(bool *_retval); \
  NS_METHOD OnEndVacuum(bool aSucceeded); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(_MYCLASS_, mozIStorageVacuumParticipant)
