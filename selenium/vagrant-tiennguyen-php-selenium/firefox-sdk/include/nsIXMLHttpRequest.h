...
#define NS_DECL_NON_VIRTUAL_NSIXMLHTTPREQUESTEVENTTARGET \
  /* no methods! */
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIXMLHTTPREQUESTUPLOAD \
  /* no methods! */
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIXMLHTTPREQUEST \
  NS_METHOD GetChannel(nsIChannel * *aChannel); \
  NS_METHOD GetResponseXML(nsIDOMDocument * *aResponseXML); \
  NS_METHOD GetResponseText(nsAString & aResponseText); \
  NS_METHOD GetResponseType(nsAString & aResponseType); \
  NS_METHOD SetResponseType(const nsAString & aResponseType); \
  NS_METHOD GetResponse(JSContext* cx, JS::MutableHandleValue aResponse); \
  NS_METHOD GetStatus(uint32_t *aStatus); \
  NS_METHOD GetStatusText(nsACString & aStatusText); \
  NS_METHOD SlowAbort(void); \
  NS_METHOD GetAllResponseHeaders(nsACString & _retval); \
  NS_METHOD GetResponseHeader(const nsACString & header, nsACString & _retval); \
  NS_METHOD Open(const nsACString & method, const nsACString & url, bool async, const nsAString & user, const nsAString & password, uint8_t _argc); \
  NS_METHOD Send(nsIVariant *body); \
  NS_METHOD SetRequestHeader(const nsACString & header, const nsACString & value); \
  NS_METHOD GetTimeout(uint32_t *aTimeout); \
  NS_METHOD SetTimeout(uint32_t aTimeout); \
  NS_METHOD GetReadyState(uint16_t *aReadyState); \
  NS_METHOD SlowOverrideMimeType(const nsAString & mimetype); \
  NS_METHOD GetMozBackgroundRequest(bool *aMozBackgroundRequest); \
  NS_METHOD SetMozBackgroundRequest(bool aMozBackgroundRequest); \
  NS_METHOD GetWithCredentials(bool *aWithCredentials); \
  NS_METHOD SetWithCredentials(bool aWithCredentials); \
  NS_METHOD Init(nsIPrincipal *principal, nsIScriptContext *scriptContext, nsIGlobalObject *globalObject, nsIURI *baseURI, nsILoadGroup *loadGroup); \
  NS_METHOD GetUpload(nsIXMLHttpRequestUpload * *aUpload); \
  NS_METHOD GetOnreadystatechange(JSContext* cx, JS::MutableHandleValue aOnreadystatechange); \
  NS_METHOD SetOnreadystatechange(JSContext* cx, JS::HandleValue aOnreadystatechange); \
  NS_METHOD GetMozAnon(bool *aMozAnon); \
  NS_METHOD GetMozSystem(bool *aMozSystem); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIXHRSENDABLE \
  NS_METHOD GetSendInfo(nsIInputStream * *body, uint64_t *contentLength, nsACString & contentType, nsACString & charset); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIJSXMLHTTPREQUEST \
  /* no methods! */
