...
#define NS_DECL_NON_VIRTUAL_NSIPARENTALCONTROLSSERVICE \
  NS_METHOD GetParentalControlsEnabled(bool *aParentalControlsEnabled); \
  NS_METHOD GetBlockFileDownloadsEnabled(bool *aBlockFileDownloadsEnabled); \
  NS_METHOD IsAllowed(int16_t aAction, nsIURI *aUri, bool *_retval); \
  NS_METHOD RequestURIOverride(nsIURI *aTarget, nsIInterfaceRequestor *aWindowContext, bool *_retval); \
  NS_METHOD RequestURIOverrides(nsIArray *aTargets, nsIInterfaceRequestor *aWindowContext, bool *_retval); \
  NS_METHOD GetLoggingEnabled(bool *aLoggingEnabled); \
  NS_METHOD Log(int16_t aEntryType, bool aFlag, nsIURI *aSource, nsIFile *aTarget); 
