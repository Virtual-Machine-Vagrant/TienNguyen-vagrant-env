...
#define NS_IDOMHTMLDOCUMENT_IID_STR "cd31e61f-cfc2-4b91-9385-17b6a2d0633d"

#define NS_IDOMHTMLDOCUMENT_IID \
  {0xcd31e61f, 0xcfc2, 0x4b91, \
...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLDOCUMENT \
  NS_METHOD GetDomain(nsAString & aDomain); \
  NS_METHOD SetDomain(const nsAString & aDomain); \
  NS_METHOD GetCookie(nsAString & aCookie); \
  NS_METHOD SetCookie(const nsAString & aCookie); \
  NS_METHOD GetHead(nsIDOMHTMLHeadElement * *aHead); \
  NS_METHOD GetBody(nsIDOMHTMLElement * *aBody); \
  NS_METHOD SetBody(nsIDOMHTMLElement *aBody); \
  NS_METHOD GetImages(nsIDOMHTMLCollection * *aImages); \
  NS_METHOD GetEmbeds(nsIDOMHTMLCollection * *aEmbeds); \
  NS_METHOD GetPlugins(nsIDOMHTMLCollection * *aPlugins); \
  NS_METHOD GetLinks(nsIDOMHTMLCollection * *aLinks); \
  NS_METHOD GetForms(nsIDOMHTMLCollection * *aForms); \
  NS_METHOD GetScripts(nsIDOMHTMLCollection * *aScripts); \
  NS_METHOD GetElementsByName(const nsAString & elementName, nsIDOMNodeList * *_retval); \
  NS_METHOD GetItems(const nsAString & types, nsIDOMNodeList * *_retval); \
  NS_METHOD Open(const nsAString & aContentTypeOrUrl, const nsAString & aReplaceOrName, const nsAString & aFeatures, JSContext* cx, uint8_t _argc, nsISupports * *_retval); \
  NS_METHOD Close(void); \
  NS_METHOD Write(const nsAString & text, JSContext* cx); \
  NS_METHOD Writeln(const nsAString & text, JSContext* cx); \
  NS_METHOD GetDesignMode(nsAString & aDesignMode); \
  NS_METHOD SetDesignMode(const nsAString & aDesignMode); \
  NS_METHOD ExecCommand(const nsAString & commandID, bool doShowUI, const nsAString & value, bool *_retval); \
  NS_METHOD QueryCommandEnabled(const nsAString & commandID, bool *_retval); \
  NS_METHOD QueryCommandIndeterm(const nsAString & commandID, bool *_retval); \
  NS_METHOD QueryCommandState(const nsAString & commandID, bool *_retval); \
  NS_METHOD QueryCommandSupported(const nsAString & commandID, bool *_retval); \
  NS_METHOD QueryCommandValue(const nsAString & commandID, nsAString & _retval); \
  NS_METHOD GetFgColor(nsAString & aFgColor); \
  NS_METHOD SetFgColor(const nsAString & aFgColor); \
  NS_METHOD GetBgColor(nsAString & aBgColor); \
  NS_METHOD SetBgColor(const nsAString & aBgColor); \
  NS_METHOD GetLinkColor(nsAString & aLinkColor); \
  NS_METHOD SetLinkColor(const nsAString & aLinkColor); \
  NS_METHOD GetVlinkColor(nsAString & aVlinkColor); \
  NS_METHOD SetVlinkColor(const nsAString & aVlinkColor); \
  NS_METHOD GetAlinkColor(nsAString & aAlinkColor); \
  NS_METHOD SetAlinkColor(const nsAString & aAlinkColor); \
  NS_METHOD GetAnchors(nsIDOMHTMLCollection * *aAnchors); \
  NS_METHOD GetApplets(nsIDOMHTMLCollection * *aApplets); \
  NS_METHOD Clear(void); \
  NS_METHOD GetSelection(nsISelection * *_retval); \
  NS_METHOD CaptureEvents(void); \
  NS_METHOD ReleaseEvents(void); 
