...
#define NS_DECL_NON_VIRTUAL_NSISECRETDECODERRING \
  NS_METHOD Encrypt(unsigned char *data, int32_t dataLen, unsigned char **result, int32_t *_retval); \
  NS_METHOD Decrypt(unsigned char *data, int32_t dataLen, unsigned char **result, int32_t *_retval); \
  NS_METHOD EncryptString(const char * text, char * *_retval); \
  NS_METHOD DecryptString(const char * crypt, char * *_retval); \
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSISECRETDECODERRINGCONFIG \
  NS_METHOD SetWindow(nsISupports *w); 
