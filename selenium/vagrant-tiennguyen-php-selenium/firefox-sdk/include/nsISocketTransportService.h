...
#define NS_DECL_NON_VIRTUAL_NSISOCKETTRANSPORTSERVICE \
  NS_METHOD CreateTransport(const char * *aSocketTypes, uint32_t aTypeCount, const nsACString & aHost, int32_t aPort, nsIProxyInfo *aProxyInfo, nsISocketTransport * *_retval); \
  NS_METHOD CreateUnixDomainTransport(nsIFile *aPath, nsISocketTransport * *_retval); \
  NS_METHOD AttachSocket(PRFileDesc *aFd, nsASocketHandler *aHandler); \
  NS_METHOD NotifyWhenCanAttachSocket(nsIRunnable *aEvent); 
...
#define NS_DECL_NON_VIRTUAL_NSIROUTEDSOCKETTRANSPORTSERVICE \
  NS_METHOD CreateRoutedTransport(const char * *aSocketTypes, uint32_t aTypeCount, const nsACString & aHost, int32_t aPort, const nsACString & aHostRoute, int32_t aPortRoute, nsIProxyInfo *aProxyInfo, nsISocketTransport * *_retval); 
