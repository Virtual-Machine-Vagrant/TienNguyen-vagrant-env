...
#define NS_DECL_NON_VIRTUAL_NSIASYNCOUTPUTSTREAM \
  NS_METHOD CloseWithStatus(nsresult reason); \
  NS_METHOD AsyncWait(nsIOutputStreamCallback *aCallback, uint32_t aFlags, uint32_t aRequestedCount, nsIEventTarget *aEventTarget); \
...
#define NS_DECL_NON_VIRTUAL_NSIOUTPUTSTREAMCALLBACK \
  NS_METHOD OnOutputStreamReady(nsIAsyncOutputStream *aStream); 
