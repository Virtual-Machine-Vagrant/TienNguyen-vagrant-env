...
#define NS_IDOMDOCUMENT_IID_STR "b15fa0f4-97c1-4388-af62-2ceff7a89bdf"

#define NS_IDOMDOCUMENT_IID \
  {0xb15fa0f4, 0x97c1, 0x4388, \
...
class NS_NO_VTABLE nsIDOMDocument : public nsIDOMNode {
 public:
  ...
  NS_IMETHOD GetDefaultView(mozIDOMWindowProxy * *aDefaultView) = 0;
...
#define NS_DECL_NSIDOMDOCUMENT \
  ...
  NS_IMETHOD GetDefaultView(mozIDOMWindowProxy * *aDefaultView) override; \
...
#define NS_DECL_NON_VIRTUAL_NSIDOMDOCUMENT \
  NS_METHOD GetDoctype(nsIDOMDocumentType * *aDoctype); \
  NS_METHOD GetImplementation(nsIDOMDOMImplementation * *aImplementation); \
  NS_METHOD GetDocumentElement(nsIDOMElement * *aDocumentElement); \
  NS_METHOD CreateElement(const nsAString & tagName, nsIDOMElement * *_retval); \
  NS_METHOD CreateDocumentFragment(nsIDOMDocumentFragment * *_retval); \
  NS_METHOD CreateTextNode(const nsAString & data, nsIDOMText * *_retval); \
  NS_METHOD CreateComment(const nsAString & data, nsIDOMComment * *_retval); \
  NS_METHOD CreateCDATASection(const nsAString & data, nsIDOMCDATASection * *_retval); \
  NS_METHOD CreateProcessingInstruction(const nsAString & target, const nsAString & data, nsIDOMProcessingInstruction * *_retval); \
  NS_METHOD CreateAttribute(const nsAString & name, nsIDOMAttr * *_retval); \
  NS_METHOD GetElementsByTagName(const nsAString & tagname, nsIDOMNodeList * *_retval); \
  NS_METHOD ImportNode(nsIDOMNode *importedNode, bool deep, uint8_t _argc, nsIDOMNode * *_retval); \
  NS_METHOD CreateElementNS(const nsAString & namespaceURI, const nsAString & qualifiedName, nsIDOMElement * *_retval); \
  NS_METHOD CreateAttributeNS(const nsAString & namespaceURI, const nsAString & qualifiedName, nsIDOMAttr * *_retval); \
  NS_METHOD GetElementsByTagNameNS(const nsAString & namespaceURI, const nsAString & localName, nsIDOMNodeList * *_retval); \
  NS_METHOD GetElementById(const nsAString & elementId, nsIDOMElement * *_retval); \
  NS_METHOD GetInputEncoding(nsAString & aInputEncoding); \
  NS_METHOD GetDocumentURI(nsAString & aDocumentURI); \
  NS_METHOD GetURL(nsAString & aURL); \
  NS_METHOD AdoptNode(nsIDOMNode *source, nsIDOMNode * *_retval); \
  NS_METHOD CreateRange(nsIDOMRange * *_retval); \
  NS_METHOD CreateNodeIterator(nsIDOMNode *root, uint32_t whatToShow, nsIDOMNodeFilter *filter, uint8_t _argc, nsIDOMNodeIterator * *_retval); \
  NS_METHOD CreateTreeWalker(nsIDOMNode *root, uint32_t whatToShow, nsIDOMNodeFilter *filter, uint8_t _argc, nsIDOMTreeWalker * *_retval); \
  NS_METHOD CreateEvent(const nsAString & eventType, nsIDOMEvent * *_retval); \
  NS_METHOD GetDefaultView(mozIDOMWindowProxy * *aDefaultView); \
  NS_METHOD GetCharacterSet(nsAString & aCharacterSet); \
  NS_METHOD GetDir(nsAString & aDir); \
  NS_METHOD SetDir(const nsAString & aDir); \
  NS_METHOD GetLocation(nsIDOMLocation * *aLocation); \
  ...
  NS_METHOD GetLastModified(nsAString & aLastModified); \
  NS_METHOD GetReferrer(nsAString & aReferrer); \
  NS_METHOD HasFocus(bool *_retval); \
  NS_METHOD GetActiveElement(nsIDOMElement * *aActiveElement); \
  NS_METHOD GetElementsByClassName(const nsAString & classes, nsIDOMNodeList * *_retval); \
  NS_METHOD GetStyleSheets(nsIDOMStyleSheetList * *aStyleSheets); \
  NS_METHOD GetPreferredStyleSheetSet(nsAString & aPreferredStyleSheetSet); \
  NS_METHOD GetMozSelectedStyleSheetSet(nsAString & aSelectedStyleSheetSet); \
  NS_METHOD SetMozSelectedStyleSheetSet(const nsAString & aSelectedStyleSheetSet); \
  NS_METHOD GetLastStyleSheetSet(nsAString & aLastStyleSheetSet); \
  NS_METHOD GetStyleSheetSets(nsISupports * *aStyleSheetSets); \
  NS_METHOD MozEnableStyleSheetsForSet(const nsAString & name); \
  NS_METHOD ElementFromPoint(float x, float y, nsIDOMElement * *_retval); \
  NS_METHOD GetContentType(nsAString & aContentType); \
  NS_METHOD GetMozSyntheticDocument(bool *aMozSyntheticDocument); \
  NS_METHOD GetCurrentScript(nsIDOMElement * *aCurrentScript); \
  NS_METHOD ReleaseCapture(void); \
  NS_METHOD MozSetImageElement(const nsAString & aImageElementId, nsIDOMElement *aImageElement); \
  NS_METHOD GetMozFullScreenElement(nsIDOMElement * *aMozFullScreenElement); \
  NS_METHOD MozCancelFullScreen(void); \
  NS_METHOD GetMozFullScreen(bool *aMozFullScreen); \
  NS_METHOD GetMozFullScreenEnabled(bool *aMozFullScreenEnabled); \
  NS_METHOD GetMozPointerLockElement(nsIDOMElement * *aMozPointerLockElement); \
  NS_METHOD CaretPositionFromPoint(float x, float y, nsISupports * *_retval); \
  NS_METHOD MozExitPointerLock(void); \
  NS_METHOD GetHidden(bool *aHidden); \
  NS_METHOD GetMozHidden(bool *aMozHidden); \
  NS_METHOD GetVisibilityState(nsAString & aVisibilityState); \
  NS_METHOD GetMozVisibilityState(nsAString & aMozVisibilityState); \
  NS_METHOD GetCompatMode(nsAString & aCompatMode); \
  NS_METHOD QuerySelector(const nsAString & selectors, nsIDOMElement * *_retval); \
  NS_METHOD QuerySelectorAll(const nsAString & selectors, nsIDOMNodeList * *_retval); 
...
#define NS_FORWARD_NSIDOMDOCUMENT(_to) \
  ...
  NS_IMETHOD GetDefaultView(mozIDOMWindowProxy * *aDefaultView) override { return _to GetDefaultView(aDefaultView); } \
...
#define NS_FORWARD_SAFE_NSIDOMDOCUMENT(_to) \
  ...
  NS_IMETHOD GetDefaultView(mozIDOMWindowProxy * *aDefaultView) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetDefaultView(aDefaultView); } \
...
/* readonly attribute mozIDOMWindowProxy defaultView; */
NS_IMETHODIMP nsDOMDocument::GetDefaultView(mozIDOMWindowProxy * *aDefaultView)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
