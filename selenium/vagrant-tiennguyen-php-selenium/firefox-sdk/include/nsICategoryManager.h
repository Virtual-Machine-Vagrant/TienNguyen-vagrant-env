...
#define NS_DECL_NON_VIRTUAL_NSICATEGORYMANAGER \
  NS_METHOD GetCategoryEntry(const char * aCategory, const char * aEntry, char * *_retval); \
  NS_METHOD AddCategoryEntry(const char * aCategory, const char * aEntry, const char * aValue, bool aPersist, bool aReplace, char * *_retval); \
  NS_METHOD DeleteCategoryEntry(const char * aCategory, const char * aEntry, bool aPersist); \
  NS_METHOD DeleteCategory(const char * aCategory); \
  NS_METHOD EnumerateCategory(const char * aCategory, nsISimpleEnumerator * *_retval); \
  NS_METHOD EnumerateCategories(nsISimpleEnumerator * *_retval); 
