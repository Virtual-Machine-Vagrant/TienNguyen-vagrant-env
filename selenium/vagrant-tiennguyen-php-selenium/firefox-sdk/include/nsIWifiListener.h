...
#define NS_DECL_NSIWIFILISTENER \
  NS_IMETHOD OnChange(nsIWifiAccessPoint **accessPoints, uint32_t aLen) override; \
  NS_IMETHOD OnError(nsresult error) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIWIFILISTENER \
  NS_METHOD OnChange(nsIWifiAccessPoint **accessPoints, uint32_t aLen); \
  NS_METHOD OnError(nsresult error); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIWIFILISTENER(_to) \
  NS_IMETHOD OnChange(nsIWifiAccessPoint **accessPoints, uint32_t aLen) override { return _to OnChange(accessPoints, aLen); } \
...
#define NS_FORWARD_SAFE_NSIWIFILISTENER(_to) \
  NS_IMETHOD OnChange(nsIWifiAccessPoint **accessPoints, uint32_t aLen) override { return !_to ? NS_ERROR_NULL_POINTER : _to->OnChange(accessPoints, aLen); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsWifiListener, nsIWifiListener)
