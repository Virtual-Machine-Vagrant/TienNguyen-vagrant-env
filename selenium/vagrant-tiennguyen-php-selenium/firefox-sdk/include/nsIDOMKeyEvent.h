...
#define NS_DECL_NON_VIRTUAL_NSIDOMKEYEVENT \
  NS_METHOD GetCharCode(uint32_t *aCharCode); \
  NS_METHOD GetKeyCode(uint32_t *aKeyCode); \
  ...
  NS_METHOD InitKeyEvent(const nsAString & typeArg, bool canBubbleArg, bool cancelableArg, mozIDOMWindow *viewArg, bool ctrlKeyArg, bool altKeyArg, bool shiftKeyArg, bool metaKeyArg, uint32_t keyCodeArg, uint32_t charCodeArg); \
  NS_METHOD GetModifierState(const nsAString & keyArg, bool *_retval); \
  NS_METHOD GetLocation(uint32_t *aLocation); \
  NS_METHOD GetRepeat(bool *aRepeat); \
  NS_METHOD GetKey(nsAString & aKey); 
