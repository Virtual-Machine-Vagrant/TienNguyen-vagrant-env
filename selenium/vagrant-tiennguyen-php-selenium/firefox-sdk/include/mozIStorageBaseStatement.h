...
#define NS_DECL_NON_VIRTUAL_MOZISTORAGEBASESTATEMENT \
  NS_METHOD Finalize(void); \
  NS_METHOD BindUTF8StringParameter(uint32_t aParamIndex, const nsACString & aValue); \
  NS_METHOD BindStringParameter(uint32_t aParamIndex, const nsAString & aValue); \
  NS_METHOD BindDoubleParameter(uint32_t aParamIndex, double aValue); \
  NS_METHOD BindInt32Parameter(uint32_t aParamIndex, int32_t aValue); \
  NS_METHOD BindInt64Parameter(uint32_t aParamIndex, int64_t aValue); \
  NS_METHOD BindNullParameter(uint32_t aParamIndex); \
  NS_METHOD BindBlobParameter(uint32_t aParamIndex, const uint8_t *aValue, uint32_t aValueSize); \
  NS_METHOD BindStringAsBlobParameter(uint32_t aParamIndex, const nsAString & aValue); \
  NS_METHOD BindUTF8StringAsBlobParameter(uint32_t aParamIndex, const nsACString & aValue); \
  NS_METHOD BindAdoptedBlobParameter(uint32_t aParamIndex, uint8_t *aValue, uint32_t aValueSize); \
  NS_METHOD BindParameters(mozIStorageBindingParamsArray *aParameters); \
  NS_METHOD NewBindingParamsArray(mozIStorageBindingParamsArray * *_retval); \
  NS_METHOD ExecuteAsync(mozIStorageStatementCallback *aCallback, mozIStoragePendingStatement * *_retval); \
  NS_METHOD GetState(int32_t *aState); \
  NS_METHOD EscapeStringForLIKE(const nsAString & aValue, char16_t aEscapeChar, nsAString & _retval); 
