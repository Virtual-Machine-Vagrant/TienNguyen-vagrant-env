...
#define NS_DECL_NSICOMMANDLINERUNNER \
  ...
  NS_IMETHOD SetWindowContext(nsIDOMWindow *aWindow) override; \
...
#define NS_DECL_NON_VIRTUAL_NSICOMMANDLINERUNNER \
  NS_METHOD Init(int32_t argc, const char* const *argv, nsIFile *workingDir, uint32_t state); \
  NS_METHOD SetWindowContext(nsIDOMWindow *aWindow); \
  NS_METHOD Run(void); \
  NS_METHOD GetHelpText(nsACString & aHelpText); 
...
#define NS_FORWARD_NSICOMMANDLINERUNNER(_to) \
  ...
  NS_IMETHOD SetWindowContext(nsIDOMWindow *aWindow) override { return _to SetWindowContext(aWindow); } \
...
#define NS_FORWARD_SAFE_NSICOMMANDLINERUNNER(_to) \
  ...
  NS_IMETHOD SetWindowContext(nsIDOMWindow *aWindow) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SetWindowContext(aWindow); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsCommandLineRunner, nsICommandLineRunner)
