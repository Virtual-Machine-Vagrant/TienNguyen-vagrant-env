...
#define NS_DECL_NON_VIRTUAL_NSIDASHBOARDEVENTNOTIFIER \
  NS_METHOD AddHost(const nsACString & aHost, uint32_t aSerial, bool aEncrypted); \
  NS_METHOD RemoveHost(const nsACString & aHost, uint32_t aSerial); \
  NS_METHOD NewMsgSent(const nsACString & aHost, uint32_t aSerial, uint32_t aLength); \
  NS_METHOD NewMsgReceived(const nsACString & aHost, uint32_t aSerial, uint32_t aLength); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsDashboardEventNotifier, nsIDashboardEventNotifier)
