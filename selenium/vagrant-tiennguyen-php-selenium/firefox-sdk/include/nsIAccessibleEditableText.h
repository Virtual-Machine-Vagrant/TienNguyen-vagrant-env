...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLEEDITABLETEXT \
  NS_METHOD SetTextContents(const nsAString & text); \
  NS_METHOD InsertText(const nsAString & text, int32_t position); \
  NS_METHOD CopyText(int32_t startPos, int32_t endPos); \
  NS_METHOD CutText(int32_t startPos, int32_t endPos); \
  NS_METHOD DeleteText(int32_t startPos, int32_t endPos); \
  NS_METHOD PasteText(int32_t position); 
