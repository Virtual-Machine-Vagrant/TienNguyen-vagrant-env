...
#define NS_DECL_NON_VIRTUAL_NSIAUTOCOMPLETERESULT \
  ...
  NS_METHOD GetSearchResult(uint16_t *aSearchResult); \
  NS_METHOD GetDefaultIndex(int32_t *aDefaultIndex); \
  NS_METHOD GetErrorDescription(nsAString & aErrorDescription); \
  NS_METHOD GetMatchCount(uint32_t *aMatchCount); \
  NS_METHOD GetTypeAheadResult(bool *aTypeAheadResult); \
  ...
  NS_METHOD RemoveValueAt(int32_t rowIndex, bool removeFromDb); 
