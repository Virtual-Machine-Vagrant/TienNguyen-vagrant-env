...
#define NS_DECL_NON_VIRTUAL_NSICONSOLEAPISTORAGE \
  NS_METHOD GetEvents(const nsAString & aId, JS::MutableHandleValue _retval); \
  NS_METHOD RecordEvent(const nsAString & aId, const nsAString & aOuterId, JS::HandleValue aEvent); \
  NS_METHOD ClearEvents(const nsAString & aId); 
