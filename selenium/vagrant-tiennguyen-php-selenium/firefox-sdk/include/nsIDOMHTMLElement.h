...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLELEMENT \
  ...
  NS_METHOD GetDataset(nsISupports * *aDataset); \
  NS_METHOD GetItemScope(bool *aItemScope); \
  NS_METHOD SetItemScope(bool aItemScope); \
  NS_METHOD GetItemType(nsIVariant * *aItemType); \
  NS_METHOD SetItemType(nsIVariant *aItemType); \
  NS_METHOD GetItemId(nsAString & aItemId); \
  NS_METHOD SetItemId(const nsAString & aItemId); \
  NS_METHOD GetProperties(nsISupports * *aProperties); \
  NS_METHOD GetItemValue(nsIVariant * *aItemValue); \
  NS_METHOD SetItemValue(nsIVariant *aItemValue); \
  NS_METHOD GetItemProp(nsIVariant * *aItemProp); \
  NS_METHOD SetItemProp(nsIVariant *aItemProp); \
  NS_METHOD GetItemRef(nsIVariant * *aItemRef); \
  NS_METHOD SetItemRef(nsIVariant *aItemRef); \
  NS_METHOD GetHidden(bool *aHidden); \
  NS_METHOD SetHidden(bool aHidden); \
  NS_METHOD DOMClick(void); \
  NS_METHOD GetTabIndex(int32_t *aTabIndex); \
  NS_METHOD SetTabIndex(int32_t aTabIndex); \
  NS_METHOD Focus(void); \
  NS_METHOD DOMBlur(void); \
  NS_METHOD GetAccessKey(nsAString & aAccessKey); \
  NS_METHOD SetAccessKey(const nsAString & aAccessKey); \
  NS_METHOD GetAccessKeyLabel(nsAString & aAccessKeyLabel); \
  NS_METHOD GetDraggable(bool *aDraggable); \
  NS_METHOD SetDraggable(bool aDraggable); \
  NS_METHOD GetContentEditable(nsAString & aContentEditable); \
  NS_METHOD SetContentEditable(const nsAString & aContentEditable); \
  NS_METHOD GetIsContentEditable(bool *aIsContentEditable); \
  NS_METHOD GetContextMenu(nsIDOMHTMLMenuElement * *aContextMenu); \
  NS_METHOD GetSpellcheck(bool *aSpellcheck); \
  NS_METHOD SetSpellcheck(bool aSpellcheck); \
  NS_METHOD GetInnerHTML(nsAString & aInnerHTML); \
  NS_METHOD SetInnerHTML(const nsAString & aInnerHTML); \
  NS_METHOD GetOuterHTML(nsAString & aOuterHTML); \
  NS_METHOD SetOuterHTML(const nsAString & aOuterHTML); \
  NS_METHOD InsertAdjacentHTML(const nsAString & position, const nsAString & text); \
  NS_METHOD ScrollIntoView(bool top, uint8_t _argc); \
  NS_METHOD GetOffsetParent(nsIDOMElement * *aOffsetParent); \
  NS_METHOD GetOffsetTop(int32_t *aOffsetTop); \
  NS_METHOD GetOffsetLeft(int32_t *aOffsetLeft); \
  NS_METHOD GetOffsetWidth(int32_t *aOffsetWidth); \
  NS_METHOD GetOffsetHeight(int32_t *aOffsetHeight); 
