...
#define NS_DECL_NON_VIRTUAL_NSIAPPLICATIONCACHENAMESPACE \
  NS_METHOD Init(uint32_t itemType, const nsACString & namespaceSpec, const nsACString & data); \
  NS_METHOD GetItemType(uint32_t *aItemType); \
  NS_METHOD GetNamespaceSpec(nsACString & aNamespaceSpec); \
...
#define NS_DECL_NON_VIRTUAL_NSIAPPLICATIONCACHE \
  NS_METHOD InitAsHandle(const nsACString & groupId, const nsACString & clientId); \
  NS_METHOD GetManifestURI(nsIURI * *aManifestURI); \
  NS_METHOD GetGroupID(nsACString & aGroupID); \
  NS_METHOD GetClientID(nsACString & aClientID); \
  NS_METHOD GetActive(bool *aActive); \
  NS_METHOD GetUsage(uint32_t *aUsage); \
  NS_METHOD Activate(void); \
  NS_METHOD Discard(void); \
  NS_METHOD MarkEntry(const nsACString & key, uint32_t typeBits); \
  NS_METHOD UnmarkEntry(const nsACString & key, uint32_t typeBits); \
  NS_METHOD GetTypes(const nsACString & key, uint32_t *_retval); \
  NS_METHOD GatherEntries(uint32_t typeBits, uint32_t *count, char * **keys); \
  NS_METHOD AddNamespaces(nsIArray *namespaces); \
  NS_METHOD GetMatchingNamespace(const nsACString & key, nsIApplicationCacheNamespace * *_retval); \
