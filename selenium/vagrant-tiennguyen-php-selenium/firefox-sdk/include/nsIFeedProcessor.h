...
#define NS_DECL_NON_VIRTUAL_NSIFEEDPROCESSOR \
  NS_METHOD GetListener(nsIFeedResultListener * *aListener); \
  NS_METHOD SetListener(nsIFeedResultListener *aListener); \
  NS_METHOD ParseFromStream(nsIInputStream *stream, nsIURI *uri); \
  NS_METHOD ParseFromString(const nsAString & str, nsIURI *uri); \
  NS_METHOD ParseAsync(nsIRequestObserver *requestObserver, nsIURI *uri); 
