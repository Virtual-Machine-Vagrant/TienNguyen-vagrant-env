...
#define NS_DECL_NON_VIRTUAL_NSILOCALFILEWIN \
  NS_METHOD GetVersionInfoField(const char * aField, nsAString & _retval); \
  NS_METHOD GetCanonicalPath(nsAString & aCanonicalPath); \
  NS_METHOD GetNativeCanonicalPath(nsACString & aNativeCanonicalPath); \
  NS_METHOD GetFileAttributesWin(uint32_t *aFileAttributesWin); \
  NS_METHOD SetFileAttributesWin(uint32_t aFileAttributesWin); \
  NS_METHOD SetShortcut(nsIFile *targetFile, nsIFile *workingDir, const char16_t * args, const char16_t * description, nsIFile *iconFile, int32_t iconIndex); \
  NS_METHOD OpenNSPRFileDescShareDelete(int32_t flags, int32_t mode, PRFileDesc **_retval); 
