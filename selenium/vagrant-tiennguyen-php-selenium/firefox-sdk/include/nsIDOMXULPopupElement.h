...
#define NS_DECL_NON_VIRTUAL_NSIDOMXULPOPUPELEMENT \
  NS_METHOD GetPosition(nsAString & aPosition); \
  NS_METHOD SetPosition(const nsAString & aPosition); \
  NS_METHOD ShowPopup(uint16_t alignment, nsIDOMElement *target, nsIDOMElement *anchor); \
  NS_METHOD HidePopup(void); 
