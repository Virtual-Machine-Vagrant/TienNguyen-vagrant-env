...
#define NS_DECL_NON_VIRTUAL_NSIMOBILECONNECTIONINFO \
  NS_METHOD GetState(nsAString & aState); \
  NS_METHOD GetConnected(bool *aConnected); \
  NS_METHOD GetEmergencyCallsOnly(bool *aEmergencyCallsOnly); \
  NS_METHOD GetRoaming(bool *aRoaming); \
  NS_METHOD GetNetwork(nsIMobileNetworkInfo * *aNetwork); \
  NS_METHOD GetType(nsAString & aType); \
  NS_METHOD GetSignalStrength(JS::MutableHandleValue aSignalStrength); \
  NS_METHOD GetRelSignalStrength(JS::MutableHandleValue aRelSignalStrength); \
  NS_METHOD GetCell(nsIMobileCellInfo * *aCell); 
