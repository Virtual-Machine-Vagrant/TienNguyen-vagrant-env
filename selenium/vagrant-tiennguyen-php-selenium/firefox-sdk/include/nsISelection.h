...
#define NS_DECL_NON_VIRTUAL_NSISELECTION \
  NS_METHOD GetAnchorNode(nsIDOMNode * *aAnchorNode); \
  NS_METHOD GetAnchorOffset(int32_t *aAnchorOffset); \
  NS_METHOD GetFocusNode(nsIDOMNode * *aFocusNode); \
  NS_METHOD GetFocusOffset(int32_t *aFocusOffset); \
  NS_METHOD GetIsCollapsed(bool *aIsCollapsed); \
  bool Collapsed(void); \
  NS_METHOD GetRangeCount(int32_t *aRangeCount); \
  NS_METHOD GetRangeAt(int32_t index, nsIDOMRange * *_retval); \
  NS_METHOD Collapse(nsIDOMNode *parentNode, int32_t offset); \
  NS_METHOD CollapseNative(nsINode *parentNode, int32_t offset); \
  NS_METHOD Extend(nsIDOMNode *parentNode, int32_t offset); \
  NS_METHOD ExtendNative(nsINode *parentNode, int32_t offset); \
  NS_METHOD CollapseToStart(void); \
  NS_METHOD CollapseToEnd(void); \
  NS_METHOD ContainsNode(nsIDOMNode *node, bool partlyContained, bool *_retval); \
  NS_METHOD SelectAllChildren(nsIDOMNode *parentNode); \
  NS_METHOD AddRange(nsIDOMRange *range); \
  NS_METHOD RemoveRange(nsIDOMRange *range); \
  NS_METHOD RemoveAllRanges(void); \
  NS_METHOD DeleteFromDocument(void); \
  NS_METHOD ToString(nsAString & _retval); \
  NS_METHOD Modify(const nsAString & alter, const nsAString & direction, const nsAString & granularity); 
