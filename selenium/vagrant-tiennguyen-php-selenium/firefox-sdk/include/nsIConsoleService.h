...
#define NS_DECL_NON_VIRTUAL_NSICONSOLESERVICE \
  NS_METHOD LogMessage(nsIConsoleMessage *message); \
  NS_METHOD LogStringMessage(const char16_t * message); \
  NS_METHOD GetMessageArray(uint32_t *count, nsIConsoleMessage * **messages); \
  NS_METHOD RegisterListener(nsIConsoleListener *listener); \
  NS_METHOD UnregisterListener(nsIConsoleListener *listener); \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsConsoleService, nsIConsoleService)
