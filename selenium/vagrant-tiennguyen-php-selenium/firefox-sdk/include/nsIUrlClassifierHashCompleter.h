...
#define NS_DECL_NON_VIRTUAL_NSIURLCLASSIFIERHASHCOMPLETERCALLBACK \
  NS_METHOD Completion(const nsACString & hash, const nsACString & table, uint32_t chunkId); \
  NS_METHOD CompletionFinished(nsresult status); 
...
#define NS_DECL_NON_VIRTUAL_NSIURLCLASSIFIERHASHCOMPLETER \
  NS_METHOD Complete(const nsACString & partialHash, const nsACString & gethashUrl, nsIUrlClassifierHashCompleterCallback *callback); 
