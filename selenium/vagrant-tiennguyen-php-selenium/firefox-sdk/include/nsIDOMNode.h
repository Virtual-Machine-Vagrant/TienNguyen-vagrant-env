...
#define NS_DECL_NON_VIRTUAL_NSIDOMNODE \
  NS_METHOD GetNodeName(nsAString & aNodeName); \
  NS_METHOD GetNodeValue(nsAString & aNodeValue); \
  NS_METHOD SetNodeValue(const nsAString & aNodeValue); \
  NS_METHOD GetNodeType(uint16_t *aNodeType); \
  NS_METHOD GetParentNode(nsIDOMNode * *aParentNode); \
  NS_METHOD GetParentElement(nsIDOMElement * *aParentElement); \
  ...
  NS_METHOD GetPreviousSibling(nsIDOMNode * *aPreviousSibling); \
  NS_METHOD GetNextSibling(nsIDOMNode * *aNextSibling); \
  NS_METHOD GetOwnerDocument(nsIDOMDocument * *aOwnerDocument); \
  NS_METHOD InsertBefore(nsIDOMNode *newChild, nsIDOMNode *refChild, nsIDOMNode * *_retval); \
  NS_METHOD ReplaceChild(nsIDOMNode *newChild, nsIDOMNode *oldChild, nsIDOMNode * *_retval); \
  NS_METHOD RemoveChild(nsIDOMNode *oldChild, nsIDOMNode * *_retval); \
  NS_METHOD AppendChild(nsIDOMNode *newChild, nsIDOMNode * *_retval); \
  NS_METHOD HasChildNodes(bool *_retval); \
  NS_METHOD CloneNode(bool deep, uint8_t _argc, nsIDOMNode * *_retval); \
  NS_METHOD Normalize(void); \
  NS_METHOD GetNamespaceURI(nsAString & aNamespaceURI); \
  NS_METHOD GetPrefix(nsAString & aPrefix); \
  NS_METHOD GetLocalName(nsAString & aLocalName); \
  NS_METHOD UnusedPlaceholder(bool *_retval); \
  NS_METHOD GetDOMBaseURI(nsAString & aBaseURI); \
  NS_METHOD CompareDocumentPosition(nsIDOMNode *other, uint16_t *_retval); \
  NS_METHOD GetTextContent(nsAString & aTextContent); \
  NS_METHOD SetTextContent(const nsAString & aTextContent); \
  NS_METHOD LookupPrefix(const nsAString & namespaceURI, nsAString & _retval); \
  NS_METHOD IsDefaultNamespace(const nsAString & namespaceURI, bool *_retval); \
  NS_METHOD LookupNamespaceURI(const nsAString & prefix, nsAString & _retval); \
  NS_METHOD IsEqualNode(nsIDOMNode *arg, bool *_retval); \
  NS_METHOD SetUserData(const nsAString & key, nsIVariant *data, nsIVariant * *_retval); \
  NS_METHOD GetUserData(const nsAString & key, nsIVariant * *_retval); \
  NS_METHOD Contains(nsIDOMNode *aOther, bool *_retval); 
