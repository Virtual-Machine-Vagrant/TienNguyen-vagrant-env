...
#define NS_DECL_NON_VIRTUAL_NSITREEBOXOBJECT \
  NS_METHOD GetColumns(nsITreeColumns * *aColumns); \
  NS_METHOD GetView(nsITreeView * *aView); \
  NS_METHOD SetView(nsITreeView *aView); \
  NS_METHOD GetFocused(bool *aFocused); \
  NS_METHOD SetFocused(bool aFocused); \
  NS_METHOD GetTreeBody(nsIDOMElement * *aTreeBody); \
  NS_METHOD GetRowHeight(int32_t *aRowHeight); \
  NS_METHOD GetRowWidth(int32_t *aRowWidth); \
  NS_METHOD GetHorizontalPosition(int32_t *aHorizontalPosition); \
  NS_METHOD GetSelectionRegion(nsIScriptableRegion * *aSelectionRegion); \
  NS_METHOD GetFirstVisibleRow(int32_t *_retval); \
  NS_METHOD GetLastVisibleRow(int32_t *_retval); \
  NS_METHOD GetPageLength(int32_t *_retval); \
  NS_METHOD EnsureRowIsVisible(int32_t index); \
  NS_METHOD EnsureCellIsVisible(int32_t row, nsITreeColumn *col); \
  NS_METHOD ScrollToRow(int32_t index); \
  NS_METHOD ScrollByLines(int32_t numLines); \
  NS_METHOD ScrollByPages(int32_t numPages); \
  NS_METHOD ScrollToCell(int32_t row, nsITreeColumn *col); \
  NS_METHOD ScrollToColumn(nsITreeColumn *col); \
  NS_METHOD ScrollToHorizontalPosition(int32_t horizontalPosition); \
  NS_METHOD Invalidate(void); \
  NS_METHOD InvalidateColumn(nsITreeColumn *col); \
  NS_METHOD InvalidateRow(int32_t index); \
  NS_METHOD InvalidateCell(int32_t row, nsITreeColumn *col); \
  NS_METHOD InvalidateRange(int32_t startIndex, int32_t endIndex); \
  NS_METHOD InvalidateColumnRange(int32_t startIndex, int32_t endIndex, nsITreeColumn *col); \
  NS_METHOD GetRowAt(int32_t x, int32_t y, int32_t *_retval); \
  NS_METHOD GetCellAt(int32_t x, int32_t y, int32_t *row, nsITreeColumn * *col, nsAString & childElt); \
  NS_METHOD GetCoordsForCellItem(int32_t row, nsITreeColumn *col, const nsAString & element, int32_t *x, int32_t *y, int32_t *width, int32_t *height); \
  NS_METHOD IsCellCropped(int32_t row, nsITreeColumn *col, bool *_retval); \
  NS_METHOD RowCountChanged(int32_t index, int32_t count); \
  NS_METHOD BeginUpdateBatch(void); \
  NS_METHOD EndUpdateBatch(void); \
  NS_METHOD ClearStyleAndImageCaches(void); 
