...
#define NS_DECL_NON_VIRTUAL_NSITELEPHONYCALLINFO \
  NS_METHOD GetClientId(uint32_t *aClientId); \
  NS_METHOD GetCallIndex(uint32_t *aCallIndex); \
  NS_METHOD GetCallState(uint16_t *aCallState); \
  NS_METHOD GetDisconnectedReason(nsAString & aDisconnectedReason); \
  NS_METHOD GetNumber(nsAString & aNumber); \
  NS_METHOD GetNumberPresentation(uint16_t *aNumberPresentation); \
  NS_METHOD GetName(nsAString & aName); \
  NS_METHOD GetNamePresentation(uint16_t *aNamePresentation); \
  NS_METHOD GetIsOutgoing(bool *aIsOutgoing); \
  NS_METHOD GetIsEmergency(bool *aIsEmergency); \
  NS_METHOD GetIsConference(bool *aIsConference); \
  NS_METHOD GetIsSwitchable(bool *aIsSwitchable); \
  NS_METHOD GetIsMergeable(bool *aIsMergeable); 
