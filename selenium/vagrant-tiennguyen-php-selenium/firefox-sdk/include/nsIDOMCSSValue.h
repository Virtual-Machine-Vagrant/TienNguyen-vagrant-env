...
#define NS_DECL_NSIDOMCSSVALUE \
  ...
  NS_IMETHOD GetCssValueType(uint16_t *aCssValueType) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIDOMCSSVALUE \
  NS_METHOD GetCssText(nsAString & aCssText); \
  NS_METHOD SetCssText(const nsAString & aCssText); \
  NS_METHOD GetCssValueType(uint16_t *aCssValueType); 
...
#define NS_FORWARD_NSIDOMCSSVALUE(_to) \
  ...
  NS_IMETHOD GetCssValueType(uint16_t *aCssValueType) override { return _to GetCssValueType(aCssValueType); } 
...
#define NS_FORWARD_SAFE_NSIDOMCSSVALUE(_to) \
  ...
  NS_IMETHOD GetCssValueType(uint16_t *aCssValueType) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetCssValueType(aCssValueType); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsDOMCSSValue, nsIDOMCSSValue)
