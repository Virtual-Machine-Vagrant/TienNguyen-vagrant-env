...
class NS_NO_VTABLE nsIStructuredCloneContainer : public nsISupports {
 public:
  ...
  NS_IMETHOD InitFromBase64(const nsAString & aData, uint32_t aFormatVersion) = 0;
...
#define NS_DECL_NON_VIRTUAL_NSISTRUCTUREDCLONECONTAINER \
  NS_METHOD InitFromJSVal(JS::HandleValue aData, JSContext* cx); \
  NS_METHOD InitFromBase64(const nsAString & aData, uint32_t aFormatVersion); \
  NS_METHOD DeserializeToJsval(JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD DeserializeToVariant(JSContext* cx, nsIVariant * *_retval); \
  NS_METHOD GetDataAsBase64(nsAString & _retval); \
  NS_METHOD GetSerializedNBytes(uint64_t *aSerializedNBytes); \
  NS_METHOD GetFormatVersion(uint32_t *aFormatVersion); 
...
/* void initFromBase64 (in AString aData, in unsigned long aFormatVersion); */
NS_IMETHODIMP nsStructuredCloneContainer::InitFromBase64(const nsAString & aData, uint32_t aFormatVersion)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
