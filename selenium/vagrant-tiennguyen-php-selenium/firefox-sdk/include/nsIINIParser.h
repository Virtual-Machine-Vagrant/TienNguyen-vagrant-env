...
#define NS_DECL_NON_VIRTUAL_NSIINIPARSER \
  NS_METHOD GetSections(nsIUTF8StringEnumerator * *_retval); \
  NS_METHOD GetKeys(const nsACString & aSection, nsIUTF8StringEnumerator * *_retval); \
  NS_METHOD GetString(const nsACString & aSection, const nsACString & aKey, nsACString & _retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsINIParser, nsIINIParser)
...
#define NS_DECL_NSIINIPARSERWRITER \
  ...
  NS_IMETHOD WriteFile(nsIFile *aINIFile, uint32_t aFlags) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIINIPARSERWRITER \
  NS_METHOD SetString(const nsACString & aSection, const nsACString & aKey, const nsACString & aValue); \
  NS_METHOD WriteFile(nsIFile *aINIFile, uint32_t aFlags); 
...
#define NS_FORWARD_NSIINIPARSERWRITER(_to) \
  ...
  NS_IMETHOD WriteFile(nsIFile *aINIFile, uint32_t aFlags) override { return _to WriteFile(aINIFile, aFlags); } 
...
#define NS_FORWARD_SAFE_NSIINIPARSERWRITER(_to) \
  ...
  NS_IMETHOD WriteFile(nsIFile *aINIFile, uint32_t aFlags) override { return !_to ? NS_ERROR_NULL_POINTER : _to->WriteFile(aINIFile, aFlags); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsINIParserWriter, nsIINIParserWriter)
...
#define NS_DECL_NON_VIRTUAL_NSIINIPARSERFACTORY \
  NS_METHOD CreateINIParser(nsIFile *aINIFile, nsIINIParser * *_retval); 
