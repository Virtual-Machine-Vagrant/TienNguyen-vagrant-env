...
#define NS_DECL_NON_VIRTUAL_NSIGIOMIMEAPP \
  ...
  NS_METHOD GetCommand(nsACString & aCommand); \
  NS_METHOD GetExpectsURIs(int32_t *aExpectsURIs); \
  NS_METHOD GetSupportedURISchemes(nsIUTF8StringEnumerator * *aSupportedURISchemes); \
  NS_METHOD Launch(const nsACString & uri); \
  NS_METHOD SetAsDefaultForMimeType(const nsACString & mimeType); \
  NS_METHOD SetAsDefaultForFileExtensions(const nsACString & extensions); \
  NS_METHOD SetAsDefaultForURIScheme(const nsACString & uriScheme); 
...
#define NS_DECL_NON_VIRTUAL_NSIGIOSERVICE \
  NS_METHOD GetMimeTypeFromExtension(const nsACString & extension, nsACString & _retval); \
  NS_METHOD GetAppForURIScheme(const nsACString & aURIScheme, nsIGIOMimeApp * *_retval); \
  NS_METHOD GetAppForMimeType(const nsACString & mimeType, nsIGIOMimeApp * *_retval); \
  NS_METHOD CreateAppFromCommand(const nsACString & cmd, const nsACString & appName, nsIGIOMimeApp * *_retval); \
  NS_METHOD GetDescriptionForMimeType(const nsACString & mimeType, nsACString & _retval); \
  NS_METHOD ShowURI(nsIURI *uri); \
  NS_METHOD ShowURIForInput(const nsACString & uri); \
  NS_METHOD OrgFreedesktopFileManager1ShowItems(const nsACString & path); 
...
/* [noscript] void orgFreedesktopFileManager1ShowItems (in ACString path); */
NS_IMETHODIMP nsGIOService::OrgFreedesktopFileManager1ShowItems(const nsACString & path)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
