...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLESELECTABLE \
  NS_METHOD GetSelectedItems(nsIArray * *aSelectedItems); \
  NS_METHOD GetSelectedItemCount(uint32_t *aSelectedItemCount); \
  NS_METHOD GetSelectedItemAt(uint32_t index, nsIAccessible * *_retval); \
  NS_METHOD IsItemSelected(uint32_t index, bool *_retval); \
  NS_METHOD AddItemToSelection(uint32_t index); \
  NS_METHOD RemoveItemFromSelection(uint32_t index); \
  NS_METHOD SelectAll(bool *_retval); \
  NS_METHOD UnselectAll(void); 
