...
#define NS_DECL_NON_VIRTUAL_NSIEXPATSINK \
  NS_METHOD HandleStartElement(const char16_t * aName, const char16_t * *aAtts, uint32_t aAttsCount, uint32_t aLineNumber); \
  NS_METHOD HandleEndElement(const char16_t * aName); \
  NS_METHOD HandleComment(const char16_t * aCommentText); \
  NS_METHOD HandleCDataSection(const char16_t * aData, uint32_t aLength); \
  NS_METHOD HandleDoctypeDecl(const nsAString & aSubset, const nsAString & aName, const nsAString & aSystemId, const nsAString & aPublicId, nsISupports *aCatalogData); \
  NS_METHOD HandleCharacterData(const char16_t * aData, uint32_t aLength); \
  NS_METHOD HandleProcessingInstruction(const char16_t * aTarget, const char16_t * aData); \
  NS_METHOD HandleXMLDeclaration(const char16_t * aVersion, const char16_t * aEncoding, int32_t aStandalone); \
  NS_METHOD ReportError(const char16_t * aErrorText, const char16_t * aSourceText, nsIScriptError *aError, bool *_retval); 
