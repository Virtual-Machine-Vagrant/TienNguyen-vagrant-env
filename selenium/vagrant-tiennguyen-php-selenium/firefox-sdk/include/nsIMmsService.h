...
#define NS_DECL_NON_VIRTUAL_NSIMMSSERVICE \
  NS_METHOD GetMmsDefaultServiceId(uint32_t *aMmsDefaultServiceId); \
  NS_METHOD Send(uint32_t serviceId, JS::HandleValue parameters, nsIMobileMessageCallback *request); \
  NS_METHOD Retrieve(int32_t id, nsIMobileMessageCallback *request); \
  NS_METHOD SendReadReport(const nsAString & messageID, const nsAString & toAddress, const nsAString & iccId); 
