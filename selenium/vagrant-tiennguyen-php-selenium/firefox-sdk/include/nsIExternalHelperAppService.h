...
#define NS_DECL_NON_VIRTUAL_NSIEXTERNALHELPERAPPSERVICE \
  NS_METHOD DoContent(const nsACString & aMimeContentType, nsIRequest *aRequest, nsIInterfaceRequestor *aContentContext, bool aForceSave, nsIInterfaceRequestor *aWindowContext, nsIStreamListener * *_retval); \
  NS_METHOD ApplyDecodingForExtension(const nsACString & aExtension, const nsACString & aEncodingType, bool *_retval); 
...
#define NS_DECL_NON_VIRTUAL_NSPIEXTERNALAPPLAUNCHER \
  NS_METHOD DeleteTemporaryFileOnExit(nsIFile *aTemporaryFile); \
  NS_METHOD DeleteTemporaryPrivateFileWhenPossible(nsIFile *aTemporaryFile); 
...
#define NS_DECL_NON_VIRTUAL_NSIHELPERAPPLAUNCHER \
  NS_METHOD GetMIMEInfo(nsIMIMEInfo * *aMIMEInfo); \
  NS_METHOD GetSource(nsIURI * *aSource); \
  NS_METHOD GetSuggestedFileName(nsAString & aSuggestedFileName); \
  NS_METHOD SaveToDisk(nsIFile *aNewFileLocation, bool aRememberThisPreference); \
  NS_METHOD LaunchWithApplication(nsIFile *aApplication, bool aRememberThisPreference); \
  NS_METHOD SaveDestinationAvailable(nsIFile *aFile); \
  NS_METHOD SetWebProgressListener(nsIWebProgressListener2 *aWebProgressListener); \
  NS_METHOD GetTargetFile(nsIFile * *aTargetFile); \
  NS_METHOD GetTargetFileIsExecutable(bool *aTargetFileIsExecutable); \
  NS_METHOD GetTimeDownloadStarted(PRTime *aTimeDownloadStarted); \
