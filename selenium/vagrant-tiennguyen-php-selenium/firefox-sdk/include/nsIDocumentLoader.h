...
#define NS_DECL_NON_VIRTUAL_NSIDOCUMENTLOADER \
  NS_METHOD Stop(void); \
  NS_METHOD GetContainer(nsISupports * *aContainer); \
  NS_METHOD GetLoadGroup(nsILoadGroup * *aLoadGroup); \
  NS_METHOD GetDocumentChannel(nsIChannel * *aDocumentChannel); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsDocumentLoader, nsIDocumentLoader)
