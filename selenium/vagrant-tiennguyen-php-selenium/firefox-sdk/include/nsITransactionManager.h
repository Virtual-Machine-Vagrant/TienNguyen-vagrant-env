...
#define NS_DECL_NON_VIRTUAL_NSITRANSACTIONMANAGER \
  NS_METHOD DoTransaction(nsITransaction *aTransaction); \
  NS_METHOD UndoTransaction(void); \
  NS_METHOD RedoTransaction(void); \
  NS_METHOD Clear(void); \
  NS_METHOD ClearUndoStack(void); \
  NS_METHOD ClearRedoStack(void); \
  NS_METHOD BeginBatch(nsISupports *aData); \
  NS_METHOD EndBatch(bool aAllowEmpty); \
  NS_METHOD GetNumberOfUndoItems(int32_t *aNumberOfUndoItems); \
  NS_METHOD GetNumberOfRedoItems(int32_t *aNumberOfRedoItems); \
  NS_METHOD GetMaxTransactionCount(int32_t *aMaxTransactionCount); \
  NS_METHOD SetMaxTransactionCount(int32_t aMaxTransactionCount); \
  NS_METHOD BatchTopUndo(void); \
  NS_METHOD RemoveTopUndo(void); \
  NS_METHOD PeekUndoStack(nsITransaction * *_retval); \
  NS_METHOD PeekRedoStack(nsITransaction * *_retval); \
  NS_METHOD GetUndoList(nsITransactionList * *_retval); \
  NS_METHOD GetRedoList(nsITransactionList * *_retval); \
  NS_METHOD AddListener(nsITransactionListener *aListener); \
  NS_METHOD RemoveListener(nsITransactionListener *aListener); 
