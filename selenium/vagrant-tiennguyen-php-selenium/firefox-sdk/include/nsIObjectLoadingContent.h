...
#define NS_DECL_NON_VIRTUAL_NSIOBJECTLOADINGCONTENT \
  NS_METHOD GetActualType(nsACString & aActualType); \
  NS_METHOD GetDisplayedType(uint32_t *aDisplayedType); \
  NS_METHOD GetContentTypeForMIMEType(const nsACString & aMimeType, uint32_t *_retval); \
  NS_METHOD GetBaseURI(nsIURI * *aBaseURI); \
  NS_METHOD GetPluginInstance(nsNPAPIPluginInstance **aPluginInstance); \
  NS_METHOD HasNewFrame(nsIObjectFrame *aFrame); \
  NS_METHOD GetPrintFrame(nsIFrame * *_retval); \
  NS_METHOD PluginDestroyed(void); \
  NS_METHOD PluginCrashed(nsIPluginTag *pluginTag, const nsAString & pluginDumpID, const nsAString & browserDumpID, bool submittedCrashReport); \
  NS_METHOD PlayPlugin(void); \
  NS_METHOD Reload(bool aClearActivation); \
  NS_METHOD GetActivated(bool *aActivated); \
  NS_METHOD StopPluginInstance(void); \
  NS_METHOD SyncStartPluginInstance(void); \
  NS_METHOD AsyncStartPluginInstance(void); \
  NS_METHOD InitializeFromChannel(nsIRequest *request); \
  NS_METHOD GetSrcURI(nsIURI * *aSrcURI); \
  NS_METHOD GetPluginFallbackType(uint32_t *aPluginFallbackType); \
  NS_METHOD GetHasRunningPlugin(bool *aHasRunningPlugin); \
  NS_METHOD GetRunID(uint32_t *aRunID); 
