...
#define NS_DECL_NSIDISKSPACEWATCHER \
  NS_IMETHOD GetIsDiskFull(bool *aIsDiskFull) override; \
  NS_IMETHOD GetFreeSpace(uint64_t *aFreeSpace) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIDISKSPACEWATCHER \
  NS_METHOD GetIsDiskFull(bool *aIsDiskFull); \
  NS_METHOD GetFreeSpace(uint64_t *aFreeSpace); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIDISKSPACEWATCHER(_to) \
  NS_IMETHOD GetIsDiskFull(bool *aIsDiskFull) override { return _to GetIsDiskFull(aIsDiskFull); } \
  NS_IMETHOD GetFreeSpace(uint64_t *aFreeSpace) override { return _to GetFreeSpace(aFreeSpace); } 

/* Use this macro to declare functions that forward the behavior of this interface to another object in a safe way. */
#define NS_FORWARD_SAFE_NSIDISKSPACEWATCHER(_to) \
  NS_IMETHOD GetIsDiskFull(bool *aIsDiskFull) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetIsDiskFull(aIsDiskFull); } \
  NS_IMETHOD GetFreeSpace(uint64_t *aFreeSpace) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetFreeSpace(aFreeSpace); } 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsDiskSpaceWatcher, nsIDiskSpaceWatcher)
