...
#define NS_DECL_NON_VIRTUAL_NSILOGINMANAGERCRYPTO \
  NS_METHOD Encrypt(const nsAString & plainText, nsAString & _retval); \
  NS_METHOD Decrypt(const nsAString & cipherText, nsAString & _retval); \
  NS_METHOD GetUiBusy(bool *aUiBusy); \
  NS_METHOD GetIsLoggedIn(bool *aIsLoggedIn); \
  NS_METHOD GetDefaultEncType(uint32_t *aDefaultEncType); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsLoginManagerCrypto, nsILoginManagerCrypto)
