...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLSCRIPTELEMENT \
  NS_METHOD GetSrc(nsAString & aSrc); \
  NS_METHOD SetSrc(const nsAString & aSrc); \
  NS_METHOD GetAsync(bool *aAsync); \
  NS_METHOD SetAsync(bool aAsync); \
  NS_METHOD GetDefer(bool *aDefer); \
  NS_METHOD SetDefer(bool aDefer); \
  ...
  NS_METHOD GetEvent(nsAString & aEvent); \
  NS_METHOD SetEvent(const nsAString & aEvent); 
