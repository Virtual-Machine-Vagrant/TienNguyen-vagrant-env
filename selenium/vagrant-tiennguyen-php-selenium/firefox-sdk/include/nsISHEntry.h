...
#define NS_DECL_NON_VIRTUAL_NSISHENTRY \
  ...
  NS_METHOD GetIsSubFrame(bool *aIsSubFrame); \
  NS_METHOD SetURI(nsIURI *aURI); \
  NS_METHOD GetReferrerURI(nsIURI * *aReferrerURI); \
  NS_METHOD SetReferrerURI(nsIURI *aReferrerURI); \
  NS_METHOD GetReferrerPolicy(uint32_t *aReferrerPolicy); \
  NS_METHOD SetReferrerPolicy(uint32_t aReferrerPolicy); \
  NS_METHOD GetContentViewer(nsIContentViewer * *aContentViewer); \
  NS_METHOD SetContentViewer(nsIContentViewer *aContentViewer); \
  NS_METHOD GetSticky(bool *aSticky); \
  NS_METHOD SetSticky(bool aSticky); \
  NS_METHOD GetWindowState(nsISupports * *aWindowState); \
  NS_METHOD SetWindowState(nsISupports *aWindowState); \
  NS_METHOD GetViewerBounds(nsIntRect & bounds); \
  NS_METHOD SetViewerBounds(const nsIntRect & bounds); \
  NS_METHOD AddChildShell(nsIDocShellTreeItem *shell); \
  NS_METHOD ChildShellAt(int32_t index, nsIDocShellTreeItem * *_retval); \
  NS_METHOD ClearChildShells(void); \
  NS_METHOD GetRefreshURIList(nsISupportsArray * *aRefreshURIList); \
  NS_METHOD SetRefreshURIList(nsISupportsArray *aRefreshURIList); \
  NS_METHOD SyncPresentationState(void); \
  NS_METHOD SetTitle(const nsAString & aTitle); \
  NS_METHOD GetPostData(nsIInputStream * *aPostData); \
  NS_METHOD SetPostData(nsIInputStream *aPostData); \
  ...
  NS_METHOD GetParent(nsISHEntry * *aParent); \
  NS_METHOD SetParent(nsISHEntry *aParent); \
  ...
  NS_METHOD GetSaveLayoutStateFlag(bool *aSaveLayoutStateFlag); \
  NS_METHOD SetSaveLayoutStateFlag(bool aSaveLayoutStateFlag); \
  NS_METHOD GetExpirationStatus(bool *aExpirationStatus); \
  NS_METHOD SetExpirationStatus(bool aExpirationStatus); \
  NS_METHOD GetContentType(nsACString & aContentType); \
  NS_METHOD SetContentType(const nsACString & aContentType); \
  NS_METHOD GetURIWasModified(bool *aURIWasModified); \
  NS_METHOD SetURIWasModified(bool aURIWasModified); \
  NS_METHOD SetScrollPosition(int32_t x, int32_t y); \
  NS_METHOD GetScrollPosition(int32_t *x, int32_t *y); \
  NS_METHOD Create(nsIURI *URI, const nsAString & title, nsIInputStream *inputStream, nsILayoutHistoryState *layoutHistoryState, nsISupports *cacheKey, const nsACString & contentType, nsISupports *owner, uint64_t docshellID, bool dynamicCreation); \
  NS_METHOD Clone(nsISHEntry * *_retval); \
  NS_METHOD SetIsSubFrame(bool aFlag); \
  NS_METHOD GetAnyContentViewer(nsISHEntry * *ownerEntry, nsIContentViewer * *_retval); \
  NS_METHOD GetOwner(nsISupports * *aOwner); \
  NS_METHOD SetOwner(nsISupports *aOwner); \
  NS_METHOD GetStateData(nsIStructuredCloneContainer * *aStateData); \
  NS_METHOD SetStateData(nsIStructuredCloneContainer *aStateData); \
  NS_METHOD_(nsDocShellEditorData *) ForgetEditorData(void); \
  NS_METHOD_(void) SetEditorData(nsDocShellEditorData *aData); \
  NS_METHOD_(bool) HasDetachedEditor(void); \
  NS_METHOD IsDynamicallyAdded(bool *_retval); \
  NS_METHOD HasDynamicallyAddedChild(bool *_retval); \
  NS_METHOD GetDocshellID(uint64_t *aDocshellID); \
  NS_METHOD SetDocshellID(uint64_t aDocshellID); \
  NS_METHOD GetBFCacheEntry(nsIBFCacheEntry * *aBFCacheEntry); \
  NS_METHOD_(bool) HasBFCacheEntry(nsIBFCacheEntry *aEntry); \
  NS_METHOD AdoptBFCacheEntry(nsISHEntry *aEntry); \
  NS_METHOD AbandonBFCacheEntry(void); \
  NS_METHOD SharesDocumentWith(nsISHEntry *aEntry, bool *_retval); \
  NS_METHOD GetIsSrcdocEntry(bool *aIsSrcdocEntry); \
  ...
  NS_METHOD GetScrollRestorationIsManual(bool *aScrollRestorationIsManual); \
  NS_METHOD SetScrollRestorationIsManual(bool aScrollRestorationIsManual); 
...
/* attribute boolean scrollRestorationIsManual; */
NS_IMETHODIMP nsSHEntry::GetScrollRestorationIsManual(bool *aScrollRestorationIsManual)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
NS_IMETHODIMP nsSHEntry::SetScrollRestorationIsManual(bool aScrollRestorationIsManual)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSISHENTRYINTERNAL \
  NS_METHOD_(void) RemoveFromBFCacheAsync(void); \
  NS_METHOD_(void) RemoveFromBFCacheSync(void); \
  NS_METHOD GetLastTouched(uint32_t *aLastTouched); \
  NS_METHOD SetLastTouched(uint32_t aLastTouched); \
  NS_METHOD_(nsSHEntryShared *) GetSharedState(void); 
