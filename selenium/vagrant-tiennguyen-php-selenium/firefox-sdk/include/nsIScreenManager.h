...
#define NS_DECL_NON_VIRTUAL_NSISCREENMANAGER \
  NS_METHOD ScreenForRect(int32_t left, int32_t top, int32_t width, int32_t height, nsIScreen * *_retval); \
  NS_METHOD ScreenForId(uint32_t id, nsIScreen * *_retval); \
  NS_METHOD GetPrimaryScreen(nsIScreen * *aPrimaryScreen); \
  NS_METHOD GetNumberOfScreens(uint32_t *aNumberOfScreens); \
  NS_METHOD GetSystemDefaultScale(float *aSystemDefaultScale); \
  NS_METHOD ScreenForNativeWidget(void *nativeWidget, nsIScreen * *_retval); 
