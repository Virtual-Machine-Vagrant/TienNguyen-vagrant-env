...
#define NS_DECL_NON_VIRTUAL_NSIPREFSERVICE \
  NS_METHOD ReadUserPrefs(nsIFile *aFile); \
  NS_METHOD ResetPrefs(void); \
  NS_METHOD ResetUserPrefs(void); \
  NS_METHOD SavePrefFile(nsIFile *aFile); \
  NS_METHOD GetBranch(const char * aPrefRoot, nsIPrefBranch * *_retval); \
  NS_METHOD GetDefaultBranch(const char * aPrefRoot, nsIPrefBranch * *_retval); \
  NS_METHOD GetDirty(bool *aDirty); 
