...
#define NS_DECL_NON_VIRTUAL_NSISELECTIONCONTROLLER \
  NS_METHOD SetDisplaySelection(int16_t toggle); \
  NS_METHOD GetDisplaySelection(int16_t *_retval); \
  NS_METHOD GetSelection(int16_t type, nsISelection * *_retval); \
  NS_METHOD ScrollSelectionIntoView(int16_t type, int16_t region, int16_t flags); \
  NS_METHOD RepaintSelection(int16_t type); \
  NS_METHOD SetCaretEnabled(bool enabled); \
  NS_METHOD SetCaretReadOnly(bool readOnly); \
  NS_METHOD GetCaretEnabled(bool *_retval); \
  NS_METHOD GetCaretVisible(bool *aCaretVisible); \
  NS_METHOD SetCaretVisibilityDuringSelection(bool visibility); \
  NS_METHOD CharacterMove(bool forward, bool extend); \
  NS_METHOD PhysicalMove(int16_t direction, int16_t amount, bool extend); \
  NS_METHOD CharacterExtendForDelete(void); \
  NS_METHOD CharacterExtendForBackspace(void); \
  NS_METHOD WordMove(bool forward, bool extend); \
  NS_METHOD WordExtendForDelete(bool forward); \
  NS_METHOD LineMove(bool forward, bool extend); \
  NS_METHOD IntraLineMove(bool forward, bool extend); \
  NS_METHOD PageMove(bool forward, bool extend); \
  NS_METHOD CompleteScroll(bool forward); \
  NS_METHOD CompleteMove(bool forward, bool extend); \
  NS_METHOD ScrollPage(bool forward); \
  NS_METHOD ScrollLine(bool forward); \
  NS_METHOD ScrollCharacter(bool right); \
  NS_METHOD SelectAll(void); \
  NS_METHOD CheckVisibility(nsIDOMNode *node, int16_t startOffset, int16_t endOffset, bool *_retval); \
