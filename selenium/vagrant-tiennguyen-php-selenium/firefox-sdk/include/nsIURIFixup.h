...
#define NS_DECL_NON_VIRTUAL_NSIURIFIXUPINFO \
  NS_METHOD GetConsumer(nsISupports * *aConsumer); \
  NS_METHOD SetConsumer(nsISupports *aConsumer); \
  NS_METHOD GetPreferredURI(nsIURI * *aPreferredURI); \
  NS_METHOD GetFixedURI(nsIURI * *aFixedURI); \
  NS_METHOD GetKeywordProviderName(nsAString & aKeywordProviderName); \
  NS_METHOD GetKeywordAsSent(nsAString & aKeywordAsSent); \
  NS_METHOD GetFixupChangedProtocol(bool *aFixupChangedProtocol); \
  NS_METHOD GetFixupCreatedAlternateURI(bool *aFixupCreatedAlternateURI); \
  NS_METHOD GetOriginalInput(nsACString & aOriginalInput); 
...
#define NS_DECL_NON_VIRTUAL_NSIURIFIXUP \
  NS_METHOD CreateExposableURI(nsIURI *aURI, nsIURI * *_retval); \
  NS_METHOD CreateFixupURI(const nsACString & aURIText, uint32_t aFixupFlags, nsIInputStream * *aPostData, nsIURI * *_retval); \
  NS_METHOD GetFixupURIInfo(const nsACString & aURIText, uint32_t aFixupFlags, nsIInputStream * *aPostData, nsIURIFixupInfo * *_retval); \
  NS_METHOD KeywordToURI(const nsACString & aKeyword, nsIInputStream * *aPostData, nsIURIFixupInfo * *_retval); \
  NS_METHOD IsDomainWhitelisted(const nsACString & aDomain, uint32_t aDotPos, bool *_retval); 
