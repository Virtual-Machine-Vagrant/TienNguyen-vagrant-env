...
#define NS_DECL_NON_VIRTUAL_NSIPROPERTIES \
  NS_METHOD Get(const char * prop, const nsIID & iid, void **result); \
  NS_METHOD Set(const char * prop, nsISupports *value); \
  NS_METHOD Has(const char * prop, bool *_retval); \
  NS_METHOD Undefine(const char * prop); \
  NS_METHOD GetKeys(uint32_t *count, char * **keys); 
