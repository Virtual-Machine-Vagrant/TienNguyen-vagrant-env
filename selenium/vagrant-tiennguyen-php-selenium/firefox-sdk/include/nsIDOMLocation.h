...
#define NS_DECL_NON_VIRTUAL_NSIDOMLOCATION \
  ...
  NS_METHOD Reload(bool forceget); \
  NS_METHOD Replace(const nsAString & url); \
  NS_METHOD Assign(const nsAString & url); \
  NS_METHOD ToString(nsAString & _retval); \
  NS_METHOD ValueOf(nsIDOMLocation * *_retval); 
