...
#define NS_DECL_NON_VIRTUAL_NSISTACKFRAME \
  ...
  NS_METHOD GetLineNumber(int32_t *aLineNumber); \
  NS_METHOD GetColumnNumber(int32_t *aColumnNumber); \
  NS_METHOD GetSourceLine(nsACString & aSourceLine); \
  NS_METHOD GetAsyncCause(nsAString & aAsyncCause); \
  NS_METHOD GetAsyncCaller(nsIStackFrame * *aAsyncCaller); \
  NS_METHOD GetCaller(nsIStackFrame * *aCaller); \
  NS_METHOD GetFormattedStack(nsAString & aFormattedStack); \
