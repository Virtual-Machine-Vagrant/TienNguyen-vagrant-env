...
#define NS_DECL_NON_VIRTUAL_NSIXPCCONSTRUCTOR \
  NS_METHOD GetClassID(nsIJSCID * *aClassID); \
  NS_METHOD GetInterfaceID(nsIJSIID * *aInterfaceID); \
  NS_METHOD GetInitializer(char * *aInitializer); 
...
#define NS_DECL_NON_VIRTUAL_NSIXPCCOMPONENTS_UTILS \
  NS_METHOD ReportError(JS::HandleValue error, JSContext* cx); \
  NS_METHOD GetSandbox(nsIXPCComponents_utils_Sandbox * *aSandbox); \
  NS_METHOD EvalInSandbox(const nsAString & source, JS::HandleValue sandbox, JS::HandleValue version, const nsACString & filename, int32_t lineNo, JSContext* cx, uint8_t _argc, JS::MutableHandleValue _retval); \
  NS_METHOD GetSandboxAddonId(JS::HandleValue sandbox, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GetSandboxMetadata(JS::HandleValue sandbox, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD SetSandboxMetadata(JS::HandleValue sandbox, JS::HandleValue metadata, JSContext* cx); \
  NS_METHOD Import(const nsACString & aResourceURI, JS::HandleValue targetObj, JSContext* cx, uint8_t _argc, JS::MutableHandleValue _retval); \
  NS_METHOD IsModuleLoaded(const nsACString & aResourceURI, bool *_retval); \
  NS_METHOD Unload(const nsACString & registryLocation); \
  NS_METHOD ImportGlobalProperties(JS::HandleValue aPropertyList, JSContext* cx); \
  NS_METHOD GetWeakReference(JS::HandleValue obj, JSContext* cx, xpcIJSWeakReference * *_retval); \
  NS_METHOD ForceGC(void); \
  NS_METHOD ForceCC(nsICycleCollectorListener *aListener); \
  NS_METHOD FinishCC(void); \
  NS_METHOD CcSlice(int64_t budget); \
  NS_METHOD GetMaxCCSliceTimeSinceClear(int32_t *_retval); \
  NS_METHOD ClearMaxCCTime(void); \
  NS_METHOD ForceShrinkingGC(void); \
  NS_METHOD SchedulePreciseGC(ScheduledGCCallback *callback); \
  NS_METHOD SchedulePreciseShrinkingGC(ScheduledGCCallback *callback); \
  NS_METHOD UnlinkGhostWindows(void); \
  NS_METHOD GetJSTestingFunctions(JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD CallFunctionWithAsyncStack(JS::HandleValue function, nsIStackFrame *stack, const nsAString & asyncCause, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GetGlobalForObject(JS::HandleValue obj, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD IsProxy(JS::HandleValue vobject, JSContext* cx, bool *_retval); \
  NS_METHOD ExportFunction(JS::HandleValue vfunction, JS::HandleValue vscope, JS::HandleValue voptions, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD CreateObjectIn(JS::HandleValue vobj, JS::HandleValue voptions, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD MakeObjectPropsNormal(JS::HandleValue vobj, JSContext* cx); \
  NS_METHOD IsDeadWrapper(JS::HandleValue obj, bool *_retval); \
  NS_METHOD IsCrossProcessWrapper(JS::HandleValue obj, bool *_retval); \
  NS_METHOD GetCrossProcessWrapperTag(JS::HandleValue obj, nsACString & _retval); \
  NS_METHOD PermitCPOWsInScope(JS::HandleValue obj); \
  NS_METHOD RecomputeWrappers(JS::HandleValue vobj, JSContext* cx); \
  NS_METHOD SetWantXrays(JS::HandleValue vscope, JSContext* cx); \
  NS_METHOD ForcePermissiveCOWs(JSContext* cx); \
  NS_METHOD ForcePrivilegedComponentsForScope(JS::HandleValue vscope, JSContext* cx); \
  NS_METHOD GetComponentsForScope(JS::HandleValue vscope, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD Dispatch(JS::HandleValue runnable, JS::HandleValue scope, JSContext* cx); \
  NS_METHOD GetStrict(JSContext* cx, bool *aStrict); \
  NS_METHOD SetStrict(JSContext* cx, bool aStrict); \
  NS_METHOD GetWerror(JSContext* cx, bool *aWerror); \
  NS_METHOD SetWerror(JSContext* cx, bool aWerror); \
  NS_METHOD GetStrict_mode(JSContext* cx, bool *aStrict_mode); \
  NS_METHOD SetStrict_mode(JSContext* cx, bool aStrict_mode); \
  NS_METHOD GetIon(JSContext* cx, bool *aIon); \
  NS_METHOD SetIon(JSContext* cx, bool aIon); \
  NS_METHOD SetGCZeal(int32_t zeal, JSContext* cx); \
  NS_METHOD NukeSandbox(JS::HandleValue obj, JSContext* cx); \
  NS_METHOD BlockScriptForGlobal(JS::HandleValue global, JSContext* cx); \
  NS_METHOD UnblockScriptForGlobal(JS::HandleValue global, JSContext* cx); \
  NS_METHOD IsXrayWrapper(JS::HandleValue obj, bool *_retval); \
  NS_METHOD WaiveXrays(JS::HandleValue aVal, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD UnwaiveXrays(JS::HandleValue aVal, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GetClassName(JS::HandleValue aObj, bool aUnwrap, JSContext* cx, char * *_retval); \
  NS_METHOD GetDOMClassInfo(const nsAString & aClassName, nsIClassInfo * *_retval); \
  NS_METHOD GetIncumbentGlobal(JS::HandleValue callback, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GenerateXPCWrappedJS(JS::HandleValue obj, JS::HandleValue scope, JSContext* cx, nsISupports * *_retval); \
  NS_METHOD GetWatchdogTimestamp(const nsAString & aCategory, PRTime *_retval); \
  NS_METHOD GetJSEngineTelemetryValue(JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD CloneInto(JS::HandleValue value, JS::HandleValue scope, JS::HandleValue options, JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GetWebIDLCallerPrincipal(nsIPrincipal * *_retval); \
  NS_METHOD GetObjectPrincipal(JS::HandleValue obj, JSContext* cx, nsIPrincipal * *_retval); \
  NS_METHOD GetCompartmentLocation(JS::HandleValue obj, JSContext* cx, nsACString & _retval); \
  NS_METHOD SetAddonInterposition(const nsACString & addonId, nsIAddonInterposition *interposition, JSContext* cx); \
  NS_METHOD SetAddonCallInterposition(JS::HandleValue target, JSContext* cx); \
  NS_METHOD Now(double *_retval); 
...
/* void permitCPOWsInScope (in jsval obj); */
NS_IMETHODIMP nsXPCComponents_Utils::PermitCPOWsInScope(JS::HandleValue obj)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
...
#define NS_DECL_NON_VIRTUAL_NSIXPCCOMPONENTSBASE \
  NS_METHOD GetInterfaces(nsIXPCComponents_Interfaces * *aInterfaces); \
  NS_METHOD GetInterfacesByID(nsIXPCComponents_InterfacesByID * *aInterfacesByID); \
  NS_METHOD GetResults(nsIXPCComponents_Results * *aResults); \
  NS_METHOD IsSuccessCode(nsresult result, bool *_retval); 
...
#define NS_DECL_NON_VIRTUAL_NSIXPCCOMPONENTS \
  NS_METHOD GetClasses(nsIXPCComponents_Classes * *aClasses); \
  NS_METHOD GetClassesByID(nsIXPCComponents_ClassesByID * *aClassesByID); \
  NS_METHOD GetStack(nsIStackFrame * *aStack); \
  NS_METHOD GetManager(nsIComponentManager * *aManager); \
  NS_METHOD GetUtils(nsIXPCComponents_Utils * *aUtils); \
  NS_METHOD GetID(nsIXPCComponents_ID * *aID); \
  NS_METHOD GetException(nsIXPCComponents_Exception * *aException); \
  NS_METHOD GetConstructor(nsIXPCComponents_Constructor * *aConstructor); \
  NS_METHOD GetLastResult(JSContext* cx, JS::MutableHandleValue aLastResult); \
  NS_METHOD GetReturnCode(JSContext* cx, JS::MutableHandleValue aReturnCode); \
  NS_METHOD SetReturnCode(JSContext* cx, JS::HandleValue aReturnCode); \
  NS_METHOD ReportError(JS::HandleValue error, JSContext* cx); 
