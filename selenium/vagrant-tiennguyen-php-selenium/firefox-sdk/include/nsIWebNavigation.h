...
#define NS_DECL_NON_VIRTUAL_NSIWEBNAVIGATION \
  NS_METHOD GetCanGoBack(bool *aCanGoBack); \
  NS_METHOD GetCanGoForward(bool *aCanGoForward); \
  NS_METHOD GoBack(void); \
  NS_METHOD GoForward(void); \
  NS_METHOD GotoIndex(int32_t index); \
  NS_METHOD LoadURI(const char16_t * aURI, uint32_t aLoadFlags, nsIURI *aReferrer, nsIInputStream *aPostData, nsIInputStream *aHeaders); \
  NS_METHOD LoadURIWithOptions(const char16_t * aURI, uint32_t aLoadFlags, nsIURI *aReferrer, uint32_t aReferrerPolicy, nsIInputStream *aPostData, nsIInputStream *aHeaders, nsIURI *aBaseURI); \
  NS_METHOD Reload(uint32_t aReloadFlags); \
  NS_METHOD Stop(uint32_t aStopFlags); \
  NS_METHOD GetDocument(nsIDOMDocument * *aDocument); \
  NS_METHOD GetCurrentURI(nsIURI * *aCurrentURI); \
  NS_METHOD GetReferringURI(nsIURI * *aReferringURI); \
  NS_METHOD GetSessionHistory(nsISHistory * *aSessionHistory); \
  NS_METHOD SetSessionHistory(nsISHistory *aSessionHistory); 
