...
class NS_NO_VTABLE mozITXTToHTMLConv : public nsIStreamConverter {
 public:
  ...
  NS_IMETHOD ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval) = 0;

  /* wstring scanHTML (in wstring text, in unsigned long whattodo); */
  NS_IMETHOD ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval) = 0;

  /* unsigned long citeLevelTXT (in wstring line, out unsigned long logLineStart); */
  NS_IMETHOD CiteLevelTXT(const char16_t * line, uint32_t *logLineStart, uint32_t *_retval) = 0;

  /* void findURLInPlaintext (in wstring text, in long aLength, in long aPos, out long aStartPos, out long aEndPos); */
  NS_IMETHOD FindURLInPlaintext(const char16_t * text, int32_t aLength, int32_t aPos, int32_t *aStartPos, int32_t *aEndPos) = 0;
...
#define NS_DECL_MOZITXTTOHTMLCONV \
  NS_IMETHOD ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override; \
  NS_IMETHOD ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override; \
...
#define NS_DECL_NON_VIRTUAL_MOZITXTTOHTMLCONV \
  NS_METHOD ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval); \
  NS_METHOD ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval); \
  NS_METHOD CiteLevelTXT(const char16_t * line, uint32_t *logLineStart, uint32_t *_retval); \
  NS_METHOD FindURLInPlaintext(const char16_t * text, int32_t aLength, int32_t aPos, int32_t *aStartPos, int32_t *aEndPos); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_MOZITXTTOHTMLCONV(_to) \
  NS_IMETHOD ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override { return _to ScanTXT(text, whattodo, _retval); } \
  NS_IMETHOD ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override { return _to ScanHTML(text, whattodo, _retval); } \
...
#define NS_FORWARD_SAFE_MOZITXTTOHTMLCONV(_to) \
  NS_IMETHOD ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ScanTXT(text, whattodo, _retval); } \
  NS_IMETHOD ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->ScanHTML(text, whattodo, _retval); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(_MYCLASS_, mozITXTToHTMLConv)
...
/* wstring scanTXT (in wstring text, in unsigned long whattodo); */
NS_IMETHODIMP _MYCLASS_::ScanTXT(const char16_t * text, uint32_t whattodo, char16_t * *_retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

/* wstring scanHTML (in wstring text, in unsigned long whattodo); */
NS_IMETHODIMP _MYCLASS_::ScanHTML(const char16_t * text, uint32_t whattodo, char16_t * *_retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

/* unsigned long citeLevelTXT (in wstring line, out unsigned long logLineStart); */
NS_IMETHODIMP _MYCLASS_::CiteLevelTXT(const char16_t * line, uint32_t *logLineStart, uint32_t *_retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

/* void findURLInPlaintext (in wstring text, in long aLength, in long aPos, out long aStartPos, out long aEndPos); */
NS_IMETHODIMP _MYCLASS_::FindURLInPlaintext(const char16_t * text, int32_t aLength, int32_t aPos, int32_t *aStartPos, int32_t *aEndPos)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
