...
#define NS_DECL_NON_VIRTUAL_NSITRANSFER \
  NS_METHOD Init(nsIURI *aSource, nsIURI *aTarget, const nsAString & aDisplayName, nsIMIMEInfo *aMIMEInfo, PRTime startTime, nsIFile *aTempFile, nsICancelable *aCancelable, bool aIsPrivate); \
  NS_METHOD SetSha256Hash(const nsACString & aHash); \
  NS_METHOD SetSignatureInfo(nsIArray *aSignatureInfo); \
  NS_METHOD SetRedirects(nsIArray *aRedirects); 
