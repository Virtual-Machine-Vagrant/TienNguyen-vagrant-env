...
#define NS_DECL_NON_VIRTUAL_NSISMSSERVICE \
  NS_METHOD GetSmsDefaultServiceId(uint32_t *aSmsDefaultServiceId); \
  NS_METHOD GetSegmentInfoForText(const nsAString & text, nsIMobileMessageCallback *request); \
  NS_METHOD Send(uint32_t serviceId, const nsAString & number, const nsAString & message, bool silent, nsIMobileMessageCallback *request); \
  NS_METHOD AddSilentNumber(const nsAString & number); \
  NS_METHOD RemoveSilentNumber(const nsAString & number); \
  NS_METHOD GetSmscAddress(uint32_t serviceId, nsIMobileMessageCallback *request); \
  NS_METHOD SetSmscAddress(uint32_t serviceId, const nsAString & number, uint32_t typeOfNumber, uint32_t numberPlanIdentification, nsIMobileMessageCallback *request); 
