...
#define NS_DECL_NSIWEBPAGEDESCRIPTOR \
  NS_IMETHOD LoadPage(nsISupports *aPageDescriptor, uint32_t aDisplayType) override; \
...
#define NS_DECL_NON_VIRTUAL_NSIWEBPAGEDESCRIPTOR \
  NS_METHOD LoadPage(nsISupports *aPageDescriptor, uint32_t aDisplayType); \
  NS_METHOD GetCurrentDescriptor(nsISupports * *aCurrentDescriptor); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSIWEBPAGEDESCRIPTOR(_to) \
  NS_IMETHOD LoadPage(nsISupports *aPageDescriptor, uint32_t aDisplayType) override { return _to LoadPage(aPageDescriptor, aDisplayType); } \
...
#define NS_FORWARD_SAFE_NSIWEBPAGEDESCRIPTOR(_to) \
  NS_IMETHOD LoadPage(nsISupports *aPageDescriptor, uint32_t aDisplayType) override { return !_to ? NS_ERROR_NULL_POINTER : _to->LoadPage(aPageDescriptor, aDisplayType); } \
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsWebPageDescriptor, nsIWebPageDescriptor)
