...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLEEVENT \
  NS_METHOD GetEventType(uint32_t *aEventType); \
  NS_METHOD GetAccessible(nsIAccessible * *aAccessible); \
  NS_METHOD GetAccessibleDocument(nsIAccessibleDocument * *aAccessibleDocument); \
  NS_METHOD GetDOMNode(nsIDOMNode * *aDOMNode); \
  NS_METHOD GetIsFromUserInput(bool *aIsFromUserInput); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsAccessibleEvent, nsIAccessibleEvent)
