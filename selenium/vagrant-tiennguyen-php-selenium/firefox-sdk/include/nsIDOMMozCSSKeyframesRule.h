...
#define NS_DECL_NON_VIRTUAL_NSIDOMMOZCSSKEYFRAMESRULE \
  ...
  NS_METHOD AppendRule(const nsAString & rule); \
  NS_METHOD DeleteRule(const nsAString & key); \
  NS_METHOD FindRule(const nsAString & key, nsIDOMMozCSSKeyframeRule * *_retval); 
