...
#define NS_DECL_NON_VIRTUAL_NSIMENUBUILDER \
  NS_METHOD OpenContainer(const nsAString & aLabel); \
  NS_METHOD AddItemFor(nsIDOMHTMLMenuItemElement *aElement, bool aCanLoadIcon); \
  NS_METHOD AddSeparator(void); \
  NS_METHOD UndoAddSeparator(void); \
  NS_METHOD CloseContainer(void); \
  NS_METHOD ToJSONString(nsAString & _retval); \
  NS_METHOD Click(const nsAString & aGeneratedItemId); 
