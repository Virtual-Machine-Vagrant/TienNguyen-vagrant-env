...
#define NS_DECL_NON_VIRTUAL_NSIDOMSYSTEMMESSAGECALLBACK \
  NS_METHOD HandleMessage(JS::HandleValue message); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIDOMNAVIGATORSYSTEMMESSAGES \
  NS_METHOD MozSetMessageHandler(const nsAString & type, nsIDOMSystemMessageCallback *callback); \
  NS_METHOD MozHasPendingMessage(const nsAString & type, bool *_retval); \
  NS_METHOD MozSetMessageHandlerPromise(nsISupports *promise); \
  NS_METHOD MozIsHandlingMessage(bool *_retval); 
