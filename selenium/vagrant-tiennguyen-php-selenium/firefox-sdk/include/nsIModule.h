...
#define NS_DECL_NON_VIRTUAL_NSIMODULE \
  NS_METHOD GetClassObject(nsIComponentManager *aCompMgr, const nsCID & aClass, const nsIID & aIID, void **aResult); \
  NS_METHOD RegisterSelf(nsIComponentManager *aCompMgr, nsIFile *aLocation, const char * aLoaderStr, const char * aType); \
  NS_METHOD UnregisterSelf(nsIComponentManager *aCompMgr, nsIFile *aLocation, const char * aLoaderStr); \
  NS_METHOD CanUnload(nsIComponentManager *aCompMgr, bool *_retval); 
