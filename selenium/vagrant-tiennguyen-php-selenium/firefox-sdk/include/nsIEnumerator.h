...
#define NS_DECL_NON_VIRTUAL_NSIENUMERATOR \
  NS_METHOD First(void); \
  NS_METHOD Next(void); \
  NS_METHOD CurrentItem(nsISupports * *_retval); \
  NS_METHOD IsDone(void); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSIBIDIRECTIONALENUMERATOR \
  NS_METHOD Last(void); \
  NS_METHOD Prev(void); 
