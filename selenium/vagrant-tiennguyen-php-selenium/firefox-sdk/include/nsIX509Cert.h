...
#define NS_DECL_NON_VIRTUAL_NSIX509CERT \
  ...
  NS_METHOD GetUsagesArray(bool localOnly, uint32_t *verified, uint32_t *count, char16_t * **usages); \
  NS_METHOD RequestUsagesArrayAsync(nsICertVerificationListener *cvl); \
  NS_METHOD GetUsagesString(bool localOnly, uint32_t *verified, nsAString & usages); \
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSICERTVERIFICATIONRESULT \
  NS_METHOD GetUsagesArrayResult(uint32_t *verified, uint32_t *count, char16_t * **usages); 
...
/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSICERTVERIFICATIONLISTENER \
  NS_METHOD Notify(nsIX509Cert *verifiedCert, nsICertVerificationResult *result); 
