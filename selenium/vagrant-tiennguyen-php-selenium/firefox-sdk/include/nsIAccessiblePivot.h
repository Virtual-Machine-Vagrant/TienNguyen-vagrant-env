...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLEPIVOT \
  NS_METHOD GetPosition(nsIAccessible * *aPosition); \
  NS_METHOD SetPosition(nsIAccessible *aPosition); \
  NS_METHOD GetRoot(nsIAccessible * *aRoot); \
  NS_METHOD GetModalRoot(nsIAccessible * *aModalRoot); \
  NS_METHOD SetModalRoot(nsIAccessible *aModalRoot); \
  NS_METHOD GetStartOffset(int32_t *aStartOffset); \
  NS_METHOD GetEndOffset(int32_t *aEndOffset); \
  NS_METHOD SetTextRange(nsIAccessibleText *aTextAccessible, int32_t aStartOffset, int32_t aEndOffset, bool aIsFromUserInput, uint8_t _argc); \
  NS_METHOD MoveNext(nsIAccessibleTraversalRule *aRule, nsIAccessible *aAnchor, bool aIncludeStart, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MovePrevious(nsIAccessibleTraversalRule *aRule, nsIAccessible *aAnchor, bool aIncludeStart, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MoveFirst(nsIAccessibleTraversalRule *aRule, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MoveLast(nsIAccessibleTraversalRule *aRule, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MoveNextByText(TextBoundaryType aBoundary, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MovePreviousByText(TextBoundaryType aBoundary, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD MoveToPoint(nsIAccessibleTraversalRule *aRule, int32_t aX, int32_t aY, bool aIgnoreNoMatch, bool aIsFromUserInput, uint8_t _argc, bool *_retval); \
  NS_METHOD AddObserver(nsIAccessiblePivotObserver *aObserver); \
  NS_METHOD RemoveObserver(nsIAccessiblePivotObserver *aObserver); 
...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLEPIVOTOBSERVER \
  NS_METHOD OnPivotChanged(nsIAccessiblePivot *aPivot, nsIAccessible *aOldAccessible, int32_t aOldStart, int32_t aOldEnd, PivotMoveReason aReason, bool aIsFromUserInput); 
...
#define NS_DECL_NON_VIRTUAL_NSIACCESSIBLETRAVERSALRULE \
  NS_METHOD GetPreFilter(uint32_t *aPreFilter); \
  NS_METHOD GetMatchRoles(uint32_t **aRoles, uint32_t *aCount); \
  NS_METHOD Match(nsIAccessible *aAccessible, uint16_t *_retval); 
