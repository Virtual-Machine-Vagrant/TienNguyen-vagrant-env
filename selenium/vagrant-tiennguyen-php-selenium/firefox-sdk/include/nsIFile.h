...
#define NS_DECL_NON_VIRTUAL_NSIFILE \
  NS_METHOD Append(const nsAString & node); \
  NS_METHOD AppendNative(const nsACString & node); \
  NS_METHOD Normalize(void); \
  NS_METHOD Create(uint32_t type, uint32_t permissions); \
  NS_METHOD GetLeafName(nsAString & aLeafName); \
  NS_METHOD SetLeafName(const nsAString & aLeafName); \
  NS_METHOD GetNativeLeafName(nsACString & aNativeLeafName); \
  NS_METHOD SetNativeLeafName(const nsACString & aNativeLeafName); \
  NS_METHOD CopyTo(nsIFile *newParentDir, const nsAString & newName); \
  NS_METHOD CopyToNative(nsIFile *newParentDir, const nsACString & newName); \
  NS_METHOD CopyToFollowingLinks(nsIFile *newParentDir, const nsAString & newName); \
  NS_METHOD CopyToFollowingLinksNative(nsIFile *newParentDir, const nsACString & newName); \
  NS_METHOD MoveTo(nsIFile *newParentDir, const nsAString & newName); \
  NS_METHOD MoveToNative(nsIFile *newParentDir, const nsACString & newName); \
  NS_METHOD RenameTo(nsIFile *newParentDir, const nsAString & newName); \
  NS_METHOD RenameToNative(nsIFile *newParentDir, const nsACString & newName); \
  NS_METHOD Remove(bool recursive); \
  NS_METHOD GetPermissions(uint32_t *aPermissions); \
  NS_METHOD SetPermissions(uint32_t aPermissions); \
  NS_METHOD GetPermissionsOfLink(uint32_t *aPermissionsOfLink); \
  NS_METHOD SetPermissionsOfLink(uint32_t aPermissionsOfLink); \
  NS_METHOD GetLastModifiedTime(PRTime *aLastModifiedTime); \
  NS_METHOD SetLastModifiedTime(PRTime aLastModifiedTime); \
  NS_METHOD GetLastModifiedTimeOfLink(PRTime *aLastModifiedTimeOfLink); \
  NS_METHOD SetLastModifiedTimeOfLink(PRTime aLastModifiedTimeOfLink); \
  NS_METHOD GetFileSize(int64_t *aFileSize); \
  NS_METHOD SetFileSize(int64_t aFileSize); \
  NS_METHOD GetFileSizeOfLink(int64_t *aFileSizeOfLink); \
  NS_METHOD GetTarget(nsAString & aTarget); \
  NS_METHOD GetNativeTarget(nsACString & aNativeTarget); \
  NS_METHOD GetPath(nsAString & aPath); \
  NS_METHOD GetNativePath(nsACString & aNativePath); \
  NS_METHOD Exists(bool *_retval); \
  NS_METHOD IsWritable(bool *_retval); \
  NS_METHOD IsReadable(bool *_retval); \
  NS_METHOD IsExecutable(bool *_retval); \
  NS_METHOD IsHidden(bool *_retval); \
  NS_METHOD IsDirectory(bool *_retval); \
  NS_METHOD IsFile(bool *_retval); \
  NS_METHOD IsSymlink(bool *_retval); \
  NS_METHOD IsSpecial(bool *_retval); \
  NS_METHOD CreateUnique(uint32_t type, uint32_t permissions); \
  NS_METHOD Clone(nsIFile * *_retval); \
  NS_METHOD Equals(nsIFile *inFile, bool *_retval); \
  NS_METHOD Contains(nsIFile *inFile, bool *_retval); \
  NS_METHOD GetParent(nsIFile * *aParent); \
  NS_METHOD GetDirectoryEntries(nsISimpleEnumerator * *aDirectoryEntries); \
  NS_METHOD InitWithPath(const nsAString & filePath); \
  NS_METHOD InitWithNativePath(const nsACString & filePath); \
  NS_METHOD InitWithFile(nsIFile *aFile); \
  NS_METHOD GetFollowLinks(bool *aFollowLinks); \
  NS_METHOD SetFollowLinks(bool aFollowLinks); \
  NS_METHOD OpenNSPRFileDesc(int32_t flags, int32_t mode, PRFileDesc **_retval); \
  NS_METHOD OpenANSIFileDesc(const char * mode, FILE **_retval); \
  NS_METHOD Load(PRLibrary **_retval); \
  NS_METHOD GetDiskSpaceAvailable(int64_t *aDiskSpaceAvailable); \
  NS_METHOD AppendRelativePath(const nsAString & relativeFilePath); \
  NS_METHOD AppendRelativeNativePath(const nsACString & relativeFilePath); \
  NS_METHOD GetPersistentDescriptor(nsACString & aPersistentDescriptor); \
  NS_METHOD SetPersistentDescriptor(const nsACString & aPersistentDescriptor); \
  NS_METHOD Reveal(void); \
  NS_METHOD Launch(void); \
  NS_METHOD GetRelativeDescriptor(nsIFile *fromFile, nsACString & _retval); \
  NS_METHOD SetRelativeDescriptor(nsIFile *fromFile, const nsACString & relativeDesc); \
  NS_METHOD GetRelativePath(nsIFile *fromFile, nsACString & _retval); \
  NS_METHOD SetRelativePath(nsIFile *fromFile, const nsACString & relativeDesc); 
