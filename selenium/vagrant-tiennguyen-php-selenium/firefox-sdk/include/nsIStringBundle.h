...
#define NS_DECL_NON_VIRTUAL_NSISTRINGBUNDLE \
  NS_METHOD GetStringFromID(int32_t aID, char16_t * *_retval); \
  NS_METHOD GetStringFromName(const char16_t * aName, char16_t * *_retval); \
  NS_METHOD FormatStringFromID(int32_t aID, const char16_t * *params, uint32_t length, char16_t * *_retval); \
  NS_METHOD FormatStringFromName(const char16_t * aName, const char16_t * *params, uint32_t length, char16_t * *_retval); \
  NS_METHOD GetSimpleEnumeration(nsISimpleEnumerator * *_retval); 
...
#define NS_DECL_NON_VIRTUAL_NSISTRINGBUNDLESERVICE \
  NS_METHOD CreateBundle(const char * aURLSpec, nsIStringBundle * *_retval); \
  NS_METHOD CreateExtensibleBundle(const char * aRegistryKey, nsIStringBundle * *_retval); \
  NS_METHOD FormatStatusMessage(nsresult aStatus, const char16_t * aStatusArg, char16_t * *_retval); \
  NS_METHOD FlushBundles(void); 
