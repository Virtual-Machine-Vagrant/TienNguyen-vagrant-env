...
#define NS_DECL_NON_VIRTUAL_NSICERTBLOCKLIST \
  NS_METHOD RevokeCertByIssuerAndSerial(const char * issuer, const char * serialNumber); \
  NS_METHOD RevokeCertBySubjectAndPubKey(const char * subject, const char * pubKeyHash); \
  NS_METHOD SaveEntries(void); \
  NS_METHOD IsCertRevoked(const uint8_t *issuer, uint32_t issuer_length, const uint8_t *serial, uint32_t serial_length, const uint8_t *subject, uint32_t subject_length, const uint8_t *pubkey, uint32_t pubkey_length, bool *_retval); \
  NS_METHOD IsBlocklistFresh(bool *_retval); 
