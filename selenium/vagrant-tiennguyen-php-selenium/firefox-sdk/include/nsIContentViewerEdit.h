...
#define NS_DECL_NON_VIRTUAL_NSICONTENTVIEWEREDIT \
  ...
  NS_METHOD GetCopyable(bool *aCopyable); \
  NS_METHOD CopyLinkLocation(void); \
  NS_METHOD GetInLink(bool *aInLink); \
  NS_METHOD CopyImage(int32_t aCopyFlags); \
  NS_METHOD GetInImage(bool *aInImage); \
  NS_METHOD GetContents(const char * aMimeType, bool aSelectionOnly, nsAString & _retval); \
  NS_METHOD GetCanGetContents(bool *aCanGetContents); \
  NS_METHOD SetCommandNode(nsIDOMNode *aNode); 
