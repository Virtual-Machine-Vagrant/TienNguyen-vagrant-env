...
#define NS_DECL_NON_VIRTUAL_NSIIDNSERVICE \
  NS_METHOD ConvertUTF8toACE(const nsACString & input, nsACString & _retval); \
  NS_METHOD ConvertACEtoUTF8(const nsACString & input, nsACString & _retval); \
  NS_METHOD IsACE(const nsACString & input, bool *_retval); \
  NS_METHOD Normalize(const nsACString & input, nsACString & _retval); \
  NS_METHOD ConvertToDisplayIDN(const nsACString & input, bool *isASCII, nsACString & _retval); 
