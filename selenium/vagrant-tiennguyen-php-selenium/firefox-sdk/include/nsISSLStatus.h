...
#define NS_DECL_NON_VIRTUAL_NSISSLSTATUS \
  ...
  NS_METHOD GetSecretKeyLength(uint32_t *aSecretKeyLength); \
  NS_METHOD GetProtocolVersion(uint16_t *aProtocolVersion); \
  NS_METHOD GetIsDomainMismatch(bool *aIsDomainMismatch); \
  NS_METHOD GetIsNotValidAtThisTime(bool *aIsNotValidAtThisTime); \
  NS_METHOD GetIsUntrusted(bool *aIsUntrusted); \
  NS_METHOD GetIsExtendedValidation(bool *aIsExtendedValidation); 
