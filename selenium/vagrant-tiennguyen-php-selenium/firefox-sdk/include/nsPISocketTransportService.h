...
#define NS_DECL_NON_VIRTUAL_NSPISOCKETTRANSPORTSERVICE \
  ...
  NS_METHOD GetAutodialEnabled(bool *aAutodialEnabled); \
  NS_METHOD SetAutodialEnabled(bool aAutodialEnabled); \
  NS_METHOD GetSendBufferSize(int32_t *aSendBufferSize); \
  NS_METHOD GetOffline(bool *aOffline); \
  NS_METHOD SetOffline(bool aOffline); \
  NS_METHOD GetKeepaliveIdleTime(int32_t *aKeepaliveIdleTime); \
  NS_METHOD GetKeepaliveRetryInterval(int32_t *aKeepaliveRetryInterval); \
  NS_METHOD GetKeepaliveProbeCount(int32_t *aKeepaliveProbeCount); 
