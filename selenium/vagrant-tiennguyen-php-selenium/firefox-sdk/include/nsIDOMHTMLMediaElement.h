...
#define NS_DECL_NON_VIRTUAL_NSIDOMHTMLMEDIAELEMENT \
  NS_METHOD GetError(nsIDOMMediaError * *aError); \
  NS_METHOD GetSrc(nsAString & aSrc); \
  NS_METHOD SetSrc(const nsAString & aSrc); \
  NS_METHOD GetCurrentSrc(nsAString & aCurrentSrc); \
  NS_METHOD GetNetworkState(uint16_t *aNetworkState); \
  NS_METHOD GetPreload(nsAString & aPreload); \
  NS_METHOD SetPreload(const nsAString & aPreload); \
  NS_METHOD GetBuffered(nsIDOMTimeRanges * *aBuffered); \
  NS_METHOD Load(void); \
  NS_METHOD CanPlayType(const nsAString & type, nsAString & _retval); \
  NS_METHOD GetReadyState(uint16_t *aReadyState); \
  NS_METHOD GetSeeking(bool *aSeeking); \
  NS_METHOD GetCurrentTime(double *aCurrentTime); \
  NS_METHOD SetCurrentTime(double aCurrentTime); \
  NS_METHOD GetDuration(double *aDuration); \
  NS_METHOD GetPaused(bool *aPaused); \
  NS_METHOD GetDefaultPlaybackRate(double *aDefaultPlaybackRate); \
  NS_METHOD SetDefaultPlaybackRate(double aDefaultPlaybackRate); \
  NS_METHOD GetPlaybackRate(double *aPlaybackRate); \
  NS_METHOD SetPlaybackRate(double aPlaybackRate); \
  NS_METHOD GetMozPreservesPitch(bool *aMozPreservesPitch); \
  NS_METHOD SetMozPreservesPitch(bool aMozPreservesPitch); \
  NS_METHOD GetPlayed(nsIDOMTimeRanges * *aPlayed); \
  NS_METHOD GetSeekable(nsIDOMTimeRanges * *aSeekable); \
  NS_METHOD GetEnded(bool *aEnded); \
  NS_METHOD GetMozAutoplayEnabled(bool *aMozAutoplayEnabled); \
  NS_METHOD GetAutoplay(bool *aAutoplay); \
  NS_METHOD SetAutoplay(bool aAutoplay); \
  NS_METHOD GetLoop(bool *aLoop); \
  NS_METHOD SetLoop(bool aLoop); \
  NS_METHOD Play(void); \
  NS_METHOD Pause(void); \
  NS_METHOD GetControls(bool *aControls); \
  NS_METHOD SetControls(bool aControls); \
  NS_METHOD GetVolume(double *aVolume); \
  NS_METHOD SetVolume(double aVolume); \
  NS_METHOD GetMuted(bool *aMuted); \
  NS_METHOD SetMuted(bool aMuted); \
  NS_METHOD GetDefaultMuted(bool *aDefaultMuted); \
  NS_METHOD SetDefaultMuted(bool aDefaultMuted); \
  NS_METHOD GetMozAudioCaptured(bool *aMozAudioCaptured); \
  NS_METHOD MozGetMetadata(JSContext* cx, JS::MutableHandleValue _retval); \
  NS_METHOD GetMozFragmentEnd(double *aMozFragmentEnd); \
  NS_METHOD GetMozAudioChannelType(nsAString & aMozAudioChannelType); \
  NS_METHOD SetMozAudioChannelType(const nsAString & aMozAudioChannelType); \
  NS_METHOD_(bool) IsVideo(void); 
