...
#define NS_DECL_NON_VIRTUAL_NSICYCLECOLLECTORHANDLER \
  NS_METHOD NoteRefCountedObject(const nsACString & aAddress, uint32_t aRefCount, const nsACString & aObjectDescription); \
  NS_METHOD NoteGCedObject(const nsACString & aAddress, bool aMarked, const nsACString & aObjectDescription, const nsACString & aCompartmentAddress); \
  NS_METHOD NoteEdge(const nsACString & aFromAddress, const nsACString & aToAddress, const nsACString & aEdgeName); \
  NS_METHOD DescribeRoot(const nsACString & aAddress, uint32_t aKnownEdges); \
  NS_METHOD DescribeGarbage(const nsACString & aAddress); 
...
#define NS_DECL_NON_VIRTUAL_NSICYCLECOLLECTORLOGSINK \
  NS_METHOD Open(FILE **aGCLog, FILE **aCCLog); \
  NS_METHOD CloseGCLog(void); \
  NS_METHOD CloseCCLog(void); \
  NS_METHOD GetFilenameIdentifier(nsAString & aFilenameIdentifier); \
  NS_METHOD SetFilenameIdentifier(const nsAString & aFilenameIdentifier); \
  NS_METHOD GetProcessIdentifier(int32_t *aProcessIdentifier); \
  NS_METHOD SetProcessIdentifier(int32_t aProcessIdentifier); \
  NS_METHOD GetGcLog(nsIFile * *aGcLog); \
  NS_METHOD GetCcLog(nsIFile * *aCcLog); 
...
#define NS_DECL_NON_VIRTUAL_NSICYCLECOLLECTORLISTENER \
  NS_METHOD AllTraces(nsICycleCollectorListener * *_retval); \
  NS_METHOD GetWantAllTraces(bool *aWantAllTraces); \
  NS_METHOD GetDisableLog(bool *aDisableLog); \
  NS_METHOD SetDisableLog(bool aDisableLog); \
  NS_METHOD GetLogSink(nsICycleCollectorLogSink * *aLogSink); \
  NS_METHOD SetLogSink(nsICycleCollectorLogSink *aLogSink); \
  NS_METHOD GetWantAfterProcessing(bool *aWantAfterProcessing); \
  NS_METHOD SetWantAfterProcessing(bool aWantAfterProcessing); \
  NS_METHOD ProcessNext(nsICycleCollectorHandler *aHandler, bool *_retval); \
  NS_METHOD AsLogger(nsCycleCollectorLogger **_retval); 
