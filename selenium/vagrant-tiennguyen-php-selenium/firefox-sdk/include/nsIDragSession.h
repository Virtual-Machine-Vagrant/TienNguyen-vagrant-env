...
#define NS_DECL_NON_VIRTUAL_NSIDRAGSESSION \
  NS_METHOD GetCanDrop(bool *aCanDrop); \
  NS_METHOD SetCanDrop(bool aCanDrop); \
  NS_METHOD GetOnlyChromeDrop(bool *aOnlyChromeDrop); \
  NS_METHOD SetOnlyChromeDrop(bool aOnlyChromeDrop); \
  NS_METHOD GetDragAction(uint32_t *aDragAction); \
  NS_METHOD SetDragAction(uint32_t aDragAction); \
  NS_METHOD GetTargetSize(nsSize *aTargetSize); \
  NS_METHOD SetTargetSize(nsSize aTargetSize); \
  NS_METHOD GetNumDropItems(uint32_t *aNumDropItems); \
  NS_METHOD GetSourceDocument(nsIDOMDocument * *aSourceDocument); \
  NS_METHOD GetSourceNode(nsIDOMNode * *aSourceNode); \
  NS_METHOD GetDataTransfer(nsIDOMDataTransfer * *aDataTransfer); \
  NS_METHOD SetDataTransfer(nsIDOMDataTransfer *aDataTransfer); \
  NS_METHOD GetData(nsITransferable *aTransferable, uint32_t aItemIndex); \
  NS_METHOD IsDataFlavorSupported(const char * aDataFlavor, bool *_retval); \
  NS_METHOD UserCancelled(void); \
  NS_METHOD DragEventDispatchedToChildProcess(void); \
  NS_METHOD UpdateDragEffect(void); 
