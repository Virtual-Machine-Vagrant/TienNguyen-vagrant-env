...
use Form;
use Litecms\Portfolio\Http\Requests\CategoryAdminRequest;
...
class CategoryAdminController extends BaseController
{
    ...
    public function index(CategoryAdminRequest $request)
    {
        ...
        if ($request->wantsJson()) {
            $categories  = $this->repository
                ->setPresenter('\\Litecms\\Portfolio\\Repositories\\Presenter\\CategoryListPresenter')
        ...
        return $this->theme->of('portfolio::admin.category.index')->render();
    }
    ...
    {
        if (!$category->exists) {
            return response()->view('portfolio::admin.category.new', compact('category'));
        }

        Form::populate($category);
        return response()->view('portfolio::admin.category.show', compact('category'));
    }
    ...
    public function create(CategoryAdminRequest $request)
    {
        ...
        return response()->view('portfolio::admin.category.create', compact('category'));
    ...
    public function store(CategoryAdminRequest $request)
    {
        try {
            ...
            return response()->json([
                'message'  => trans('messages.success.updated', ['Module' => trans('portfolio::category.name')]),
                'code'     => 204,
                'redirect' => trans_url('/admin/portfolio/category/'.$category->getRouteKey())
            ], 201);
    ...
    {
        Form::populate($category);
        return  response()->view('portfolio::admin.category.edit', compact('category'));
    }
    ...
    public function destroy(CategoryAdminRequest $request, Category $category)
    {
        ...
        try {
            ...
            return response()->json([
                'message'  => trans('messages.success.deleted', ['Module' => trans('portfolio::category.name')]),
                'code'     => 202,
                'redirect' => trans_url('/admin/portfolio/category/0'),
            ], 202);
