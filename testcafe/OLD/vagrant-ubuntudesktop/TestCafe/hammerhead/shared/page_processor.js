(function() {
    ...
    var $ = null, JSProcessor = null, NativeMethods = null, SharedConst = null, Util = null, UrlUtil = null;
    if (isNode) {
    ...
    var hasAttr = function(el, attr) {
        if (isNode) return attr in el.attribs; else {
            for (var i = 0; i < el.attributes.length; i++) if (el.attributes[i].name === attr) return true;
            return false;
        }
    };
    var hasEventHandler = function(el) {
        if (isNode) {
            for (var attr in el.attribs) {
                if (EVENTS.indexOf(el.attribs[attr])) return true;
            }
            return false;
        } else {
            var attrs = el.attributes;
            for (var i = 0; i < attrs.length; i++) {
                if (EVENTS.indexOf(attrs[i])) return true;
            }
            return false;
        }
    };
    ...
    var processAutoComplete = function(el) {
        var storedUrlAttr = PageProc.getStoredAttrName("autocomplete"), processed = hasAttr(el, storedUrlAttr), attrValue = getAttr(el, processed ? storedUrlAttr : "autocomplete");
        if (!processed) setAttr(el, storedUrlAttr, attrValue || attrValue === "" ? attrValue : "none");
        setAttr(el, "autocomplete", "off");
    }, processJsAttr = function(el, attr, jsProtocol) {
        var storedUrlAttr = PageProc.getStoredAttrName(attr), processed = hasAttr(el, storedUrlAttr), attrValue = getAttr(el, processed ? storedUrlAttr : attr);
        var code = jsProtocol ? attrValue.replace(JAVASCRIPT_PROTOCOL_REG_EX, "") : attrValue, matches = code.match(HTML_STRING_REG_EX);
        var setAttributes = function(value, processedValue, processedAttrValue) {
            if (value !== processedValue) {
                if (!processed) setAttr(el, storedUrlAttr, attrValue);
        ...
        if (matches && jsProtocol) {
            ...
            var processedCode = PageProc.processScript(code, true), processedAttrValue = processedCode;
        ...
        for (var i = 0; i < EVENTS.length; i++) {
            ...
            if (attrValue) processJsAttr(el, EVENTS[i], JAVASCRIPT_PROTOCOL_REG_EX.test(attrValue));
        }
        ...
        var scriptType = getAttr(script, "type"), executableScriptTypesRegEx = /(application\/((x-)?ecma|(x-)?java)script)|(text\/)(javascript(1\.{0-5})?|((x-)?ecma|x-java|js|live)script)/, isExecutableScript = !scriptType || executableScriptTypesRegEx.test(scriptType);
        if (isExecutableScript) {
            var result = scriptContent, commentPrefix = "", commentPrefixMatch = result.match(HTML_COMMENT_PREFIX_REG_EX), commentPostfix = "", commentPostfixMatch = null, hasCDATA = CDATA_REG_EX.test(result);
            ...
            if (hasCDATA) result = "\n//<![CDATA[\n" + result + "//]]>";
            if (isNode) script.children[0].data = result; else script.text = result;
        }
    }, processStyleAttr = function(el, urlReplacer) {
        var style = getAttr(el, "style");
        if (style) setAttr(el, "style", PageProc.processStylesheet(style, urlReplacer));
    }, processStylesheetElement = function(el, urlReplacer) {
        if (isNode) {
            var contentChild = el.children.length ? el.children[0] : null;
            if (contentChild && contentChild.data && urlReplacer) contentChild.data = PageProc.processStylesheet(contentChild.data, urlReplacer, true);
        } else if (urlReplacer) el.innerHTML = PageProc.processStylesheet(el.innerHTML, urlReplacer, true);
    }, processTargetBlank = function(el) {
        var attrValue = getAttr(el, "target");
        attrValue = attrValue && attrValue.replace(/\s/g, "");
        if (attrValue === "_blank" || attrValue === "blank") setAttr(el, "target", "_self");
        ...
        if (urlReplacer && pattern.urlAttr) {
            ...
            if ((resourceUrl || resourceUrl === "") && !processedOnServer) {
                if (UrlUtil.isSupportedProtocol(resourceUrl) && !EMPTY_URL_REG_EX.test(resourceUrl)) {
                    var elTagName = getTagName(el).toLowerCase(), isIframe = elTagName === "iframe", isScript = elTagName === "script", resourceType = null, target = getAttr(el, "target");
                    if (isNode && IFRAME_FLAG_TAGS.indexOf(elTagName) !== -1 && target === "_parent") return;
                    if (isIframe || PageProc.isOpenLinkInIFrame(el, hasIFrameParent)) resourceType = UrlUtil.IFRAME; else if (isScript) resourceType = UrlUtil.SCRIPT;
                    var proxyUrl = resourceUrl ? urlReplacer(resourceUrl, resourceType) : "";
                    if (isIframe) {
                        var isRelativePath = !UrlUtil.parseUrl(resourceUrl).host;
                        if (!isRelativePath) {
                            var location = urlReplacer("/"), proxyUrlObj = UrlUtil.parseProxyUrl(location), originUrlObj = proxyUrlObj.originResourceInfo, originUrl = UrlUtil.formatUrl(originUrlObj);
                            if (!UrlUtil.sameOriginCheck(originUrl, resourceUrl)) {
                                var proxyHostname = UrlUtil.parseUrl(location).hostname;
                                proxyUrl = resourceUrl ? UrlUtil.getCrossDomainIframeProxyUrl(resourceUrl, proxyHostname, crossDomainPort, proxyUrlObj.jobInfo.uid, proxyUrlObj.jobInfo.ownerToken) : "";
                            }
                        }
                    }
                    setAttr(el, storedUrlAttr, resourceUrl);
                    if (elTagName === "img" && proxyUrl !== "") setAttr(el, pattern.urlAttr, UrlUtil.resolveUrlAsOrigin(resourceUrl, urlReplacer)); else setAttr(el, pattern.urlAttr, proxyUrl);
                }
            }
        }
    }, processUrlJsAttr = function(el, urlReplacer, pattern) {
        if (JAVASCRIPT_PROTOCOL_REG_EX.test(getAttr(el, pattern.urlAttr))) processJsAttr(el, pattern.urlAttr, true);
    };
    var SELECTORS = {
    ...
    var ELEMENT_PROCESSOR_PATTERNS = [ {
        ...
        urlAttr: "href",
        elementProcessors: [ processTargetBlank, processUrlAttrs, processUrlJsAttr ]
    }, {
        ...
        urlAttr: "manifest",
        elementProcessors: [ processUrlAttrs, processUrlJsAttr ]
    }, {
        ...
        urlAttr: "content",
        elementProcessors: [ processMetaElement ]
    }, {
        selector: SELECTORS.ALL,
        elementProcessors: [ processStyleAttr ]
    }, {
        selector: SELECTORS.IS_SCRIPT,
        elementProcessors: [ processScriptElement ]
    }, {
        selector: SELECTORS.IS_STYLE,
        elementProcessors: [ processStylesheetElement ]
    }, {
        selector: SELECTORS.IS_INPUT,
        elementProcessors: [ processAutoComplete ]
    }, {
        selector: SELECTORS.HAS_EVENT_HANDLER,
        elementProcessors: [ processEvtAttr ]
    }, {
        selector: SELECTORS.IS_SANDBOXED_IFRAME,
        elementProcessors: [ processSandboxedIframe ]
    } ];
        ...
        for (var i = 0; i < ELEMENT_PROCESSOR_PATTERNS.length; i++) {
            ...
            $all.filter(function() {
                ...
                if (!this[SharedConst.ELEMENT_PROCESSED_FLAG]) {
                    for (var j = 0; j < pattern.elementProcessors.length; j++) pattern.elementProcessors[j](this, replacer, pattern, crossDomainPort, hasIFrameParent);
                }
            });
        }
    };
    PageProc.processElement = function(el, urlReplacer) {
        ...
        for (var i = 0; i < ELEMENT_PROCESSOR_PATTERNS.length; i++) {
            ...
            if (pattern.selector(elementForSelectorCheck) && !isTestCafeElement(el)) {
                for (var j = 0; j < pattern.elementProcessors.length; j++) pattern.elementProcessors[j](el, urlReplacer, pattern);
            }
        }
    };
    ...
    PageProc.processScript = function(text, withoutHeader) {
        ...
        if (!JSProcessor.isDataScript(text) && !withoutHeader && text.indexOf(SharedConst.DOM_SANDBOX_OVERRIDE_DOM_METHOD_NAME) === -1) text = SCRIPT_HEADER + text;
    ...
    function replaceStylsheetUrls(css, processor) {
        return css.replace(CSS_URL_PROPERTY_VALUE_PATTERN, function() {
            var prefix = arguments[1] || arguments[10], openQuote = arguments[2] || arguments[5] || arguments[11] || arguments[14] || "", url = arguments[3] || arguments[6] || arguments[8] || arguments[12] || arguments[15], closeQuote = arguments[4] || arguments[7] || arguments[13] || arguments[16] || "", postfix = arguments[9] || "";
    ...
    PageProc.isOpenLinkInIFrame = function(el, hasIFrameParent) {
        var tagName = getTagName(el).toLowerCase(), target = getAttr(el, "target");
        hasIFrameParent = hasIFrameParent || Util.hasIFrameParent;
        if (target !== "_top") {
            var mustProcessTag = IFRAME_FLAG_TAGS.indexOf(tagName) !== -1, isNameTarget = target ? target[0] !== "_" : false;
            if (target === "_parent") return mustProcessTag && !isTopParentIFrame(el);
            if (mustProcessTag && (hasIFrameParent(el) || isNameTarget)) return true;
        }
        return false;
    };
})();